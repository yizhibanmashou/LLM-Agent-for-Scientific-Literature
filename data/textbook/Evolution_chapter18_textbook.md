...
## Evolution_chapter18_001 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches
...
## Evolution_chapter18_002 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / VARIANCE IN SHORT-TERM RESPONSE
...
> **Figure 18.1** · page 3 · source: `Evolution_chapter18`
>
> ![Figure 18.1](figures/Evolution_18.1.png)
>
...
## Evolution_chapter18_003 · VARIANCE IN SHORT-TERM RESPONSE / Expected Variance in Response Generated by Drift
...
caveats summarized in Table 13.2). Assuming $ b t \sim (0, \sigma b^2) $, the variance about the expected mean value is

**[推导 Derivation]**

$$
\mathrm{SE}\left(\widehat{h}^{2}-\widehat{h} {r}^{2}\right)=\sqrt{\left(\mathrm{SE}[\widehat{h}^{2}]\right)^{2}+\left(\mathrm{SE}[\widehat{h} {r}^{2}]\right)^{2}}
$$

Selection in a single direction without a control line. Equation 18.30a has an extra term, $ \sigma {b}^{2} $, accounting for the between-generation variation in environmental effects.

where the (design-specific) coefficients, A and $ B t $, are given in Table 18.7. If the number of reproducing individuals varies over time, then the tA term is replaced by the sum over time of the components in A, for example by $ \sum {i=1}^t (1/N i) $, or related expressions. Finally, recall (Equation 18.3) that for unidirectional selection without a control, the variance in response has an additional term, $ \sigma b^2 $, accounting for the between-generation environmental variation.
...
$$
\frac{p(1-p)}{2N {e}}\gg\frac{p(1-p)}{2}\frac{\bar{\tau}^{2}h^{2}}{n}
$$
...
$$
n\gg N {e}\left(\bar{\imath}^{2}h^{2}\right)
$$
...
$$
\frac{\sigma^{2}(\delta p)}{\sigma^{2}(\delta p)+(\Delta p)^{2}}\simeq\frac{1/N {e}}{1/N {e}+\bar{\imath}^{2}h^{2}/n}=\frac{1}{1+\bar{\imath}^{2}h^{2}N {e}/n}
$$

**[推导 Derivation]**

$$
\frac{\sigma^{2}(\delta p)}{\sigma^{2}(\delta p)+(\Delta p)^{2}}\simeq1-\bar{\imath}^{2}h^{2}N {e}/n,\quad\mathrm{w h e n}\quad n\gg N e
$$

For $ h^2 = 0.25 $, $ \bar{i} = 2.06 $, $ n = 100 $, and $ N e = 25 $, Equation 18.45c returns a value of 0.79, while the approximation given by Equation 18.45d yields 0.74. Hence, in this setting almost 80% of the expected squared allele-frequency change is due to drift.

Finally, a rough calculation suggests the conditions under which $ \sigma^2(\delta p) \gg (\Delta p)^2 $, i.e., when the variance in allele-frequency change from drift exceeds the square of the deterministic change from selection. Equation 5.2b shows that $ \Delta p \simeq sp(1 - p) $ for a very weakly selected additive locus, while Equation 5.21 yields $ s \simeq \bar{\imath} a / \sigma z $, where $ \bar{\imath} $ is the selection intensity on the trait, yielding $ \Delta p i \simeq (\bar{\imath} a i / \sigma z) p i (1 - p i) $. Recalling that $ \sigma^2(A i) = 2a i^2 p i (1 - p i) $ is the additive genetic variance associated with locus $ i $

**[推导 Derivation]**

$$
\begin{aligned}(\Delta p_i)^2 &\simeq (\bar{\imath}a_i/\sigma_z)^2p_i^2(1-p_i)^2 = \bar{\imath}^2p_i(1-p_i)\left(\frac{a_i^2p_i(1-p_i)}{\sigma_z^2}\right)\\ &= \bar{\imath}^2p_i(1-p_i)\left(\frac{\sigma^2(A_i)/2}{\sigma_z^2}\right) = \frac{\bar{\imath}^2p_i(1-p_i)}{2}h_i^2\end{aligned}
$$

where $ h {i}^{2} $ is the heritability due to locus i. Assuming that n loci underlying the trait contribute roughly the same to the overall heritability, $ h^{2} $, then

**[推导 Derivation]**

$$
\left(\Delta p {i}\right)^{2}\simeq\frac{p {i}(1-p {i})}{2}\frac{\bar{\imath}^{2}h^{2}}{n}
$$
...
Under pure drift, $ E(g_t) = 0 $, while under selection, $ E(g_t) $ is given by the breeder's equation, $ \sum_i^t h_i^2 S_i = th^2 S $, if the heritability and selection differential remain constant (given the usual caveats summarized in *[See Table 13.2 at the end of this section.]*). Assuming $ b_t \sim (0, \sigma_b^2) $, the variance about the expected mean value is
...
## Evolution_chapter18_004 · VARIANCE IN SHORT-TERM RESPONSE / Variance in Predicted Response Versus Variance in Actual Response
...
## Evolution_chapter18_005 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / REALIZED HERITABILITIES
...
While one can use this approach to estimate $ h^2 $, any complication in predicting response using the breeder's equation (*[See Table 13.2 at the end of this section.]*) will usually make $ \widehat{h}_r^2 $ a biased estimator. Turning this point around suggests that one test for the success of the breeder's equation is to compare how close realized heritabilities are to estimates based on resemblance between relatives in the unselected base population. If the breeder's equation generally provides an accurate model of selection response, we expect these two different estimates to be similar (i.e., within sampling error).
...
## Evolution_chapter18_006 · REALIZED HERITABILITIES / Estimators for Several Generations of Selection
...
## Evolution_chapter18_007 · REALIZED HERITABILITIES / Weighted Least-squares Estimates of Realized Heritability
...
$$
V_{ij}=\sigma_{e}(i,j)=\sigma\left[R_{C}(i),R_{C}(j)\right]
$$
...
## Evolution_chapter18_008 · REALIZED HERITABILITIES / Standard Errors of Realized Heritability Estimates
...
## Evolution_chapter18_009 · REALIZED HERITABILITIES / Power: Estimation of $ h^{2} $ from Relatives or Selection Response?
...
## Evolution_chapter18_010 · REALIZED HERITABILITIES / Empirical Versus Predicted Standard Errors
...
> **Figure 18.3** · page 13 · source: `Evolution_chapter18`
>
> ![Figure 18.3](figures/Evolution_18.3.png)
>
...
## Evolution_chapter18_011 · REALIZED HERITABILITIES / Realized Heritability With Rank Data
...
## Evolution_chapter18_012 · REALIZED HERITABILITIES / Infinitesimal-model Corrections for Disequilibrium
...
> **Figure 18.4** · page 15 · source: `Evolution_chapter18`
>
> ![Figure 18.4](figures/Evolution_18.4.png)
>
...
Here $ h^2 $ is the (unselected) base population heritability and $ \kappa $ is a measure of the reduction in phenotypic variance due to selection (the variance following selection being $ (1 - \kappa)\sigma_z^2 $, Equation 16.10a). In particular, $ \kappa = \tilde{\tau}(\tilde{\tau} - z_{[1-p]}) $ for truncation selection that saves a fraction, $ p $, of the population for a normal trait (*[See Table 16.1 at the end of this section.]*). One can thus approximately treat the realized heritability as an estimate of the equilibrium heritability $ \tilde{h}_r^2 \simeq \tilde{h}^2 $, using Equation 18.21 to numerically solve for the base population heritability $ (h^2) $, an approach first used by Atkins and Thompson (1986). Figure 18.4 shows the relative percentage by which the base population heritability is underestimated by the uncorrected realized heritability.
...
## Evolution_chapter18_014 · EXPERIMENTAL EVALUATION OF THE BREEDER'S EQUATION / Most Traits Respond to Selection
...
*[See Table 18.1 at the end of this section.]* genetic variation) for specific combinations of traits, despite significant heritabilities in each component. In such settings, selection on only a specific trait results in a response, while selection on that trait as part of a multivariate selection index may not yield any response, or even may yield a response in the opposite direction of selection on that trait (Volume 3).

**[Table]**

*[See Table 18.1 at the end of this section.]*

> **Table 18.1** · `18.1` · page 17 · source: `Evolution_chapter18_014`
...
## Evolution_chapter18_015 · EXPERIMENTAL EVALUATION OF THE BREEDER'S EQUATION / Sheridan's Analysis
...
> **Table 18.2** · `18.2` · page 18 · source: `Evolution_chapter18_015`
...
> **Table 18.3** · `18.3` · page 18 · source: `Evolution_chapter18_015`
...
Finally, $ N_{e} $ decreases as selection intensity increases (Chapter 3 and 26), thus increasing the amount of inbreeding and (generally) reducing the additive variance (Chapter 11). When inbreeding occurs, the short-term response is generally found to be less than that predicted from the breeder's equation when using variance components estimated from the base population (*[See Table 18.5 at the end of this section.]*). This is expected, as inbreeding (generally) decreases the additive genetic variance within a line (Chapter 11), thus reducing response. Exceptions can occur if nonadditive genetic variance is present, in which case inbreeding may actually result in an increase in the within-line additive variance (Chapter 11), increasing response. The effects of drift on long-term experiments are examined in detail in Chapter 26. Inbreeding depression is another complication, which we will address shortly.

**[Table]**

*[See Table 18.6 at the end of this section.]*

**[Table]**

*[See Table 18.4 at the end of this section.]*

**[Table]**

*[See Table 18.5 at the end of this section.]*

> **Table 18.4** · `18.4` · page 19 · source: `Evolution_chapter18_016`
...
> **Table 18.5** · `18.5` · page 19 · source: `Evolution_chapter18_016`
...
> **Table 18.6** · `18.6` · page 21 · source: `Evolution_chapter18_016`
...
## Evolution_chapter18_017 · EXPERIMENTAL EVALUATION OF THE BREEDER'S EQUATION / Asymmetric Selection Response
...
> **Figure 18.5** · page 20 · source: `Evolution_chapter18`
>
> ![Figure 18.5](figures/Evolution_18.5.png)
>
...
> **Figure 18.7** · page 22 · source: `Evolution_chapter18`
>
> ![Figure 18.7](figures/Evolution_18.7.png)
>
...
> **Figure 18.8** · page 24 · source: `Evolution_chapter18`
>
> ![Figure 18.8](figures/Evolution_18.8.png)
>
...
## Evolution_chapter18_018 · EXPERIMENTAL EVALUATION OF THE BREEDER'S EQUATION / Reversed Response
...
> **Figure 18.9** · page 25 · source: `Evolution_chapter18`
>
> ![Figure 18.9](figures/Evolution_18.9.png)
>
...
## Evolution_chapter18_019 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / CONTROL POPULATIONS AND EXPERIMENTAL DESIGNS
...
## Evolution_chapter18_020 · CONTROL POPULATIONS AND EXPERIMENTAL DESIGNS / Basic Theory of Control Populations
...
## Evolution_chapter18_021 · CONTROL POPULATIONS AND EXPERIMENTAL DESIGNS / Divergent Selection Designs
...
## Evolution_chapter18_022 · CONTROL POPULATIONS AND EXPERIMENTAL DESIGNS / Variance in Response
...
*[See Table 18.7 at the end of this section.]*

> **Table 18.7** · `18.7` · page 29 · source: `Evolution_chapter18_022`
...
## Evolution_chapter18_023 · CONTROL POPULATIONS AND EXPERIMENTAL DESIGNS / Control Populations and Variance in Response

When does using a control population in a undirectional selection experiment reduce the variance in response? Equation 18.30a, along with the coefficients from *[See Table 18.7 at the end of this section.]*, gives the expected variance with and without the use of a control. Assuming $ M = M_s = M_c $ and $ N = N_s = N_c $, this equation yields
...
*[See Table 18.8 at the end of this section.]*

> **Table 18.8** · `18.8` · page 30 · source: `Evolution_chapter18_023`
...
## Evolution_chapter18_024 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / OPTIMAL EXPERIMENTAL DESIGNS
...
## Evolution_chapter18_025 · OPTIMAL EXPERIMENTAL DESIGNS / Nicholas' Criterion
...
## Evolution_chapter18_026 · OPTIMAL EXPERIMENTAL DESIGNS / Replicate Lines
...
If the number sampled and number used as parents within a replicate are $ M^* = M/r $ and $ N^* = N/r $, respectively, then it is easily seen from *[See Table 18.8 at the end of this section.]* that the variance of a replicate line is simply r times the variance of a population with N and M. Hence, variance in the sample mean from r replicate lines with $ N^* $ and $ M^* $ is the same as the variance with a single line with N and M, provided that the number of individuals within each replicate line is sufficiently large to avoid significant inbreeding. Richardson et al. (1968), Irgang et al. (1985), and Muir (1986a, 1986b) developed regression approaches that correct for between-generation environmental changes when replicate lines are used.

---

## Evolution_chapter18_027 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / LINE-CROSS ANALYSIS OF SELECTION EXPERIMENTS
...
## Evolution_chapter18_028 · LINE-CROSS ANALYSIS OF SELECTION EXPERIMENTS / The Simple Additive Model
...
## Evolution_chapter18_029 · LINE-CROSS ANALYSIS OF SELECTION EXPERIMENTS / The Hammond-Gardner Model
...
appears as a quadratic function of cycle number, with terms of the form $ k^{2}D_{q} $ for cycle k and $ k\dot{j}D_{q} $ for crosses between lines from cycles j and k. This is perhaps the most interesting dominance term, as it measures both inbreeding depression and the expected heterosis when two lines are crossed. As discussed below, $ D_{q} $ has also been interpreted as the component of change in the mean due to the effects of drift, with $ A + D $ representing the response from selection (the expected deterministic change). Recall (LW Chapter 10) that the heterosis, H, between two lines
...
## Evolution_chapter18_030 · LINE-CROSS ANALYSIS OF SELECTION EXPERIMENTS / Accounting for Inbreeding Depression and Drift
...
*(See Example 18.11.)* *(See Example 18.12.)*
