...
## Evolution_appendix3_001 · Appendix: Introduction
...
## Evolution_appendix3_002 · Appendix: Introduction / MONTE CARLO INTEGRATION
...
$$
\int_{a}^{b}h(x)dx=E_{p(x)}[f(x)]\simeq\frac{1}{n}\sum_{i=1}^{n}f(x_{i})
$$

This approach is referred to as Monte Carlo integration, and it can be used to approximate posterior (or marginal posterior) distributions required for a Bayesian analysis. Consider the integral $ I(y) = \int f(y \mid x) p(x) \, dx $, which we approximate by

$$
\widehat{I}(y)=\frac{1}{n}\sum_{i=1}^{n}f(y\mid x_{i})
$$
...
$$
\mathrm{SE}^{2}[\widehat{I}(y)]=\frac{1}{n}\left[\frac{1}{n-1}\sum_{i=1}^{n}\left(f(y\mid x_{i})-\widehat{I}(y)\right)^{2}\right]
$$
...
## Evolution_appendix3_003 · MONTE CARLO INTEGRATION / Importance Sampling
...
$$
\int f(x)q(x)dx\simeq\frac{1}{n}\sum_{i=1}^{n}f(x_{i})\left(\frac{q(x_{i})}{p(x_{i})}\right)
$$
...
$$
J(y)\simeq\frac{1}{n}\sum_{i=1}^{n}f(y\mid x_{i})\left(\frac{q(x_{i})}{p(x_{i})}\right)
$$
...
$$
C^{-1}=\int q(x)d x\simeq\frac{1}{n}\sum_{i=1}^{n}w_{i}\quad\mathrm{w h e r e}\quad w_{i}=\frac{q(x_{i})}{p(x_{i})}
$$

with the $ x_{i} $ drawn from $ p(x) $. Hence

$$
C\cdot\int f(x)q(x)dx\simeq\widehat{I}=\sum_{i=1}^{n}w_{i}f(x_{i})\bigg/\sum_{i=1}^{n}w_{i}
$$

which has an associated Monte Carlo variance of

$$
\mathrm{Var}\left(\widehat{I}\right)=\sum_{i=1}^{n}w_{i}\left[f(x_{i})-\widehat{I}\right]^{2}\bigg/\sum_{i=1}^{n}w_{i}
$$

---

## Evolution_appendix3_004 · Appendix: Introduction / INTRODUCTION TO MARKOV CHAINS
...
$$
\Pr(X_{t+1}=s_{j}\mid X_{0}=s_{k},\cdots,X_{t}=s_{i})=\Pr(X_{t+1}=s_{j}\mid X_{t}=s_{i})
$$
...
$$
P(i,j)=P(i\rightarrow j)=\Pr(X_{t+1}=s_{j}\mid X_{t}=s_{i})
$$

We use the notation $ P(i \to j) $ to imply a move from $ i $ to $ j $, as some texts define $ P(i,j) = P(j \to i) $, so we will use the arrow notation to avoid any potential confusion. Let

$$
\pi_{j}(t)=\Pr(X_{t}=s_{j})
$$
...
$$
\begin{aligned}\pi_{j}(t+1)&=\Pr(X_{t+1}=s_{j})\\&=\sum_{i}\Pr(X_{t+1}=s_{j}\mid X_{t}=s_{i})\cdot\Pr(X_{t}=s_{i})\\&=\sum_{i}P(i\rightarrow j)\pi_{i}(t)=\sum_{i}P(i,j)\pi_{i}(t)\end{aligned}
$$
...
$$
\boldsymbol{\pi}(t)=\boldsymbol{\pi}(t-1)\mathbf{P}=\left[\boldsymbol{\pi}(t-2)\mathbf{P}\right]\mathbf{P}=\boldsymbol{\pi}(t-2)\mathbf{P}^{2}
$$
...
$$
p_{ij}^{(n)}=\Pr(X_{t+n}=s_{j}\mid X_{t}=s_{i})
$$
...
$$
\pi_{t}(y)=\int\pi_{t-1}(x)P(x,y)dx
$$
...
## Evolution_appendix3_005 · Appendix: Introduction / THE METROPOLIS-HASTINGS ALGORITHM
...
## Evolution_appendix3_006 · THE METROPOLIS-HASTINGS ALGORITHM / Burning-in the Sampler
...
## Evolution_appendix3_007 · THE METROPOLIS-HASTINGS ALGORITHM / Simulated Annealing
...
$$
\alpha_{S A}=\min\left[1,\left(\frac{f(\theta^{*})}{f(\theta_{t-1})}\right)^{1/T(t)}\right]=\alpha^{1/T(t)}
$$
...
$$
T(t)=\max\left(T_{0}\left(\frac{T_{f}}{T_{0}}\right)^{t/n},T_{f}\right)
$$

In particular, to cool down to Metropolis sampling (by step n), we set $ T_{f} = 1 $ and the cooling schedule becomes

$$
T(t)=\max\left(T_{0}^{1-t/n},1\right)
$$

---

## Evolution_appendix3_008 · THE METROPOLIS-HASTINGS ALGORITHM / Choosing a Jumping (Proposal) Distribution
...
## Evolution_appendix3_009 · THE METROPOLIS-HASTINGS ALGORITHM / Autocorrelation and Sample Size Inflation
...
$$
\begin{align*}\widehat{\rho}_k=\frac{\mathrm{Cov}(\theta_t,\theta_{t+k})}{\mathrm{Var}(\theta_t)}=\frac{\sum\limits_{t=1}^{n-k}\left(\theta_t-\overline{\theta}\right)\left(\theta_{t+k}-\overline{\theta}\right)}{\sum\limits_{t=1}^{n-k}\left(\theta_t-\overline{\theta}\right)^2},\quad\textrm{with}\quad\overline{\theta}=\frac{1}{n}\sum\limits_{t=1}^n\theta_t\end{align*}
$$
...
Some indication of the required sample size comes from the theory of a first-order autoregressive process $ (AR_{1}) $, where

$$
\theta_{t}=\mu+\alpha\big(\theta_{t-1}-\mu\big)+\epsilon
$$

where $ |\alpha| < 1 $ and $ \epsilon $ is white noise, namely, $ \epsilon \sim N(0, \sigma^2) $. Here $ \rho_1 = \alpha $ and the $ k $th order autocorrelation is given by $ \rho_k = \rho_1^k = \alpha^k $. Under this process, $ E(\bar{\theta}) = \mu $ with a standard error of

$$
\mathrm{SE}\left(\overline{\theta}\right)=\frac{\sigma}{\sqrt{n}}\sqrt{\frac{1+\rho}{1-\rho}}
$$
...
## Evolution_appendix3_010 · THE METROPOLIS-HASTINGS ALGORITHM / The Monte Carlo Variance of a MCMC-Based Estimate
...
$$
\widehat{h}=\frac{1}{n}\sum_{i=1}^{n}h(\theta_{i})
$$
...
$$
\mathrm{Var}\left(\widehat{h}\right)=\frac{1}{c-1}\sum_{j=1}^{c}\left(\widehat{h}_{j}-\widehat{h^{*}}\right)^{2}\quad\mathrm{w h e r e}\quad\widehat{h^{*}}=\frac{1}{c}\sum_{j=1}^{c}\widehat{h}_{j}
$$
...
$$
\widehat{\gamma}(k)=\frac{1}{n-1}\sum_{i=1}^{n-k}\left[\left(h(\theta_{i})-\widehat{h}\right)\left(h(\theta_{i+k})-\widehat{h}\right)\right]
$$
...
$$
\mathrm{Var}\left(\widehat{h}\right)=\frac{1}{n}\left(\widehat{\gamma}(0)+2\sum_{i=1}^{2\delta+1}\widehat{\gamma}(i)\right)
$$
...
$$
\widehat{n}=\frac{\widehat{\gamma}(0)}{\operatorname{Var}\left(\widehat{h}\right)}
$$
...
## Evolution_appendix3_011 · Appendix: Introduction / CONVERGENCE DIAGNOSTICS
...
## Evolution_appendix3_012 · CONVERGENCE DIAGNOSTICS / Visual Analysis
...
> **Figure A3.1** · page 8 · source: `Evolution_appendix3`
>
> ![Figure A3.1](figures/Evolution_A3.1.png)
>
...
> **Figure A3.2** · page 13 · source: `Evolution_appendix3`
>
> ![Figure A3.2](figures/Evolution_A3.2.png)
>
...
## Evolution_appendix3_013 · CONVERGENCE DIAGNOSTICS / More Formal Approaches
...
$$
B=\frac{n}{m-1}\sum_{i=1}^{m}(\overline{\theta}_{i.}-\overline{\theta}_{..})^{2}
$$

be the among-chain variance, where $ \bar{\theta}_i $. is the mean parameter value for chain $ i $, and $ \bar{\theta}_i $. is the grand mean over all chains. Likewise, for the within-chain variance, we define

$$
W=\frac{1}{m}\left(\frac{1}{n-1}\sum_{i=1}^{m}\sum_{j=1}^{n}(\overline{\theta}_{ij}-\overline{\theta}_{i.})^{2}\right)
$$
...
where $ \lambda_1 $ is the leading eigenvalue of the matrix product $ W^{-1}B/n $, and where W and B are the sample within- and among-covariance matrices for the parameters. Again, a value near 1 is consistent with convergence.
...
## Evolution_appendix3_014 · CONVERGENCE DIAGNOSTICS / Practical MCMC: How Many Chains and How Long Should They Run?
...
## Evolution_appendix3_015 · Appendix: Introduction / THE GIBBS SAMPLER
...
To introduce the Gibbs sampler, consider a bivariate random variable $ (x, y) $, and suppose we wish to compute one or both of the marginals, $ p(x) $ and $ p(y) $. The idea behind the sampler is that it is far easier to consider a sequence of conditional distributions, $ p(x \mid y) $ and $ p(y \mid x) $, than it is to obtain the marginal by integration of the joint density $ p(x, y) $, namely, $ p(x) = \int p(x, y) \, dy $. The sampler starts with some initial value, $ y_0 $, for y and obtains $ x_0 $ by generating a random variable from the conditional distribution $ p(x \mid y = y_0) $. The sampler then uses $ x_0 $ to generate a new value of $ y_1 $, drawing from the conditional distribution based on the value of $ x_0 $, $ p(y \mid x = x_0) $. The sampler proceeds as follows:
...
$$
\theta_{i}^{(k)}\sim p(\theta^{(k)}\mid\theta^{(1)}=\theta_{i}^{(1)},\cdots,\theta^{(k-1)}=\theta_{i}^{(k-1)},\theta^{(k+1)}=\theta_{i-1}^{(k+1)},\cdots,\theta^{(n)}=\theta_{i-1}^{(n)})
$$

For example, if there are four variables, $ (w, x, y, z) $, the sampler becomes

$$
w_{i}\sim p(w\mid x=x_{i-1},y=y_{i-1},z=z_{i-1})
$$

$$
x_{i}\sim p(x\mid w=w_{i},y=y_{i-1},z=z_{i-1})
$$

$$
y_{i}\sim p(y\mid w=w_{i},x=x_{i},z=z_{i-1})
$$

$$
z_{i}\sim p(z\mid w=w_{i},x=x_{i},y=y_{i})
$$
...
## Evolution_appendix3_016 · THE GIBBS SAMPLER / Using the Gibbs Sampler to Approximate Marginal Distributions

Any feature of interest for the marginals can be computed from the m realizations of the Gibbs sequence. If $ \theta_1, \cdots, \theta_m $ is an appropriately burned-in set of realizations from a Gibbs sampler, the expectation of any function, $ f $, of the random variable $ \theta $ is approximated by

$$
E[f(\theta)]_{m}=\frac{1}{m}\sum_{i=1}^{m}f(\theta_{i})
$$

This is the Monte-Carlo (MC) estimate of $ f(x) $, as $ E[f(\theta)]_m \to E[f(\theta)] $ as $ m \to \infty $. Likewise, the MC estimate for any function of $ n $ variables $ (\theta^{(1)}, \cdots, \theta^{(n)}) $ is calculated by

$$
E[f(\theta^{(1)},\cdots,\theta^{(n)})]_{m}=\frac{1}{m}\sum_{i=1}^{m}f(\theta_{i}^{(1)},\cdots,\theta_{i}^{(n)})
$$
...
$$
p(x)=\int p(x\mid y)p(y)dy=E_{y}\left[p(x\mid y)\right]
$$

one can approximate the marginal density using

$$
\widehat{p}_{m}(x)=\frac{1}{m}\sum_{i=1}^{m}p(x\mid y=y_{i})
$$
...
## Evolution_appendix3_017 · Appendix: Introduction / REJECTION SAMPLING AND APPROXIMATE BAYESIAN COMPUTATION (ABC)
