...
## chapter18_003 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Expected Variance in Response Generated by Drift
...
## chapter18_004 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Variance in Predicted Response Versus Variance in Actual Response
...
## chapter18_006 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Estimators for Several Generations of Selection
...
## chapter18_007 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Weighted Least-squares Estimates of Realized Heritability
...
> **Example 18.3** · ref: `18.3` · source: `chapter18_007.json` · blocks 2–2
>
...
## chapter18_008 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Standard Errors of Realized Heritability Estimates
...
> **Formula (18.18)** · `18.18` · source: `chapter18_block_040` · Standard Errors of Realized Heritability Estimates
>
...
## chapter18_009 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Power: Estimation of $ h^{2} $ from Relatives or Selection Response?
...
> **Example 18.5** · ref: `18.5` · source: `chapter18_009.json` · blocks 5–5
>
...
## chapter18_010 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Empirical Versus Predicted Standard Errors
...
## chapter18_011 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Realized Heritability With Rank Data
...
## chapter18_012 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Infinitesimal-model Corrections for Disequilibrium
...
> **Example 18.6** · ref: `18.6` · source: `chapter18_012.json` · blocks 2–2
>
...
## chapter18_014 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Most Traits Respond to Selection
...
## chapter18_015 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Sheridan's Analysis
...
## chapter18_016 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Realized Heritabilities, Selection Intensity, and Inbreeding
...
## chapter18_017 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Asymmetric Selection Response
...
## chapter18_018 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Reversed Response
...
## chapter18_020 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Basic Theory of Control Populations
...
## chapter18_021 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Divergent Selection Designs
...
## chapter18_022 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Variance in Response
...
## chapter18_023 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Control Populations and Variance in Response
...
## chapter18_025 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Nicholas' Criterion
...
> **Example 18.8** · ref: `18.8` · source: `chapter18_025.json` · blocks 1–1
>
...
## chapter18_026 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Replicate Lines
...
## chapter18_028 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / The Simple Additive Model
...
## chapter18_029 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / The Hammond-Gardner Model
...
> **Example 18.9** · ref: `18.9` · source: `chapter18_029.json` · blocks 5–5
>
...
The resulting vector, y, of means and the design matrix, X, become $$ \mathbf{y}=\begin{pmatrix}{{{59.7}}} \\{{{63.4}}} \\{{{68.9}}} \\{{{42.3}}} \\{{{61.2}}} \\{{{79.6}}} \\{{{72.3}}} \\{{{40.3}}}\end{pmatrix},\qquad\mathbf{X}=\begin{pmatrix}{{{1}}}&{{{0}}}&{{{1}}}&{{{0}}} \\{{{1}}}&{{{4}}}&{{{1}}}&{{{4}}} \\{{{1}}}&{{{7}}}&{{{1}}}&{{{7}}} \\{{{1}}}&{{{7}}}&{{{0.5}}}&{{{3.5}}} \\{{{1}}}&{{{2}}}&{{{1}}}&{{{2}}} \\{{{1}}}&{{{3.5}}}&{{{1}}}&{{{3.5}}} \\{{{1}}}&{{{5.5}}}&{{{1}}}&{{{5.5}}} \\{{{1}}}&{{{5.5}}}&{{{0.5}}}&{{{2.75}}}\end{pmatrix} $$

Solving for the vector of parameter estimates yields $$ \begin{aligned}\left(\begin{array}{c}\widehat{m}\\\widehat{A}\\\widehat{D}_{0}\\\widehat{D}\\\end{array}\right)=\left(\mathbf{X}^{T}\mathbf{X}\right)^{-1}\mathbf{X}^{T}\mathbf{y}=\left(\begin{array}{c}4.266\\1.072\\57.402\\0.523\\\end{array}\right)\end{aligned} $$
...
> **Example 18.10** · ref: `18.10` · source: `chapter18_030.json` · blocks 0–0
>
> Example 18.10. Here we derive the line and cross means when dominance is present. Because the focus is on the contribution from a specific locus, for ease of presentation we suppress the subscript (on a, d, and p) denoting the locus being followed. Assuming no epistasis, the mean is derived by summing the contributions over all loci. To allow for dominance, the genotypic values at our target locus become 0: a + d: 2a. With p being the frequency of the favorable allele, the mean contribution from this locus is $$ 0\cdot(1-p^{2})+(a+d)\cdot2p(1-p)+(2a)\cdot p^{2}=2ap+2dp(1-p) $$
> 
> Notice that the contribution from $a$ is a linear function of the allele frequency (and hence a linear function of allele frequency change), while the contribution from $d$ has both linear ($p$) and quadratic ($p^2$) components. The contribution from this locus to the line mean in cycle $k$ follows by replacing $p$ with $p(k) \simeq p + k \Delta p$ in Equation 18.36b to yield $$ \begin{aligned}C_{k}&=\mu+2ap(k)+2dp(k)[1-p(k)]\\&=\mu+2a[p+k\Delta p]+2d[p+k\Delta p][1-(p-k\Delta p)]\\&=\left(\mu+2ap\right)+k\cdot2a\Delta p+2dp(1-p)+k\cdot2d\Delta p\cdot(1-2p)-k^{2}\cdot2d\left(\Delta p\right)^{2}\\&=m+k\cdot A+D_{0}+k\cdot D+k^{2}\cdot D_{q}\\ \end{aligned} $$
> 
> Now suppose that this line is selfed. Homozygotes replicate themselves, while segregation of genotypes occurs when heterozygotes are selfed, with half of the offspring being $ a + d $ heterozygotes, one-quarter of the offspring being 2a homozygotes, and the final one-quarter being 0-valued homozygotes. The resulting mean value following a generation of selfing is the sum of these contributions, $$ p^{2}\left[2a\right]+2p(1-p)\left[(1/4)(2a)+(1/2)(a+d)+(1/4)\cdot0\right]+(1-p^{2})\cdot0=2ap+dp(1-p) $$
> 
> Note that all terms involving d are exactly half of their random-mating values, yielding $$ C_{k}(s)=m+k\cdot A+\frac{1}{2}\left(D_{0}+k\cdot D+k^{2}\cdot D_{q}\right) $$
> 
> Now consider the mean of the cross of lines from cycles k and j. As above, the chance of a 0-valued homozygote is $ [1-p(k)][1-p(j)] $, whereas it is $ p(k)[1-p(j)]+p(j)[1-p(k)] $ for an $ a+d $ heterozygote, and $ p(k)p(j) $ for a 2a homozygote, yielding a resulting mean of $$ \begin{aligned}C_{k\times j}&=\mu+(a+d)\cdot\left(p(k)\left[1-p(j)\right]+p(j)\left[1-p(k)\right]\right)+2a\cdot p(k)p(j)\\&=\mu+a\Biggl(p(k)+p(j)\Biggr)+d\cdot\Biggl(p(k)+p(j)-2p(k)p(j)\Biggr)\\=\left(\mu+2ap\right)&+(k+j)a\Delta p+d\Biggl(2p(1-p)+(k+j)\cdot\Delta p(1-2p)-k j\cdot2\left(\Delta p\right)^{2}\Biggr)\\&=m+\frac{k+j}{2}\cdot A+D_{0}+\frac{k+j}{2}\cdot D+k j\cdot D_{q}\end{aligned} $$
> 
...
> 
> Finally, when we form an $ F_{2} $ by randomly mating the $ F_{1} $ progeny from a line cross, the new allele frequency is $$ \frac{p(k)+p(j)}{2}\simeq p+\frac{k+j}{2}\Delta p $$ yielding a mean of $$ \begin{align*}C_{k\times j}(rm)&=\mu+2a\left(p+\frac{k+j}{2}\Delta p\right)+2d\left(p+\frac{k+j}{2}\Delta p\right)\left(1-p-\frac{k+j}{2}\Delta p\right)\\=(\mu+2ap)+\frac{k+j}{2}2a\Delta p+2dp(1-p)+\frac{k+j}{2}2d\Delta p(1-2p)-\left(\frac{k+j}{2}\right)^{2}2d(\Delta p)^{2}\\&=m+\frac{k+j}{2}\cdot A+D_{0}+\frac{k+j}{2}\cdot D+\left(\frac{k+j}{2}\right)^{2}\cdot D_{q}\end{align*} $$
> 
> Recalling Equation 18.38c, we can express this as $$ C_{k\times j}(r m)=C_{k\times j}-k j\cdot D_{q}+\left(\frac{k+j}{2}\right)^{2}\cdot D_{q}=C_{k\times j}+\frac{(k-j)^{2}}{4}\cdot D_{q} $$
> 
> The additional term is due to the decay in heterosis following random mating, as the heterosis in the $ F_{1} $ cross between lines from cycles j and k is $$ \begin{aligned}H&=\sum(p_{i}(k)-p_{i}(j))^{2}d_{i}=\sum(p+k\Delta p-p-j\Delta p)^{2}d_{i}\\&=(k-j)^{2}\sum(\Delta p)^{2}d_{i}=-(k-j)^{2}D_{q}/2\end{aligned} $$
> 
> Recall (LW Chapter 9) that the amount, H, of heterosis from the $ F_{1} $ is reduced to $ H/2 $ in the (random-mating generated) $ F_{2} $. Hence $$ C_{k\times j}(rm)-C_{k\times j}=-\frac{H}{2}=(k-j)^{2}\frac{D_{q}}{4} $$ as found above.


---

## chapter18_031 · Analysis of Short-term Selection Experiments: 1. Least-squares Approaches / Accounting for Inbreeding Depression and Drift
...
> **Example 18.11** · ref: `18.11` · source: `chapter18_031.json` · blocks 8–8
>
...
> 
> Finally, a rough calculation suggests the conditions under which $ \sigma^2(\delta p) \gg (\Delta p)^2 $, i.e., when the variance in allele-frequency change from drift exceeds the square of the deterministic change from selection. Equation 5.2b shows that $ \Delta p \simeq sp(1 - p) $ for a very weakly selected additive locus, while Equation 5.21 yields $ s \simeq \bar{\imath} a / \sigma_z $, where $ \bar{\imath} $ is the selection intensity on the trait, yielding $ \Delta p_i \simeq (\bar{\imath} a_i / \sigma_z) p_i (1 - p_i) $. Recalling that $ \sigma^2(A_i) = 2a_i^2 p_i (1 - p_i) $ is the additive genetic variance associated with locus $ i $ $$ \begin{align*}(\Delta p_{i})^{2}&\simeq(\bar{\imath}a_{i}/\sigma_{z})^{2}p_{i}^{2}(1-p_{i})^{2}=\bar{\imath}^{2}p_{i}(1-p_{i})\left(\frac{a_{i}^{2}p_{i}(1-p_{i})}{\sigma_{z}^{2}}\right)\\&=\bar{\imath}^{2}p_{i}(1-p_{i})\left(\frac{\sigma^{2}(A_{i})/2}{\sigma_{z}^{2}}\right)=\frac{\bar{\imath}^{2}p_{i}(1-p_{i})}{2}h_{i}^{2}\end{align*} $$ where $ h_{i}^{2} $ is the heritability due to locus i. Assuming that n loci underlying the trait contribute roughly the same to the overall heritability, $ h^{2} $, then
> 
> > **Formula (18.45a)** · `18.45a` · source: `chapter18_block_164` · Accounting for Inbreeding Depression and Drift
> >
> > $$ (\Delta p_{i})^{2}\simeq\frac{p_{i}(1-p_{i})}{2}\frac{\bar{\imath}^{2}h^{2}}{n} $$
> 
> 
> Drift dominates the expected quadratic change when $ \sigma^{2}(\delta p) \gg (\Delta p)^{2} $, or $$ \frac{p(1-p)}{2N_{e}}\gg\frac{p(1-p)}{2}\frac{\bar{\tau}^{2}h^{2}}{n} $$ which reduces to
> 
> > **Formula (18.45b)** · `18.45b` · source: `chapter18_block_165` · Accounting for Inbreeding Depression and Drift
> >
...
> 
> > **Formula (18.45c)** · `18.45c` · source: `chapter18_block_166` · Accounting for Inbreeding Depression and Drift
> >
...
> or
> 
> > **Formula (18.45d)** · `18.45d` · source: `chapter18_block_166` · Accounting for Inbreeding Depression and Drift
> >
