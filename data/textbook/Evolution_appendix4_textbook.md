...
## Evolution_appendix4_001 · Appendix: Introduction
...
## Evolution_appendix4_002 · Appendix: Introduction / COMBINING p VALUES OVER INDEPENDENT TESTS
...
$$
Z_{s}=\frac{\sum_{i=1}^{k}Z_{i}}{\sqrt{k}}
$$
...
## Evolution_appendix4_006 · BONFERRONI CORRECTIONS AND THEIR EXTENSIONS / Standard Bonferroni Corrections
...
## Evolution_appendix4_007 · BONFERRONI CORRECTIONS AND THEIR EXTENSIONS / Sequential Bonferroni Corrections
...
## Evolution_appendix4_008 · BONFERRONI CORRECTIONS AND THEIR EXTENSIONS / Holm's Method
...
## Evolution_appendix4_009 · BONFERRONI CORRECTIONS AND THEIR EXTENSIONS / Simes-Hochberg Method
...
## Evolution_appendix4_010 · BONFERRONI CORRECTIONS AND THEIR EXTENSIONS / Hommel's Method
...
## Evolution_appendix4_011 · BONFERRONI CORRECTIONS AND THEIR EXTENSIONS / Cheverud's Method and Other Approach for Dealing with Dependence
...
$$
n_{e,Cheverud}=n\left(1-\frac{(n-1)\sigma^{2}(\lambda)}{n^{2}}\right),\qquad\mathrm{where}\qquad\sigma^{2}(\lambda)=\frac{1}{n-1}\sum_{i=1}^{n}(\lambda_{i}-1)^{2}
$$
...
$$
n_{e,Li-Ji}=\sum_{i=1}^{N}I(\lambda_{i}\geq1)+\sum_{i=1}^{N}(\lambda_{i}-\mathrm{floor}[\lambda_{i}])
$$

where the indicator function $ I(x \geq 1) $ returns a value of one when $ x \geq 1 $, and otherwise returns a value of zero. Hence, the first sum in Equation A4.5b is the number of eigenvalues of C that are $ \geq 1 $. The floor[x] function in the second term corresponds to the largest integer $ \leq x $, so the second sum is all of the remainder terms (the effects of partial correlations among tests). Additional corrections for dealing with correlated tests have been proposed (e.g., Owen 2005; Efron 2007; Leek and Storey 2007, 2008; Li et al. 2012).

---

## Evolution_appendix4_012 · Appendix: Introduction / DETECTING AN EXCESS NUMBER OF SIGNIFICANT TESTS
...
## Evolution_appendix4_013 · DETECTING AN EXCESS NUMBER OF SIGNIFICANT TESTS / How Many False Positives?
...
$$
\Pr(j\mathsf{f a l s e p o s i t i v e s})=\frac{n!}{(n-j)!j!}(1-\alpha)^{n-j}\alpha^{j}
$$
...
$$
\Pr(j\mathsf{f a l s e p o s i t i v e s})\simeq\frac{(n\alpha)^{j}e^{-n\alpha}}{j!}
$$
...
## Evolution_appendix4_014 · DETECTING AN EXCESS NUMBER OF SIGNIFICANT TESTS / Schweder-Spjøtvoll plots

Testing for an excessive number of significant tests is a rather crude indicator of the actual number ($n 0$) of the $n$ tests that are true nulls. It is very possible that $n 0 < n$ and yet we would not detect an excess of significant tests by the above method. Likewise, if an excessive number is detected, what really can we say about $n 0$ other than $n 0 < n$? For instance, Example A4.5 shows an excess of 9 significant tests (observed 15, expected 6), but clearly assuming $n 0 = n - 9$ is a bit naive. Finally, the outcome varies with our choice of $\alpha$. One could easily imagine an excess of significant tests using $\alpha = 0.05$, but not when using $\alpha = 0.01$. Ideally, we would like to have an estimate for $n 0$ that is independent of the choice of $\alpha$.
...
## Evolution_appendix4_015 · DETECTING AN EXCESS NUMBER OF SIGNIFICANT TESTS / Estimating $ n_{0} $: Subsampling From a Uniform Distribution
...
> **Figure A4.2** · page 11 · source: `Evolution_appendix4`
>
> ![Figure A4.2](figures/Evolution_A4.2.png)
>
...
> **Figure A4.3** · page 11 · source: `Evolution_appendix4`
>
> ![Figure A4.3](figures/Evolution_A4.3.png)
>
...
$$
\phi_{u}(p)=1\quad for\quad0\leq p\leq1
$$
...
$$
\begin{align*}n_0\int_a^b\phi_u(p)dp=n_0\int_a^b1\cdot dp=n_0(b-a)\end{align*}
$$

Hence

$$
\widehat{n}_{0}(a,b)=\frac{Number of p(i)values in(a,b)}{b-a}
$$

Likewise, an estimate for the fraction $ \pi_{0}=n_{0}/n $ of true nulls is

$$
\begin{aligned}\widehat{\pi}_{0}(a,b)&=\frac{\text{Number of}p(i)\text{values in}(a,b)}{n(b-a)}\\&=\frac{\text{Fraction of}p(i)\text{values in}(a,b)}{b-a}\end{aligned}
$$
...
Storey and Tibshirani (2003) considered the number of $p$ values exceeding some tunable parameter value, $\lambda$ (taking $a = \lambda$ and $b = 1$ in Equation A4.8b), on the logic that for larger values of $\lambda$, most of these draws are from the uniform corresponding to draws from the null. Let $\widehat{\pi}_0(\lambda)$ denote the estimated fraction of truly null hypotheses based on using a tuning value of $ \lambda $, then

$$
\widehat{\pi}_{0}(\lambda)=\frac{Number of p(i)values>\lambda}{n(1-\lambda)}
$$

and

$$
\widehat{n}_{0}(\lambda)=n\cdot\widehat{\pi}_{0}(\lambda)=\frac{Number of p(i)values>\lambda}{1-\lambda}
$$
...
## Evolution_appendix4_016 · DETECTING AN EXCESS NUMBER OF SIGNIFICANT TESTS / Estimating $ n_{0} $: Mixture Models
...
> **Figure A4.4** · page 13 · source: `Evolution_appendix4`
>
> ![Figure A4.4](figures/Evolution_A4.4.png)
>
...
$$
\ell(p)=(1-\pi_{0})\phi_{A}(p)+\pi_{0}\phi_{u}(p)=(1-\pi_{0})\frac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}p^{a-1}(1-p)^{b-1}+\pi_{0}
$$

with the resulting total likelihood over the n sampled p values (from independent tests) calculated by

$$
\ell(\mathbf{p})=\prod_{i=1}^{n}\ell(p_{i})
$$
...
## Evolution_appendix4_017 · Appendix: Introduction / FDR: THE FALSE-DISCOVERY RATE
...
$$
\mathrm{FDR}=\mathrm{Pr}(i is truly null\mid i is deemed significant)=\delta
$$

whereas for the false-positive rate, we condition on the hypothesis being null

$$
\mathrm{FPR}=\mathrm{Pr}(i is deemed significant\mid i is truly null)=\alpha
$$

---

## Evolution_appendix4_018 · FDR: THE FALSE-DISCOVERY RATE / Morton's Posterior Error Rate (PER) and the FDR
...
$$
\Pr(S=n=1)=\alpha\pi_{0}+(1-\beta)(1-\pi_{0})
$$

Substituting Equations A4.14a and A4.14b into Equation A4.13 yields

$$
PER=\frac{\alpha\cdot\pi_{0}}{\alpha\cdot\pi_{0}+\left(1-\beta\right)\cdot\left(1-\pi_{0}\right)}=\left(1+\frac{\left(1-\beta\right)\cdot\left(1-\pi_{0}\right)}{\alpha\cdot\pi_{0}}\right)^{-1}
$$

Figure A4.5 plots Equation A4.15a for various values of $ \pi_{0} $ and $ \beta $.

> **Figure A4.5** · page 16 · source: `Evolution_appendix4`
>
> ![Figure A4.5](figures/Evolution_A4.5.png)
>
...
$$
\begin{align*}\alpha=\left({\gamma\over1-\gamma}\right)\left({1-\pi_0\over\pi_0}\right)(1-\beta)\end{align*}
$$

In particular, if there is complete power ($ \beta = 0 $) and only one of the $ n $ tested hypotheses departs from the null ($ \pi_0 = n/[n + 1] $), Equation A4.15c reduces to

$$
\begin{align*}\alpha=\left({\gamma\over1-\gamma}\right)\left({1\over n}\right)\simeq{\gamma\over n}\end{align*}
$$
...
## Evolution_appendix4_019 · FDR: THE FALSE-DISCOVERY RATE / A Technical Aside: Different Definitions of False-discovery Rate
...
## Evolution_appendix4_021 · FDR: THE FALSE-DISCOVERY RATE / A (Slightly More) Formal Derivation of the Estimated FDR
...
$$
\Pr(p\leq\tau\mid null hypothesis)=\int_{0}^{\tau}\phi_{u}(p)dp=\tau
$$
...
$$
E[F(\tau)]=n_{0}\cdot\Pr(p\leq\tau\mid null hypothesis)\simeq n_{0}\cdot\tau
$$

Hence,

$$
E[FDR(\tau)]=\frac{n_{0}\cdot\tau}{S(\tau)}
$$

Setting $ \tau = p(k) $, then $ S(\tau) = k $, and Equation A4.21 becomes $ n_0 p(k)/k $, recovering Equation A4.17. Using the Storey-Tibshirani estimator for $ n_0 $ (Equation A4.9b), an estimated value for the FDR using threshold value, $ \tau $ (and based on the tuning parameter, $ \lambda $, in the Storey-Tibshirani estimator), becomes

$$
\widehat{F D R}(\tau)=n_{0}\cdot\frac{\tau}{S(\tau)}=\left(\frac{N[p(i)\ values>\lambda]}{1-\lambda}\right)\cdot\left(\frac{\tau}{N[p(i)\ values\leq\tau]}\right)
$$
...
$$
\mathbf{c}\mathbf{d}\mathbf{f}_{U}(x)=\int_{0}^{x}\phi_{U}(p)d p=x\qquad\mathbf{c}\mathbf{d}\mathbf{f}_{A}(x)=\int_{0}^{x}\phi_{A}(p)d p
$$
...
## Evolution_appendix4_022 · FDR: THE FALSE-DISCOVERY RATE / Storey's q Value
...
$$
\widehat{q}\left[p(i)\right]=\min_{\tau\geq p(i)}\widehat{F D R}(\tau)
$$
...
## Evolution_appendix4_023 · FDR: THE FALSE-DISCOVERY RATE / Closing Caveats in Using the FDR
...
## Evolution_appendix4_024 · Appendix: Introduction / FORMAL META-ANALYSIS
...
## Evolution_appendix4_025 · FORMAL META-ANALYSIS / Informal, or Narrative, Meta-analysis
...
## Evolution_appendix4_026 · FORMAL META-ANALYSIS / Standardizing Effect Sizes
...
$$
T_{i}=\overline{z}_{i},\qquad\mathrm{w i t h}\qquad s_{i}^{2}=\frac{1}{n_{i}-1}\sum_{j=1}^{n_{i}}\left(z_{i j}-\overline{z}_{i}\right)^{2}
$$
...
$$
s_{i,p}^{2}=\frac{(n_{i,t}-1)s_{i,t}^{2}+(n_{i,c}-1)s_{i,c}^{2}}{n_{i,c}+n_{i,t}}
$$
...
$$
g_{i}=\frac{\overline{z}_{i,t}-\overline{z}_{i,c}}{s_{i,p}},\quad\mathrm{w h e r e}\quad s_{i,p}^{2}=\frac{(n_{i,t}-1)s_{i,t}^{2}+(n_{i,c}-1)s_{i,c}^{2}}{n_{i,c}+n_{i,t}-2}
$$

which is the unbiased OLS estimate (and hence the REML estimate) of the pooled standard error. One further correction is often applied to $ g_i $, in that the ratio given by Equation A4.28a is a biased estimator of $ (\mu_t - \mu_c)/\sigma_p $, which can be adjusted by using

$$
g_{i}\left(1-\frac{3}{4(n_{i,c}+n_{i,t})-9}\right)
$$
...
$$
r_{i}^{*}=r_{i}+\frac{r_{i}(1-r_{i}^{2})}{2(n_{i}-3)}
$$

Fisher's z-transformation is used to both stabilize the variance and remove some of the skew:

$$
z_{i,r}=\frac{1}{2}\ln\left(\frac{1+r_{i}}{1-r_{i}}\right),\qquad\mathrm{w i t h}\qquad s_{i}^{2}=\frac{1}{n_{i}-3}
$$
...
## Evolution_appendix4_027 · FORMAL META-ANALYSIS / Fixed-effects, Random-effects, and Mixed-model Meta-analysis
...
$$
\overline{T}=\frac{\sum_{i=1}^{k}w_{i}T_{i}}{\sum_{i=1}^{k}w_{i}},\quad\mathrm{w h e r e}\quad w_{i}=\frac{1}{s_{i}^{2}}
$$
...
$$
s_{T}^{2}=\frac{1}{\sum_{i=1}^{k}w_{i}}
$$

For the situation where we assume that each individual observation in a given study has the same variance, so $ \sigma^2(T_i) = \sigma^2/n_i $, then for $ k $ studies, each of size $ n $,

$$
\sigma^{2}(\overline{T})=\frac{\sigma^{2}}{nk}
$$
...
$$
Q=\sum_{i=1}^{k}\frac{\left(T_{i}-\overline{T}\right)^{2}}{s_{i}^{2}}
$$

where (under the null of $ \theta_1 = \cdots = \theta_k $, and assuming that the values of $ T_i $ are normally distributed), the distribution of $ Q $ is $ \chi^2 $ with $ (k-1) $ degrees of freedom.
...
$$
T_{i}=\theta+\sum_{j=1}^{m}b_{j}M_{ij}+e_{i}
$$
...
$$
w_{i}=\frac{1}{s_{i}^{2}+\widehat{\sigma}_{u}^{2}}
$$
...
$$
\widehat{\sigma}_{u}^{2}=\frac{Q-(k-1)}{S_{1}-(S_{2}/S_{1})},\quad\mathrm{w h e r e}\quad S_{j}=\sum_{i=1}^{k}s_{i}^{-2j}
$$
...
To see why this occurs, assume we are using the same design that led to Equation A4.30d, namely, k studies all of size n, and all with a common residual variance ($ \sigma^{2} $) for each observation. In this case, the variance of the estimate of $ \mu $ becomes

$$
\sigma^{2}(\overline{T})=\frac{\sigma^{2}}{nk}+\frac{\sigma_{u}^{2}}{k}
$$
...
$$
T_{i}=\mu+\sum_{j=1}^{m}b_{j}M_{ij}+u_{i}+e_{i}
$$
...
$$
\mathbf{y}=\mathbf{M}\mathbf{b}+\mathbf{u}+\mathbf{e}
$$

where $ y_i = T_i $, and the $ i $th row of the $ k \times m $ matrix, $ \mathbf{M} $, contains the values of the moderator variables associated with study $ i $. The vectors, $ \mathbf{u} $ and $ \mathbf{e} $, of random effects are assumed uncorrelated, with $ \mathbf{e} \sim (\mathbf{0}, \mathbf{R}) $ and $ \mathbf{u} \sim (\mathbf{0}, \mathbf{G}) $, where $ \mathbf{R} $ is a known diagonal matrix, $ \text{diag}(s_1^2, s_2^3, \cdots, s_k^2) $, and $ \mathbf{G} = \sigma_u^2 \mathbf{C} $, where $ \mathbf{C} $ is a matrix of known constants.
...
## Evolution_appendix4_028 · FORMAL META-ANALYSIS / Publication and Other Sources in Bias
...
> **Figure A4.6** · page 27 · source: `Evolution_appendix4`
>
> ![Figure A4.6](figures/Evolution_A4.6.png)
>
...
While funnel asymmetry is essentially an informal visual judgment, the basic idea has been used for more formal tests. The Begg-Mazumdar (1994) rank correlation test looks for associations between the ranks of the standard errors $ (s_{i}) $ and study estimates $ (T_{i}) $. The Egger regression test (Egger et al. 1997) performs the regression

$$
y_{i}=a+b s_{i}+e,\quad\mathrm{w h e r e}\quad y_{i}=(T_{i}-\overline{T})
$$
...
> **Figure A4.7** · page 28 · source: `Evolution_appendix4`
>
> ![Figure A4.7](figures/Evolution_A4.7.png)
>
...
## Evolution_appendix4_029 · FORMAL META-ANALYSIS / Bias When Estimating Magnitudes
...
$$
E[|\theta|]=\sigma_{u}\sqrt{2/\pi},\qquad\mathrm{w h i l e}\qquad E[|T|]=\sqrt{\sigma_{u}^{2}+\sigma_{e}^{2}}\cdot\sqrt{2/\pi}
$$
...
> **Figure A4.8** · page 30 · source: `Evolution_appendix4`
>
> ![Figure A4.8](figures/Evolution_A4.8.png)
>
