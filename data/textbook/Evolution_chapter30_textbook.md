...
## Evolution_chapter30_001 · Measuring Multivariate Selection: Introduction
...
## Evolution_chapter30_002 · Measuring Multivariate Selection: Introduction / SELECTION ON MULTIVARIATE PHENOTYPES: DIFFERENTIALS AND GRADIENTS
...
## Evolution_chapter30_003 · SELECTION ON MULTIVARIATE PHENOTYPES: DIFFERENTIALS AND GRADIENTS / Changes in the Mean Vector: The Directional Selection Differential Vector, S
...
> **Figure 30.1** · page 3 · source: `Evolution_chapter30`
>
> ![Figure 30.1](figures/Evolution_30.1.png)
>
...
## Evolution_chapter30_004 · SELECTION ON MULTIVARIATE PHENOTYPES: DIFFERENTIALS AND GRADIENTS / The Directional Selection Gradient Vector, $ \beta $
...
with $ z_{sd,i} $ having a mean of zero and unit variance (this is often denoted by $ z'_{i} $ in the literature). As mentioned in Chapter 29, one could also mean-standardize $ (z_{i}/\mu_{i}) $, but when the term standardized variable is used in the evolutionary and ecological literature, it usually implies Equation 30.4a. The selection differential of the standardized variable is simply the selection intensity $ \bar{\tau} $ (Equation 13.6a), as

$$
S_{sd,i}=\sigma(z_{sd,i},w)=\sigma(z_{i}/\sigma_{i},w)=\frac{\sigma(z_{i},w)}{\sigma_{i}}=\frac{S_{i}}{\sigma_{i}}=\bar{\imath}_{i}
$$
...
Equations 30.4b and 30.4c imply that the phenotypic covariance matrix $ (P {sd}) $ for the standardized variables is simply the matrix of all pairwise correlations. Following our notation from Chapter 29, we denote selection gradients using variance-standardized variables by $ \beta {sd,i} $, although they are often denoted by $ \beta i' $ in the literature. When expressed with standardized variables, Equation 30.2b becomes

**[推导 Derivation]**

$$
\begin{array}{r l}{w(\mathbf{z} {s d})=1+\beta {s d}^{T}\mathbf{z} {s d}+e,}&{{}\mathrm{w h e r e}\quad\beta {s d}=\mathbf{P} {s d}^{-1}\mathbf{S} {s d}}\end{array}
$$

Here $ \beta {sd,i} $ is the expected change in relative fitness given a change of one standard deviation in trait $ z i $ when all of the other measured trait values (i.e., the elements of $ \mathbf{z} $) are held constant.
...
$$
\boldsymbol{\beta} =\sqrt{\boldsymbol{\beta}^{T}\boldsymbol{\beta}}=\sqrt{\sum\boldsymbol{\beta} {i}^{2}}
$$

Morrissey (2014b) noted that $ E[\lVert\widehat{\boldsymbol{\beta}}\rVert] \geq \lVert\boldsymbol{\beta}\rVert $, as the values of $ \beta_i $ are measured with error, and such error carries over into the norm. Suppose $ k $ independent traits are measured, where $ \widehat{\beta}_i \sim N(\beta_i, \sigma_i^2) $, so that each $ \widehat{\beta}_i $ is independent and identically distributed, with sampling error $ \sigma_i^2 $. In this case, $ E[\lVert\widehat{\boldsymbol{\beta}}\rVert] = \sqrt{k(\beta_i^2 + \sigma_i^2)} > \lVert\boldsymbol{\beta}\rVert = \sqrt{k\beta_i^2} $. Given that the standard error of an estimate of $ \beta $ is often of the same order as the estimate itself (Morrissey 2014b), the total strength of selection can be considerably overestimated by $ \lVert\widehat{\boldsymbol{\beta}}\rVert $.
...
Motivated by the univariate case wherein $ C = \sigma[w, (z - \mu)(z - \mu)] $, the multivariate quadratic selection differential is defined as the square $ (n \times n) $ matrix, C, whose elements are the covariances between all pairs of quadratic deviations, $ (z i - \mu i)(z j - \mu j) $, and relative fitness, w, namely,

**[推导 Derivation]**

$$
C {i j}=\sigma[w,(z {i}-\mu {i})(z {j}-\mu {j})]
$$
...
where $ y i = \mathbf{e} i^T(\mathbf{z} - \mathbf{z} 0) $ and $ w 0 $ is shown by Equation 30.21b. Equation 30.26 is called the A canonical form and Equation 30.27, the B canonical form (Box and Draper 1987). Both forms represent a rotation of the original axis to the new set of coordinates (the canonical axes of $ \gamma $) that align them with axes of symmetry of the original quadratic surface. The B canonical form further shifts the origin to the stationary point, $ z 0 $. Because the contribution to individual fitness from $ b^T z $ is a hyperplane, its effect is to tilt the fitness surface. The B canonical form effectively levels this tilting, thus allowing us to focus entirely on the curvature (quadratic) aspects of the fitness surface.

**[推导 Derivation]**

$$
\beta_{JS,i}=E_{z_i}\left[\frac{\partial w(\mathbf{z})}{\partial z_i}\right]\simeq\frac{1}{n}\sum_{j=1}^{n}\left.\frac{\partial w(\mathbf{z})}{\partial z_i}\right|_{\mathbf{Z}=\mathbf{Z}_j}
$$
...
## Evolution_chapter30_005 · SELECTION ON MULTIVARIATE PHENOTYPES: DIFFERENTIALS AND GRADIENTS / Directional Gradients, Fitness Surface Geometry, and Selection Response
...
$$
\nabla_{\mathbf{x}}f(\mathbf{x})=\begin{pmatrix}\partial f/\partial x_{1}\\ \partial f/\partial x_{2}\\ \vdots\\ \partial f/\partial x_{n}\end{pmatrix}
$$
...
which holds provided $ z \sim MVN $ (Lande and Arnold 1983). To see this, we use integration by parts to yield
...
## Evolution_chapter30_006 · SELECTION ON MULTIVARIATE PHENOTYPES: DIFFERENTIALS AND GRADIENTS / Changes in the Covariance Matrix: The Quadratic Selection Differential Matrix, C
...
## Evolution_chapter30_007 · SELECTION ON MULTIVARIATE PHENOTYPES: DIFFERENTIALS AND GRADIENTS / The Quadratic Selection Gradient Matrix, $ \gamma $
...
> **Figure 30.3** · page 17 · source: `Evolution_chapter30`
>
> ![Figure 30.3](figures/Evolution_30.3.png)
>
...
## Evolution_chapter30_008 · SELECTION ON MULTIVARIATE PHENOTYPES: DIFFERENTIALS AND GRADIENTS / Quadratic Gradients, Fitness Surface Geometry, and Selection Response
...
The major features of linear and quadratic differentials and gradients discussed here are summarized in *[See Table 30.1 at the end of this section.]*. Excellent overviews were also provided by Brodie et al. (1995) and Arnold et al. (2001).

---

## Evolution_chapter30_009 · Measuring Multivariate Selection: Introduction / MULTIDIMENSIONAL QUADRATIC FITNESS REGRESSIONS
...
*[See Table 30.1 at the end of this section.]*

> **Table 30.1** · `30.1` · page 12 · source: `Evolution_chapter30_009`
...
> Differentials measure the covariance between relative fitness and phenotype | $ S_{i}=\sigma\left[w,z_{i}\right] $<br><br>$ C_{ij}=\sigma\left[w,(z_{i}-\mu_{i})(z_{j}-\mu_{j})\right] $
...
## Evolution_chapter30_010 · MULTIDIMENSIONAL QUADRATIC FITNESS REGRESSIONS / Estimation, Hypothesis Testing, and Confidence Intervals
...
$$
w=a+\frac{1}{2}\mathbf{z}_{sd}^{T}\gamma_{sd}\mathbf{z}_{sd}+e
$$
...
## Evolution_chapter30_011 · MULTIDIMENSIONAL QUADRATIC FITNESS REGRESSIONS / Regression Packages and Coefficients of $ \gamma $
...
$$
\left(\frac{\gamma_{11}}{2}z_{1}\right)+\left(\frac{\gamma_{22}}{2}z_{2}\right)+\left(\gamma_{12}\cdot z_{1}\cdot z_{2}\right)
$$

However, many regression packages output the quadratic coefficients as

$$
(b_{11}\cdot z_{1})+(b_{22}\cdot z_{2})+(b_{12}\cdot z_{1}\cdot z_{2})
$$
...
## Evolution_chapter30_012 · MULTIDIMENSIONAL QUADRATIC FITNESS REGRESSIONS / Geometric Aspects
...
## Evolution_chapter30_014 · MULTIDIMENSIONAL QUADRATIC FITNESS REGRESSIONS / Canonical Transformation of $ \gamma $
...
Returning to Figure 30.3, we see that the axes of symmetry of the quadratic surface are the canonical axes of $ \gamma $. For $ \gamma_{12} = 0.25 $ (Figure 30.3A), $ \lambda_1 = -2.06 $ and $ \lambda_2 = -0.94 $, and so the fitness surface is concave along each canonical axis, with more extreme curvature along the $ y_1 $ axis. When $ \gamma = \sqrt{2} $ (Figure 30.3B), one eigenvalue is zero while the other is -3, so the surface shows no curvature along one axis (it is a plane) but is strongly concave along the other. Finally, when $ \gamma_{12} = 4 $ (Figure 30.3C), the two eigenvalues differ in sign, as they are -5.53 and 2.53. This generates a saddle point, with a surface that is concave along one axis and convex along with other, and here with the concave curvature being more the extreme. From Equation 30.26, we can see that the fitness change along a particular axis ($ e_i $) is $ \theta_i y_i + (\lambda_i / 2) y_i^2 $. If $ |\theta_i| \gg |\lambda_i| > 0 $, the curvature of the fitness surface along this axis is dominated by the effects of linear (as opposed to quadratic) selection for modest values of $ y_i = e_i^T z $. If $ \lambda_i = 0 $, the fitness surface along $ y_i $ has no curvature, so the fitness surface is a ridge along this axis. If $ \theta_i > 0 $, this is a rising ridge (fitness increases as $ y_i $ increases), whereas it is a falling ridge (fitness decreases as $ y_i $ increases) if $ \theta_i < 0 $, and it is flat if $ \theta_i = 0 $. Even if $ \gamma $ is not singular, it may be nearly so, with some of the eigenvalues being very close to zero. In this case, the fitness surface shows little curvature along the axes given by the eigenvectors associated with these nearly zero eigenvalues. Further issues relating to the visualization of multivariate fitness surfaces are discussed in Phillips and Arnold (1989), while Box and Draper (1987) review the statistical foundations of this approach.

---

## Evolution_chapter30_015 · MULTIDIMENSIONAL QUADRATIC FITNESS REGRESSIONS / Are Traits Based on Canonical Axes Meaningful?
...
## Evolution_chapter30_016 · MULTIDIMENSIONAL QUADRATIC FITNESS REGRESSIONS / Strength of Selection: $ \gamma_{ii} $ Versus $ \lambda $
...
## Evolution_chapter30_017 · MULTIDIMENSIONAL QUADRATIC FITNESS REGRESSIONS / Significance and Confidence Regions for a Stationary Point
...
A classical result for a quadratic regression is the Box-Hunter confidence region. Suppose we let $ d_z $ denote the gradient vector of the best quadratic regression of w on z, which (Equation 30.20a) is calculated by
...
## Evolution_chapter30_019 · Measuring Multivariate Selection: Introduction / MULTIVARIATE SEMIPARAMETRIC FITNESS SURFACE ESTIMATION
...
> **Figure 30.4** · page 23 · source: `Evolution_chapter30`
>
> ![Figure 30.4](figures/Evolution_30.4.png)
>
...
## Evolution_chapter30_020 · MULTIVARIATE SEMIPARAMETRIC FITNESS SURFACE ESTIMATION / Projection-pursuit Regression and Thin-plate Splines
...
Note that these two sets of projection vectors from the different regressions are rather distinct. If we apply Equation A5.2b, the angle between $ e_1 $ and $ a_1 $ will be $ 88^\circ $, while the angle between $\mathbf{e}_2$ and $\mathbf{a}_2$ will be $87.5^\circ$. Thus, the major axes of the fitness surface are different (here, they are essentially orthogonal) if one uses a quadratic approximation as opposed to a projection-pursuit regression approximation. As Figure 30.4A shows, the best-fitting quadratic regression using $\mathbf{e}_1$ and $\mathbf{e}_2$ (where $y_1 = \mathbf{e}_1^T \mathbf{z}$ and $y_2 = \mathbf{e}_2^T \mathbf{z}$ are the trait values on this fitness surface for an individual with a trait vector, $\mathbf{z}$) shows disruptive selection, with fitness rising in all directions from a central minimum. If one uses these axes but then fits the data using thin-plate splines, the result is a rather different-looking fitness function (Figure 30.4B), but one that also shows multiple peaks corresponding to different combinations of the traits that females find attractive. When projection-pursuit regression is used (Figure 30.4C), the fitness surface (in rough appearance) is similar to that for thin-plate splines using the quadratic axes ($\mathbf{e}_1$, $\mathbf{e}_2$). However, because the values for the two sets of axes ($x_1 = \mathbf{a}_1^T \mathbf{z}$, $x_2 = \mathbf{a}_2^T \mathbf{z}$ versus $y_1 = \mathbf{e}_1^T \mathbf{z}$, $y_2 = \mathbf{e}_2^T \mathbf{z}$) are rather different, this superficial visual appearance must be mapped into trait values (as indicated in Figure 30.4). Again, the result is that the fitness surface has multiple peaks.

---

## Evolution_chapter30_021 · MULTIVARIATE SEMIPARAMETRIC FITNESS SURFACE ESTIMATION / Gradients for General Fitness Surfaces

*[See Table 30.1 at the end of this section.]* shows that when z is multivariate-normal, Lande-Arnold gradients describe the average geometry of an individual fitness surface and are the sole measure of selection in response equations (provided breeding values are Gaussian). When z is not multivariate normal, these features do not necessarily hold. We addressed the issue of how univariate generalized gradients are computed in such settings (and what they imply) in Chapter 29, and these same concepts easily extend to multivariate settings. Janzen-Stein gradients (Equations 29.35a and 29.35b) measure the average geometry of the individual fitness surface. Suppose we let $ \mathbf{z}_i, \cdots, \mathbf{z}_n $ denote the n vectors of phenotypic observations. For trait $ i $, namely, the average value (over the $n$ data vectors, $\mathbf{z}_{1},\cdots,\mathbf{z}_{n}$) of the gradient with respect to the focal trait.

Morrissey-Sakrejda gradients (Equation 29.35d) are calculated based on the partials of the mean fitness landscape. Fitness-landscape partials are critical because these are what quantifies the nature of phenotypic selection in both the multivariate response equations when the distribution of breeding values is multivariate normal (*[See Table 30.1 at the end of this section.]*) and also in the more general Barton-Turelli expression (Equation 24.26) for when normality is not assumed. The Morrissey-Sakrejda gradient for trait i is
...
## Evolution_chapter30_022 · MULTIVARIATE SEMIPARAMETRIC FITNESS SURFACE ESTIMATION / Calsbeek's Tensor Approach for Detecting Variation in Fitness Surfaces
...
## Evolution_chapter30_023 · MULTIVARIATE SEMIPARAMETRIC FITNESS SURFACE ESTIMATION / Model Selection
...
$$
\mathrm{AIC}_{c}=-2\ln(L)+2k+\frac{2k(k+1)}{n-k-1}=-2\ln(L)+\frac{2kn}{n-k-1}
$$
...
which was introduced by Schwarz (1978), and thus is also known as the Schwarz criterion. While AIC and BIC are often used interchangeably, they are actually designed for slightly different purposes. When one of the models being compared is the true model, then BIC picks this model with a probability approaching one in large samples. Conversely, AIC considers the situation where none of the candidate models may be correct and then tries to pick among the best of these. As noted by Shaw and Geyer (2010), if one is just considering a few traits, BIC should be used, but with a large number of traits AIC (or AICc) is a better choice. Investigators often report both, as the two metrics often rank models differently.

---

## Evolution_chapter30_024 · Measuring Multivariate Selection: Introduction / THE STRENGTH AND PATTERN OF SELECTION IN NATURAL POPULATIONS
...
## Evolution_chapter30_025 · THE STRENGTH AND PATTERN OF SELECTION IN NATURAL POPULATIONS / Meta-analysis
...
## Evolution_chapter30_026 · THE STRENGTH AND PATTERN OF SELECTION IN NATURAL POPULATIONS / Kingsolver's Analysis
...
> **Figure 30.5** · page 28 · source: `Evolution_chapter30`
>
> ![Figure 30.5](figures/Evolution_30.5.png)
>
...
> **Figure 30.6** · page 29 · source: `Evolution_chapter30`
>
> ![Figure 30.6](figures/Evolution_30.6.png)
>
...
## Evolution_chapter30_027 · THE STRENGTH AND PATTERN OF SELECTION IN NATURAL POPULATIONS / Directional Selection: Strong or Weak?
...
Conversely, if concerns about publication bias and low power are correct, the true median value of $ |\beta_{sd}| $ may be substantially less than the “weak” median value of 0.16 found by Kingsolver. Hersch and Phillips (2004) and Knapczyk and Conner (2007) examined the validity of these concerns. Figure 29.9 presented power curves for a univariate analysis (a single-trait fitness regression), which is a function of $ \rho = \bar{\gamma}/\sqrt{I} $. Hersch and Phillips showed that when multiple (potentially correlated) traits are considered, the adjusted version of the trait-fitness correlation, $ \rho $, used in power calculations becomes
...
> **Figure 30.7** · page 31 · source: `Evolution_chapter30`
>
> ![Figure 30.7](figures/Evolution_30.7.png)
>
...
> **Figure 30.9** · page 33 · source: `Evolution_chapter30`
>
> ![Figure 30.9](figures/Evolution_30.9.png)
>
...
> **Figure 30.8** · page 32 · source: `Evolution_chapter30`
>
> ![Figure 30.8](figures/Evolution_30.8.png)
>
...
## Evolution_chapter30_028 · THE STRENGTH AND PATTERN OF SELECTION IN NATURAL POPULATIONS / Total Versus Direct Selection, Tradeoffs, and Temporal Variation
...
## Evolution_chapter30_029 · THE STRENGTH AND PATTERN OF SELECTION IN NATURAL POPULATIONS / Directional Selection on Body Size and Cope's Law
...
> **Figure 30.10** · page 36 · source: `Evolution_chapter30`
>
> ![Figure 30.10](figures/Evolution_30.10.png)
>
...
## Evolution_chapter30_030 · THE STRENGTH AND PATTERN OF SELECTION IN NATURAL POPULATIONS / Quadratic Selection: Strong or Weak?
...
## Evolution_chapter30_031 · THE STRENGTH AND PATTERN OF SELECTION IN NATURAL POPULATIONS / Where Is All the Stabilizing Selection?
...
## Evolution_chapter30_032 · THE STRENGTH AND PATTERN OF SELECTION IN NATURAL POPULATIONS / A Plea to Fully Publish
...
## Evolution_chapter30_033 · Measuring Multivariate Selection: Introduction / UNMEASURED CHARACTERS AND OTHER BIOLOGICAL CAVEATS
...
> **Figure 30.11** · page 38 · source: `Evolution_chapter30`
>
> ![Figure 30.11](figures/Evolution_30.11.png)
>
...
> **Figure 30.12** · page 38 · source: `Evolution_chapter30`
>
> ![Figure 30.12](figures/Evolution_30.12.png)
>
...
## Evolution_chapter30_034 · Measuring Multivariate Selection: Introduction / PATH ANALYSIS AND FITNESS ESTIMATION
...
> **Figure 30.13** · page 39 · source: `Evolution_chapter30`
>
> ![Figure 30.13](figures/Evolution_30.13.png)
>
...
> **Figure 30.14** · page 41 · source: `Evolution_chapter30`
>
> ![Figure 30.14](figures/Evolution_30.14.png)
>
...
> **Figure 30.16** · page 47 · source: `Evolution_chapter30`
>
> ![Figure 30.16](figures/Evolution_30.16.png)
>
...
One potential path diagram is seed weight → height → fitness, which states that seed weight only impacts fitness through its effect on plant height. A path analysis of this model first variance-standardizes all of the variables and then performs regressions (or partial regressions), generating correlations among the items in the path. Suppose the resulting coefficients are

$$
\begin{aligned}weight\quad\xrightarrow{0.35}\quad height\quad\xrightarrow{0.15}\quad w\end{aligned}
$$

implying a correlation of $ \rho = 0.35 $ between seed weight and plant height, and a correlation of 0.15 between height and fitness. As drawn, this diagram implies that seed weight does not have a direct effect on fitness, but it does have an indirect effect through height. Because $ \rho^2 $ is the fraction of variation accounted for by a factor, we see that seed weight explains 13% ($ 0.35^2 = 0.13 $) of the variance in plant height and that plant height accounts for 2% ($ 0.15^2 $) of the variance in fitness. Further, the correlation between any two connected items in a path is the product of their path coefficients, giving a correlation between seed weight and fitness of $ 0.35 \cdot 0.15 = 0.0525 $, showing that seed weight accounts for 0.3% ($ 0.0525^2 $) of the variance in fitness.
...
## Evolution_chapter30_035 · PATH ANALYSIS AND FITNESS ESTIMATION / Regressions Versus Path Analysis
...
## Evolution_chapter30_036 · PATH ANALYSIS AND FITNESS ESTIMATION / germination, $ g \rightarrow $ vegetative growth rate, $ r \rightarrow $ flowering time, f
...
## Evolution_chapter30_037 · PATH ANALYSIS AND FITNESS ESTIMATION / Morrissey's Extended Selection Gradient Vector, $ \eta $

Consider the following two path diagrams connecting a focal trait (z), fitness (w), and some unmeasured factor (f):

$$
\operatorname{path}1\colon z\leftarrow f\rightarrow w\qquad\qquad\operatorname{path}2\colon z\rightarrow f\rightarrow w
$$
...
$$
\boldsymbol{\varPhi}=(\mathbf{I}-\mathbf{B})^{-1}
$$
...
## Evolution_chapter30_038 · PATH ANALYSIS AND FITNESS ESTIMATION / Selection on Latent Variables
...
> **Figure 30.15** · page 46 · source: `Evolution_chapter30`
>
> ![Figure 30.15](figures/Evolution_30.15.png)
>
...
## Evolution_chapter30_039 · PATH ANALYSIS AND FITNESS ESTIMATION / Elasticity Path Diagrams
...
## Evolution_chapter30_040 · Measuring Multivariate Selection: Introduction / LEVELS OF SELECTION
...
## Evolution_chapter30_041 · LEVELS OF SELECTION / Contextual Analysis
...
## Evolution_chapter30_042 · LEVELS OF SELECTION / Selection Can Be Antagonistic Across Levels
...
## Evolution_chapter30_043 · LEVELS OF SELECTION / Group Selection Is Likely Density-Dependent
...
## Evolution_chapter30_044 · LEVELS OF SELECTION / Selection Differentials Can Be Misleading in Levels of Selection
...
$$
\sigma(z,w)=E_{i}\left[\sigma(w_{i j},z_{i j})\right]+\sigma(\overline{{z}}_{i},\overline{{w}}_{i})
$$
...
## Evolution_chapter30_045 · LEVELS OF SELECTION / Hard, Soft, and Group Selection: A Contextual Analysis Viewpoint
...
*[See Table 30.2 at the end of this section.]*

> **Table 30.2** · `30.2` · page 53 · source: `Evolution_chapter30_045`
...
## Evolution_chapter30_046 · LEVELS OF SELECTION / Early Survival: Offspring or Maternal Fitness Component?
...
> **Figure 30.18** · page 55 · source: `Evolution_chapter30`
>
> ![Figure 30.18](figures/Evolution_30.18.png)
>
...
$$
w_{o(t)}+w_{m(t-1)}=\left(\alpha_{o(t)}+\alpha_{m(t-1)}\right)+b_{m}z_{m(t-1)}+b_{d}z_{m(t-1)}+b_{o}z_{o(t)}+\left(\epsilon_{(t)}+\epsilon_{(t-1)}\right)
$$
