...
## Evolution_chapter19_001 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction
...
## Evolution_chapter19_002 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / MIXED-MODEL VERSUS LEAST-SQUARES ANALYSIS
...
> **Figure 19.1** · page 2 · source: `Evolution_chapter19`
>
> ![Figure 19.1](figures/Evolution_19.1.png)
>
...
## Evolution_chapter19_003 · MIXED-MODEL VERSUS LEAST-SQUARES ANALYSIS / BLUP Selection
...
## Evolution_chapter19_004 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / BASICS OF MIXED-MODEL ANALYSIS
...
$$
\begin{pmatrix}\mathbf{X}^{T}\mathbf{X}&\mathbf{X}^{T}\mathbf{Z}\\\mathbf{Z}^{T}\mathbf{X}&\mathbf{Z}^{T}\mathbf{Z}+\lambda\mathbf{A}^{-1}\end{pmatrix}\begin{pmatrix}\widehat{\beta}\\\widehat{\mathbf{a}}\end{pmatrix}=\begin{pmatrix}\mathbf{X}^{T}\mathbf{y}\\\mathbf{Z}^{T}\mathbf{y}\end{pmatrix}
$$
...
## Evolution_chapter19_005 · BASICS OF MIXED-MODEL ANALYSIS / REML Estimation of Unknown Variance Components
...
## Evolution_chapter19_006 · BASICS OF MIXED-MODEL ANALYSIS / REML Often Returns Variance Estimates Unbiased by Selection
...
## Evolution_chapter19_007 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / ANIMAL-MODEL ANALYSIS OF SELECTION EXPERIMENTS
...
## Evolution_chapter19_008 · ANIMAL-MODEL ANALYSIS OF SELECTION EXPERIMENTS / The Basic Animal Model
...
To apply the animal model to a selection experiment, one first vectorizes the observations from the entire experiment by letting $ y_{ij} $ denote the trait measurement on the $ j $th individual from generation $ i $, where $ 0 \leq i \leq t $ (generation 0 represents the unselected base population) and $ 1 \leq j \leq n_i $, where $ n_i $ is the number of measured individuals in generation $ i $. Let the vector $ y $ denote the observations on all measured individuals from the entire experiment

$$
\mathbf{y}=\begin{pmatrix}\mathbf{y}_{0}\\ \mathbf{y}_{1}\\ \vdots\\ \mathbf{y}_{t}\end{pmatrix}\qquad where\qquad\mathbf{y}_{i}=\begin{pmatrix}\mathbf{y}_{i1}\\ \vdots\\ \mathbf{y}_{in_{i}}\end{pmatrix}
$$
...
## Evolution_chapter19_009 · ANIMAL-MODEL ANALYSIS OF SELECTION EXPERIMENTS / Response Is Measured by Change in Mean Breeding Values
...
## Evolution_chapter19_010 · ANIMAL-MODEL ANALYSIS OF SELECTION EXPERIMENTS / Fixed Effects Alter Heritabilities
...
## Evolution_chapter19_011 · ANIMAL-MODEL ANALYSIS OF SELECTION EXPERIMENTS / Model Validation
...
## Evolution_chapter19_012 · ANIMAL-MODEL ANALYSIS OF SELECTION EXPERIMENTS / Separating Genetic and Environmental Trends
...
For $T$ generations, there are $T$ estimable fixed effects ($\mu$ and all but one $b_t$). Typically, one either constrains the $b_t$ to sum to zero or arbitrarily sets one of the $b_t$ equal to zero. We do the latter with $b_1$, so the vector of fixed effects becomes

$$
\boldsymbol{\beta}=(\mu,b_{2},\cdots,b_{T})^{T}
$$
...
$$
\mathbf{X}=\begin{pmatrix}{{{1}}}&{{{0}}}&{{{\cdots}}}&{{{0}}} \\{{{1}}}&{{{0}}}&{{{\cdots}}}&{{{0}}} \\{{{\vdots}}}&{{{\vdots}}} \\{{{1}}}&{{{1}}}&{{{\cdots}}}&{{{0}}} \\{{{1}}}&{{{1}}}&{{{\cdots}}}&{{{0}}} \\{{{\vdots}}}&{{{\vdots}}} \\{{{1}}}&{{{0}}}&{{{\cdots}}}&{{{1}}} \\{{{1}}}&{{{0}}}&{{{\cdots}}}&{{{1}}}\end{pmatrix}
$$
...
## Evolution_chapter19_013 · ANIMAL-MODEL ANALYSIS OF SELECTION EXPERIMENTS / Validation That a Trend Is Indeed Genetic
...
> **Figure 19.2** · page 16 · source: `Evolution_chapter19`
>
> ![Figure 19.2](figures/Evolution_19.2.png)
>
...
## Evolution_chapter19_014 · ANIMAL-MODEL ANALYSIS OF SELECTION EXPERIMENTS / Replicate Lines
...
## Evolution_chapter19_016 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / THE RELATIONSHIP MATRIX, A, ACCOUNTS FOR DRIFT AND DISEQUILIBRIUM
...
## Evolution_chapter19_017 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / MODIFICATIONS OF THE BASIC ANIMAL MODEL
...
## Evolution_chapter19_018 · MODIFICATIONS OF THE BASIC ANIMAL MODEL / Models with Additional Random Effects

We have assumed that residuals of a mixed model are uncorrelated and homoscedastic, making their covariance matrix $ \mathbf{Var}(\mathbf{e}) = \sigma_e^2 \mathbf{I} $. When additional random effects are present but ignored by the model, they are inadvertently subsumed into the residuals, introducing correlations and heteroscedasticity, which, in turn, may bias the BLUP estimates. For example, if sibs share a common maternal environment, this introduces correlations between them beyond those accounted for by A. If the model only includes a and e, this additional covariance appears between the residuals of sibs, and the true covariance matrix for e is no longer the assumed diagonal, which leads to biased estimates of the BLUEs and BLUPs. By suitably incorporating additional random effects, we can develop a new model, in which the residuals again have the simple OLS covariance structure, $ \mathbf{Var}(\mathbf{e}) = \sigma_e^2 \mathbf{I} $.
...
## Evolution_chapter19_019 · MODIFICATIONS OF THE BASIC ANIMAL MODEL / Common Family and Maternal Effects
...
## Evolution_chapter19_020 · MODIFICATIONS OF THE BASIC ANIMAL MODEL / Treating Certain Breeding Values as Fixed Effects
...
## Evolution_chapter19_021 · MODIFICATIONS OF THE BASIC ANIMAL MODEL / Dominance
...
## Evolution_chapter19_022 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / BAYESIAN MIXED-MODEL ANALYSIS
...
## Evolution_chapter19_023 · BAYESIAN MIXED-MODEL ANALYSIS / Introduction to Bayesian Statistics
...
$$
p(\mathbf{y}\mid\mu)=\frac{1}{\sqrt{2\pi\sigma^{2}}}\exp\left(-\sum_{i=1}^{n}\frac{(y_{i}-\mu)^{2}}{2\sigma^{2}}\right)
$$
...
## Evolution_chapter19_024 · BAYESIAN MIXED-MODEL ANALYSIS / Computing Posteriors and Marginals: MCMC and the Gibbs Sampler
...
$$
\begin{aligned}&w_{i}\sim p(w\mid x=x_{i-1},y=y_{i-1},z=z_{i-1})\\&x_{i}\sim p(x\mid w=w_{i},y=y_{i-1},z=z_{i-1})\\&y_{i}\sim p(y\mid w=w_{i},x=x_{i},z=z_{i-1})\\&z_{i}\sim p(z\mid w=w_{i},x=x_{i},y=y_{i})\\ \end{aligned}
$$
...
## Evolution_chapter19_025 · BAYESIAN MIXED-MODEL ANALYSIS / Bayesian Analysis of the Animal Model
...
$$
\mathbf{y}=\mathbf{X}\boldsymbol{\beta}+\mathbf{Z}\mathbf{a}+\mathbf{e},\quad where\quad\mathbf{y}\sim MVN(\mathbf{X}\boldsymbol{\beta},\sigma_{A}^{2}\mathbf{Z}\mathbf{A}\mathbf{Z}^{T}+\sigma_{e}^{2}\mathbf{I})
$$

The (unconditional) mean and variance for this model follow from $ E[\mathbf{y}] = \mathbf{X} / \beta $, and $ \operatorname{Var}(\mathbf{y}) = \operatorname{Var}(\mathbf{Z}a + e) = \sigma_A^2 \mathbf{Z} \mathbf{Z}^T + \sigma_e^2 \mathbf{I} $. Wang et al. (1993, 1994a, 1994b), Sorensen et al. (1994), and Jensen et al. (1994) developed Gibbs samplers for this model and its extensions. As before, the conditional distribution of the data, given the vectors of fixed effects, $ \beta $, breeding values, a, and the environmental variance, $ \sigma_e^2 $, is multivariate normal

$$
\mathbf{y}\mid\beta,\mathbf{a},\sigma_{e}^{2}\sim\mathrm{MVN}(\mathbf{X}\beta+\mathbf{Z}\mathbf{a},\sigma_{e}^{2}\mathbf{I})
$$
...
2. Using the current values of a, $ \beta $, $ \sigma_{A}^{2} $, and $ \sigma_{e}^{2} $ and the conditional distributions (see Sorensen et al. 1994 for exact expressions): (i) Update the fixed effects by sequentially drawing (for $ j = 1, \cdots, q $) from the conditionals (which are univariate normals)

$$
\beta_{j,i}\sim p(\beta_{j}\mid\beta_{1,i},\cdot,\beta_{j-1,i},\beta_{j+1,i-1},\cdot,\beta_{q,i-1},\mathbf{a}_{i-1},\sigma_{A,i-1}^{2},\sigma_{e,i-1}^{2})
$$

where $ \beta_{j,i} $ is the value of $ \beta_j $ during the $ i $th iteration of the sample. The current values of these parameters define the mean and variance for a normal, from which a random value is drawn. For factor $ j $, we take the values for a and the variances from the last iteration $ (i-1) $, the values of $ \beta_1 $ to $ \beta_{j-1} $ from the current iteration $ (i) $, and the values of $ \beta_{j+1} $ to $ \beta_q $ from the last iteration $ (i-1) $. These values are inserted to give the parameters (here, the conditional mean and variance) for the univariate normal that corresponds to the conditional distribution for $ \beta_j $ and a random variable is drawn from this distribution to yield $ \beta_{j,i} $.
...
$$
a_{j,i}\sim p(a_{j}\mid\beta_{i},a_{1,i},\cdots,a_{j-1,i},a_{j+1,i-1},\cdots,a_{p,i-1},\sigma_{A,i-1}^{2},\sigma_{e,i-1}^{2})
$$

(iii) Update the additive variance by drawing from the conditional (a scaled inverse- $ \chi^{2} $ distribution)

$$
\sigma_{A,i}^{2}\sim p(\sigma_{A}^{2}\mid\boldsymbol{\beta}_{i},\mathbf{a}_{i},\sigma_{e,i-1}^{2})
$$

(iv) Update the error variance by drawing from the conditional (again, a scaled inverse- $ \chi^{2} $ distribution)

$$
\sigma_{e,i}^{2}\sim p(\sigma_{e}^{2}\mid\boldsymbol{\beta}_{i},\mathbf{a}_{i},\sigma_{A,i}^{2})
$$
...
## Evolution_chapter19_026 · BAYESIAN MIXED-MODEL ANALYSIS / Application: Estimating Selection Response in Pig Litter-size Components
...
$$
\mathbf{y}\left|\boldsymbol{\beta},\mathbf{a},\sigma_{e}^{2}\sim\mathrm{MVN}(\mathbf{X}\boldsymbol{\beta}+\mathbf{Z}\mathbf{a},\mathbf{I}\sigma_{e}^{2})\right.\quad\mathrm{a n d}\quad\mathbf{a}\left|\mathbf{A},\sigma_{A}^{2}\sim\mathrm{MVN}(\mathbf{0},\mathbf{A}\sigma_{A}^{2})\right.
$$
...
$$
\mathbf{y}\left|\beta,\mathbf{a},\mathbf{c},\sigma_{e}^{2}\right.\sim\mathrm{M V N}(\mathbf{X}\beta+\mathbf{Z}\mathbf{a}+\mathbf{W}\mathbf{c},\mathbf{I}\sigma_{e}^{2})
$$

$$
\mathbf{a}\mid\mathbf{A},\sigma_{A}^{2}\sim\mathrm{M V N}(\mathbf{0},\mathbf{A}\sigma_{A}^{2})\quad\mathrm{a n d}\quad\mathbf{c}\mid\sigma_{c}^{2}\sim\mathrm{M V N}(\mathbf{0},\mathbf{I}\sigma_{c}^{2})
$$
...
> **Figure 19.3** · page 37 · source: `Evolution_chapter19`
>
> ![Figure 19.3](figures/Evolution_19.3.png)
>
...
> **Table 19.1** · `19.1` · page 38 · source: `Evolution_chapter19_026`
...
## Evolution_chapter19_027 · BAYESIAN MIXED-MODEL ANALYSIS / LS, MM, OR BAYES?
