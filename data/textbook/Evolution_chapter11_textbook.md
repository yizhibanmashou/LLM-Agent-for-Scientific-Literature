...
## Evolution_chapter11_001 · Changes in Genetic Variation Induced by Drift: Introduction
...
## Evolution_chapter11_002 · RESPONSE OF WITHIN-POPULATION GENETIC VARIANCE TO DRIFT / Complete Additivity
...
## Evolution_chapter11_003 · RESPONSE OF WITHIN-POPULATION GENETIC VARIANCE TO DRIFT / The Effects of Dominance
...
$$
\sigma_{D I}^{2}=\sum_{k=1}^{n}\sum_{i=1}^{n_{k}}\left(p_{k i}\delta_{k i i}^{2}-\iota_{k}^{2}\right)=\sum_{k=1}^{n}\left(E[\delta_{k i i}^{2}]-\iota_{k}^{2}\right)
$$
...
$$
\sigma_{ADI}=2\sum_{k=1}^{n}\sum_{i=1}^{n_{k}}p_{ki}\alpha_{ki}\delta_{kii}=2\sum_{k=1}^{n}E[\alpha_{ki}\delta_{kii}]
$$

and dominance effects in inbred individuals variance depend on seven quadratic properties of the base population (*[See Table 11.1 at the end of this section.]*), as well as on several expectations for the higher-order moments of allele and gamete frequencies. We first present the general model, and then consider some illuminating results that arise under special conditions.

> **Table 11.1** · `11.1` · page 3 · source: `Evolution_chapter11_003`
...
> **Table 11.2** · `11.2` · page 4 · source: `Evolution_chapter11_004`
...
*[See Table 11.3 at the end of this section.]*

> **Table 11.3** · `11.3` · page 7 · source: `Evolution_chapter11_005`
...
## Evolution_chapter11_006 · RESPONSE OF WITHIN-POPULATION GENETIC VARIANCE TO DRIFT / Impact of Drift Under Nonadditive Variance

With definitions in hand for the quadratic expressions in the base population (*[See Table 11.1 at the end of this section.]*) and the temporal dynamics of the identity coefficients (Equations 11.3 and 11.4), we are now in a position to explore the impact of finite population size on the components of variance for a quantitative trait with a nonadditive genetic basis. The expected dynamics are determined by summing the products of the seven quadratic terms listed across the top of *[See Table 11.3 at the end of this section.]* with their associated tabulated identity coefficients in the table. For example, the expected within-population dominance variance is

$$
[1-3f+2(\Delta+\gamma-\delta)]\sigma_{D}^{2}+(f+\delta-2\gamma)\sigma_{D I}^{2}+(f+\Delta-2\gamma)\iota^{*}+(\widetilde{f}-2\widetilde{\gamma}+\widetilde{\Delta})(\iota^{2}-\iota^{*})
$$

Here, $ \sigma_{D}^{2} $, $ \sigma_{DI}^{2} $, etc. are the base-population values of these components under random mating.

To gain a more intuitive feel for the source of the expressions in *[See Table 11.3 at the end of this section.]*, we first consider the total genetic variance in a collection of lines, each inbred to level $ f $, while ignoring epistasis until the following section. Subscripting loci with $ k $ and the two alleles at a locus as $ i $ and $ j $, the genotypic value, $ G $ (expressed as a deviation from the mean), of an individual can be written as
...
To compute the mean genotypic value, $ \mu_G = E[G] $, note that the expected value of $ \phi_{kij} $ is $ f $ and that we can write the expected value of $ G $ as two components: if not inbred $ (\phi_{kij} = 0) $, then $ E[\alpha_{ki} + \alpha_{kj} + \delta_{kij}] = 0 $, as these are deviations from the mean, leaving

$$
E[G]=E\left[\sum_{k=1}^{n}\phi_{k i j}(2\alpha_{k i}+\delta_{k i i})\right]=f\left[\sum_{k=1}^{n}\sum_{i=1}^{n_{k}}p_{k i}(2\alpha_{k i}+\delta_{k i i})\right]
$$
...
$$
E[G]=\mu_{G}=f\sum_{k=1}^{n}\sum_{i=1}^{n_{k}}\delta_{kii}p_{ki}=f\cdot\iota
$$

the last step following from the definition of $ \iota $ (*[See Table 11.1 at the end of this section.]*). Under complete inbreeding, $ \mu_G = \iota $, the change in the mean from inbreeding depression (as $ \mu_G = 0 $ under random mating). In addition,
...
Further simplification is achieved by adding and subtracting $ f(1 - f)\iota^* $ on the right side of this expression, which, after using the expressions in *[See Table 11.1 at the end of this section.]*, leads to

$$
\sigma_{G}^{2}=(1+f)\sigma_{A}^{2}+(1-f)\sigma_{D}^{2}+2f\sigma_{ADI}+f\sigma_{DI}^{2}+f(1-f)\iota^{*}+(\widetilde{f}-f^{2})(\iota^{2}-\iota^{*})
$$

in agreement with the final row in *[See Table 11.3 at the end of this section.]*.

Although the preceding results apply to the genetic variance summed within and among a set of hypothetical isolated subpopulations, an expression for the average within-population variance can be obtained by removing the among-population component. The simplest route to this result is to recall the general rule that the variance among groups is equivalent to the covariance between individuals within groups (e.g., LW Chapter 18). Using this principle, the contribution of each quadratic component in *[See Table 11.1 at the end of this section.]* to the among-population variance can be obtained in the following way.
...
Although it may not be immediately apparent, the coefficients in the final two (bottom) columns in *[See Table 11.3 at the end of this section.]* are equivalent to measures of identity disequilibrium (Cockerham 1984a). For example, $ \widetilde{f} - f^2 $ is the deviation of the double identity-by-descent within gametes in the same population from that based on the assumption of independence between loci. This difference is simply the logical extension of the notion of disequilibrium, where we defined $ D_{AB} = p_{AB} - p_{APB} $, with $ \widetilde{f} $ being the analog of $ p_{AB} $ (the frequency of the AB gamete) and $ f $ being the analog of $ p_A $ and $ p_B $. Although $ \widetilde{f} $ depends upon the average linkage relationships between all relevant pairs of loci (Weir and Cockerham 1968, 1969), in most cases, if most pairs of loci are on different chromosomes, or if the population is randomly mated and expanded following the bottleneck, $ \widetilde{f} $ will be approximately equal to $ f^2 $. Under such conditions, $ (\widetilde{f} - f^2) $, as well as the other coefficients of $ (\iota^2 - \iota^*) $ in *[See Table 11.3 at the end of this section.]*, will be very close to zero, removing at least this one term from the variance expressions (Figure 11.2).

---

## Evolution_chapter11_007 · RESPONSE OF WITHIN-POPULATION GENETIC VARIANCE TO DRIFT / The Effects of Epistasis

The fundamental point in the preceding section is that because dominance is a function of a two-allele interaction, the variance in dominance effects can be altered in unexpected ways when inbreeding alters the average background on which an allele appears. This same issue applies to epistatic effects, although on a potentially larger scale because the additive × additive epistatic variance ($ \sigma_{AA}^2 $) is a function of $ n^2 $ terms, while all of the other quadratic components in *[See Table 11.1 at the end of this section.]* (except $ \iota^2 - \iota^* $, which seems to be of little significance) are functions of only $ n $ terms. If we assume unlinked loci, the coefficient of the $ \sigma_{AA}^2 $

In addition, for both of the inbred generations, the observed values of the coefficient of variation for $ \text{Var}(A) $, $ \text{CV}[\text{Var}(A)] $, are close to the theoretical minimum $ \sqrt{8/(3N e)} = 1.04 $ (using $ N e = 2.5 $ for full-sib mating). In principle, several generations of random mating would be expected to cause a reduction in $ \text{CV}[\text{Var}(A)] $ through the elimination of

*[See Table 11.4 at the end of this section.]* contribution to the additive genetic variance rises to nearly 1.0 in a little over $ N_e $ generations, i.e., the equivalent of all of the base-population $ \sigma_{AA}^2 $ is added to the (otherwise declining) additive genetic variance at this point (Figure 11.2).
...
This is simply Equation 11.7a with a dominance term $ \sigma_D^2 $ added. While this expression appears in the literature (e.g., Taft and Roff 2012), comparison with the exact result (given from the A row in *[See Table 11.3 at the end of this section.]*) shows that Equation 11.7c ignores other quadratic components involving dominance ($ \sigma_{ADI} $ and $ \sigma_{DI}^2 $) and the contributions from other identity coefficients besides $ f $, and thus may be a poor approximation in some settings.
...
> **Table 11.4** · `11.4` · page 11 · source: `Evolution_chapter11_007`
...
## Evolution_chapter11_008 · RESPONSE OF WITHIN-POPULATION GENETIC VARIANCE TO DRIFT / Sampling Error
...
## Evolution_chapter11_009 · RESPONSE OF WITHIN-POPULATION GENETIC VARIANCE TO DRIFT / Empirical Data
...
> **Figure 11.3** · page 16 · source: `Evolution_chapter11`
>
> ![Figure 11.3](figures/Evolution_11.3.png)
>
...
> **Figure 11.4** · page 18 · source: `Evolution_chapter11`
>
> ![Figure 11.4](figures/Evolution_11.4.png)
>
...
$$
R=\ln\left(\frac{V_{A}(bott)}{V_{A}(cont)}\right)=\ln(1-f)
$$
...
Some attention has been given to the contribution of additive × additive genetic variance to the resemblance between inbred relatives (Cockerham 1984b; Cockerham and Tachida 1988; Tachida and Cockerham 1989). In this case, Equation 11.13 requires an additional term,
...
$$
\sigma_{G}(x_{i},y_{j},t)=(1+f_{t})\sigma_{A}^{2}
$$
...
> **Figure 11.6** · page 25 · source: `Evolution_chapter11`
>
> ![Figure 11.6](figures/Evolution_11.6.png)
>
...
*[See Table 11.6 at the end of this section.]*

> **Table 11.5** · `11.5` · page 23 · source: `Evolution_chapter11_010`
...
> **Table 11.6** · `11.6` · page 24 · source: `Evolution_chapter11_010`
...
## Evolution_chapter11_011 · COVARIANCE BETWEEN INBRED RELATIVES / REML Estimates
...
as obtained by Smith and Mäiki-Tanila (1990), de Boer and Hoeschele (1993), Shaw and Woolliams (1999), and Abney et al. (2000). This approach is computationally demanding, *[See Table 11.7 at the end of this section.]* as the standard iterative approach used to obtain the REML estimates must invert the V matrix following each update of its component variances (LW Chapter 27).
...
*[See Table 11.8 at the end of this section.]*

> **Table 11.8** · `11.8` · page 28 · source: `Evolution_chapter11_012`
...
## Evolution_chapter11_013 · Changes in Genetic Variation Induced by Drift: Introduction / DRIFT-MUTATION EQUILIBRIUM
...
> **Figure 11.7** · page 30 · source: `Evolution_chapter11`
>
> ![Figure 11.7](figures/Evolution_11.7.png)
>
...
## Evolution_chapter11_014 · DRIFT-MUTATION EQUILIBRIUM / Subdivided Populations
