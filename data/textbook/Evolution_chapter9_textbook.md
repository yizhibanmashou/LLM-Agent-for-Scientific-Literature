...
## Evolution_chapter9_001 · Using Molecular Data to Detect Selection: Signatures from Recent Single Events: Introduction
...
## Evolution_chapter9_002 · Using Molecular Data to Detect Selection: Signatures from Recent Single Events: Introduction / AN OVERVIEW OF STRATEGIES BASED ON SEGREGATING VARIATION
...
## Evolution_chapter9_004 · AN OVERVIEW OF STRATEGIES BASED ON SEGREGATING VARIATION / SNP Ascertainment Bias
...
## Evolution_chapter9_005 · AN OVERVIEW OF STRATEGIES BASED ON SEGREGATING VARIATION / SNP Polarity Assignment Errors
...
## Evolution_chapter9_006 · AN OVERVIEW OF STRATEGIES BASED ON SEGREGATING VARIATION / Background Selection as the More Appropriate Null?
...
## Evolution_chapter9_007 · AN OVERVIEW OF STRATEGIES BASED ON SEGREGATING VARIATION / Structure of the Remainder of This Chapter
...
## Evolution_chapter9_008 · Using Molecular Data to Detect Selection: Signatures from Recent Single Events: Introduction / ALLELE-FREQUENCY CHANGE IN A SINGLE POPULATION
...
This divergence has an expected value of zero (as $ E[\widehat{p} {t}] = p {0} $), with a variance of

**[推导 Derivation]**

$$
\sigma^{2}\left(\widehat{\delta} {t}\right)=\sigma^{2}\left(\widehat{p} {t}-\widehat{p} {0}\right)=\sigma^{2}\left(\widehat{p} {t}\right)+\sigma^{2}\left(\widehat{p} {0}\right)-2\sigma\left(\widehat{p} {t},\widehat{p} {0}\right)
$$

where $ \widehat{p} {i}=p {i}+e {i} $, the true value plus an error due to finite sampling. Because these are draws from a binomial distribution, the sampling variance of the initial frequency is

**[推导 Derivation]**

$$
\sigma^{2}\left(\widehat{p} {0}\right)=\frac{p {0}\left(1-p {0}\right)}{2n {0}}
$$
...
$$
\begin{aligned}\sigma^2(\widehat{p}_t)&=p_0(1-p_0)\left[1-\left(1-\frac{1}{2n_t}\right)\left(1-\frac{1}{2N_e}\right)^t\right]\\ &\simeq p_0(1-p_0)\left[\frac{1}{2n_t}+\frac{t}{2N_e}\left(1-\frac{1}{2n_t}\right)\right]\end{aligned}
$$

If sampling is done (without replacement) before reproduction, then $ \sigma(\widehat{p} {t},\widehat{p} {0})=0 $, and substitution of Equations 9.2a and 9.2b into Equation 9.1b yields

If sampling is done either after reproduction or with replacement, this generates a covariance between the sample estimators $ \widehat{p} t $ and $ \widehat{p} 0 $; see Nei and Tajima (1981b) and especially Waples (1989a, 1989b) for details. Assuming this is not the case, so that $ \sigma(\widehat{p} 0, \widehat{p} t) = 0 $, Equation 9.2c yields the correct variance for the null hypothesis of random genetic drift, giving the test statistic as

**[推导 Derivation]**

$$
\frac{\widehat{\delta} {t}^{2}}{\sigma^{2}\left(\widehat{\delta} {t}\right)}
$$
...
$$
\sigma^{2}(p {t})=p {0}(1-p {0})\left[1-\left(1-\frac{1}{2N {e}}\right)^{t}\right]\simeq p {0}(1-p {0})\frac{t}{2N {e}}\quad\mathrm{f o r}t\ll N {e}
$$

where $ p {0} $ is the initial allele frequency (Equation 2.14a). Consider a population sampled at two time points (0 and t), with sample sizes of $ n {0} $ and $ n {t} $, respectively. The estimated divergence is

**[推导 Derivation]**

$$
\widehat{\delta} {t}=\widehat{p} {t}-\widehat{p} {0}
$$

which is approximately $ \chi {1}^{2} $-distributed. This is the Waples adjusted test (Waples 1989b).

The application of this test requires an accurate estimate of $ p 0 $ to compute the sample variance (Equation 9.2c). While the sample estimate, $ \widehat{p} 0 $, can be used, this can be improved upon by noting that the expected allele frequency change is zero, meaning that $ \widehat{p} t $ also contributes information about $ p 0 $. A simple average of the two frequencies is not appropriate, as $ \widehat{p} 0 $ has a smaller drift variance and the two estimates may differ in informational value due to differences in sample size, $ n i $. Given these concerns, Schaffer et al. (1977) and Waples (1989b) proposed a generalized (i.e., weighted) least-squares (GLS) estimator (LW Chapter 8) for $ p 0 $. Let $ \mathbf{p} = (\widehat{p} 0, \widehat{p} t)^T $ denote the two sample estimates and denote its sampling variance-covariance matrix by

**[推导 Derivation]**

$$
\mathbf{V}=\left(\begin{array}{cc}\sigma^{2}\left(\widehat{p} {0}\right)&\sigma\left(\widehat{p} {0},\widehat{p} {t}\right)\\ \sigma\left(\widehat{p} {0},\widehat{p} {t}\right)&\sigma^{2}\left(\widehat{p} {t}\right)\end{array}\right)
$$

Finally, let $ \mathbf{1} = (1, 1)^T $ be a vector of ones. The underlying statistical model is $ p i = p 0 + e i $, which can be written in general linear model form as $ \mathbf{p} = p 0 \mathbf{X} + \mathbf{e} $, where $ \mathbf{V} $ is the covariance matrix for the vector, $ \mathbf{e} $, of residuals and $ \mathbf{X} = 1 $. Recalling LW Equation 8.34 for GLS regression, the resulting estimate of $ p 0 $ is given by $ \left( \mathbf{X}^T \mathbf{V}^{-1} \mathbf{X} \right)^{-1} \mathbf{X}^T \mathbf{V}^{-1} \mathbf{p} $, which reduces to
...
where $ b n = \sum {j=1}^{n-1} 1/j^2 $ (Equation 4.4b).
...
$$
\widehat{\theta} {S}=\frac{S}{a {n}}\qquad\widehat{\theta} {\Pi}=\Pi\qquad\widehat{\theta} {1}=\zeta {1},\qquad\widehat{\theta} {1^{ }}=\frac{n}{n-1}\eta {1}
$$
...
$$
\theta\ by\ S/a {n}\quad and\quad\theta^{2}\ by\ \frac{S(S-1)}{a {n}^{2}+b {n}}
$$
...
$$
t=\frac{\widehat{\theta} {i}-\widehat{\theta} {j}}{\sigma(\widehat{\theta} {i}-\widehat{\theta} {j})}
$$
...
## Evolution_chapter9_009 · ALLELE-FREQUENCY CHANGE IN A SINGLE POPULATION / Allele-frequency Change Over Two Sample Points: The Waples Adjusted Test
...
## Evolution_chapter9_010 · ALLELE-FREQUENCY CHANGE IN A SINGLE POPULATION / Allele-frequency Change Over a Times Series: The Fisher-Ford Test
...
## Evolution_chapter9_011 · ALLELE-FREQUENCY CHANGE IN A SINGLE POPULATION / Schaffer's Linear Trend Test
...
$$
\mathbf{X}=\begin{pmatrix}{{{1}}}&{{{0}}} \\{{{1}}}&{{{4}}} \\{{{1}}}&{{{8}}}\end{pmatrix},\qquad\widehat{\boldsymbol{\beta}}=\begin{pmatrix}{{{0.609}}} \\{{{-0.028}}}\end{pmatrix}
$$
...
## Evolution_chapter9_012 · ALLELE-FREQUENCY CHANGE IN A SINGLE POPULATION / Scans and Simulation-based Approaches
...
> **Figure 9.1** · page 12 · source: `Evolution_chapter9`
>
> ![Figure 9.1](figures/Evolution_9.1.png)
>
...
## Evolution_chapter9_013 · ALLELE-FREQUENCY CHANGE IN A SINGLE POPULATION / Birthdate Selection Mapping (BDSM)
...
## Evolution_chapter9_014 · Using Molecular Data to Detect Selection: Signatures from Recent Single Events: Introduction / DIVERGENCE BETWEEN POPULATIONS: TWO-POPULATION COMPARISONS
...
## Evolution_chapter9_015 · DIVERGENCE BETWEEN POPULATIONS: TWO-POPULATION COMPARISONS / DIVERGENCE BETWEEN POPULATIONS: $ F_{ST} $-BASED TESTS
...
## Evolution_chapter9_016 · DIVERGENCE BETWEEN POPULATIONS: TWO-POPULATION COMPARISONS / Outlier Approaches
...
## Evolution_chapter9_017 · DIVERGENCE BETWEEN POPULATIONS: TWO-POPULATION COMPARISONS / Tests Based on $ F_{ST} $-generated Branch-lengths
...
> **Figure 9.2** · page 17 · source: `Evolution_chapter9`
>
> ![Figure 9.2](figures/Evolution_9.2.png)
>
...
## Evolution_chapter9_018 · DIVERGENCE BETWEEN POPULATIONS: TWO-POPULATION COMPARISONS / The Lewontin-Krakauer Test: Basics
...
For example, suppose the median for single-locus $ F_{ST} $ values among a collection of loci sampled over five populations is 0.127. Because $ \Pr(\chi_4^2 \leq 3.357) = 0.5 $, the median value of a $ \chi_4^2 $ is 3.357, yielding

$$
\widehat{\lambda}=\frac{\operatorname{med}(F)}{\operatorname{med}(\chi_{n-1}^{2})}=\frac{0.127}{3.357}=0.038
$$
...
## Evolution_chapter9_019 · DIVERGENCE BETWEEN POPULATIONS: TWO-POPULATION COMPARISONS / Second-generation Lewontin-Krakauer Tests: Model-based Adjustments
...
## Evolution_chapter9_020 · DIVERGENCE BETWEEN POPULATIONS: TWO-POPULATION COMPARISONS / Third-generation Lewontin-Krakauer Tests: Correcting for Population Structure
...
## Evolution_chapter9_022 · ALLELE-FREQUENCY CORRELATIONS WITH ENVIRONMENTAL VARIABLES / Joost's Spatial Analysis Method (SAM)
...
## Evolution_chapter9_023 · ALLELE-FREQUENCY CORRELATIONS WITH ENVIRONMENTAL VARIABLES / Accounting for Population Structure: Coop's Bayenv and Frichot's LFMM
...
## Evolution_chapter9_024 · Using Molecular Data to Detect Selection: Signatures from Recent Single Events: Introduction / CHANGES IN THE CHROMOSOMAL PATTERN OF NEUTRAL VARIATION
...
## Evolution_chapter9_025 · CHANGES IN THE CHROMOSOMAL PATTERN OF NEUTRAL VARIATION / Simple Visual Scans for Changes in Nucleotide and STR Diversity
...
> **Figure 9.3** · page 26 · source: `Evolution_chapter9`
>
> ![Figure 9.3](figures/Evolution_9.3.png)
>
...
## Evolution_chapter9_026 · CHANGES IN THE CHROMOSOMAL PATTERN OF NEUTRAL VARIATION / Tests Based on STR Variation Across Populations
...
## Evolution_chapter9_027 · CHANGES IN THE CHROMOSOMAL PATTERN OF NEUTRAL VARIATION / Tests of Sweeps Using Bottleneck Models
...
## Evolution_chapter9_028 · CHANGES IN THE CHROMOSOMAL PATTERN OF NEUTRAL VARIATION / Tests of Sweeps Using Genomic Positional Information: CLRT-GOF
...
## Evolution_chapter9_029 · CHANGES IN THE CHROMOSOMAL PATTERN OF NEUTRAL VARIATION / Tests of Sweeps Using Genomic Positional Information: “SweepFinder”
...
## Evolution_chapter9_030 · Using Molecular Data to Detect Selection: Signatures from Recent Single Events: Introduction / Tests of Sweeps Using Genomic Positional Information: CLRT-GOF
...
## Evolution_chapter9_031 · CHANGES IN THE CHROMOSOMAL PATTERN OF NEUTRAL VARIATION / Ascertainment Issues
...
## Evolution_chapter9_032 · CHANGES IN THE CHROMOSOMAL PATTERN OF NEUTRAL VARIATION / Model Fragility: Demography, Mutation, Recombination, and Gene Conversion
...
## Evolution_chapter9_033 · Using Molecular Data to Detect Selection: Signatures from Recent Single Events: Introduction / TESTS BASED ON SITE-FREQUENCY SPECTRUM STATISTICS
...
$$
\eta_{i}=\zeta_{i}+\zeta_{n-i}\quad for\quad1\leq i\leq[n/2]
$$
...
## Evolution_chapter9_034 · TESTS BASED ON SITE-FREQUENCY SPECTRUM STATISTICS / Summary Statistics Based on Infinite-sites Models
...
> **Table 9.1** · `9.1` · page 37 · source: `Evolution_chapter9_034`
...
## Evolution_chapter9_035 · TESTS BASED ON SITE-FREQUENCY SPECTRUM STATISTICS / Tajima's D Test
...
## Evolution_chapter9_036 · TESTS BASED ON SITE-FREQUENCY SPECTRUM STATISTICS / Achaz's Y and Y* Tests
...
## Evolution_chapter9_037 · TESTS BASED ON SITE-FREQUENCY SPECTRUM STATISTICS / Fu and Li's $ D^{*} $ and $ F^{*} $ Tests
...
## Evolution_chapter9_038 · TESTS BASED ON SITE-FREQUENCY SPECTRUM STATISTICS / Fay and Wu's H Test
...
> **Figure 9.4** · page 42 · source: `Evolution_chapter9`
>
> ![Figure 9.4](figures/Evolution_9.4.png)
>
...
## Evolution_chapter9_039 · TESTS BASED ON SITE-FREQUENCY SPECTRUM STATISTICS / Zeng et al.'s E Test
...
## Evolution_chapter9_044 · HAPLOTYPE-BASED TESTS / Defining and Inferring Haplotypes
...
> **Table 9.2** · `9.2` · page 49 · source: `Evolution_chapter9_045`
...
## Evolution_chapter9_046 · HAPLOTYPE-BASED TESTS / The Ewens-Watterson Test
...
## Evolution_chapter9_047 · HAPLOTYPE-BASED TESTS / Other Infinite-alleles Tests: Conditioning on $ \hat{\theta} $
...
As with W, this is also a one-sided test. $ F_S $ is negative when there is an excess of rare alleles/haplotypes (as would occur with a selective sweep or population expansion), with a sufficiently large negative value serving as evidence for selection or population expansion. Fu (1997) showed that $ F_S $ is more powerful than Tajima's D and the Fu-Li $ D^* $ and $ F^* $ tests (*[See Table 9.1 at the end of this section.]*) for detecting selective sweeps or population expansion following a bottleneck.

---

## Evolution_chapter9_048 · HAPLOTYPE-BASED TESTS / Other Infinite-alleles Tests: Conditioning on S
...
$$
\Pr(n_{i}\geq1 for i\geq j\mid S)
$$
...
## Evolution_chapter9_049 · HAPLOTYPE-BASED TESTS / Garud et al.'s $ H_{12} $ and $ H_{2} $ Tests
...
## Evolution_chapter9_050 · HAPLOTYPE-BASED TESTS / Recombination and Infinite-alleles-based Tests
...
## Evolution_chapter9_051 · HAPLOTYPE-BASED TESTS / Pairwise Disequilibrium Tests: Kelly's $ Z_{nS} $ and Kim and Nielsen's $ \omega_{max} $
...
## Evolution_chapter9_052 · HAPLOTYPE-BASED TESTS / Contrasting Allele-frequency vs. Intra-allelic Variation Estimates of Haplotype Age
...
For large $ n, n/(1 + n\tau_x/2) \simeq 2/\tau_x $, reducing Equation 9.38a to
...
## Evolution_chapter9_053 · HAPLOTYPE-BASED TESTS / Long-haplotype Tests Using Within-population Data: rEHH, LDD, iHS, $ nS_{L} $, SDS, and DIND
...
> **Figure 9.6** · page 61 · source: `Evolution_chapter9`
>
> ![Figure 9.6](figures/Evolution_9.6.png)
>
...
> **Figure 9.7** · page 62 · source: `Evolution_chapter9`
>
> ![Figure 9.7](figures/Evolution_9.7.png)
>
...
## Evolution_chapter9_054 · HAPLOTYPE-BASED TESTS / Long-haplotype Tests Using Between-population Data: XP-EHH, Rsb, rHH, rMHH, and $ \chi_{MD} $
...
## Evolution_chapter9_055 · HAPLOTYPE-BASED TESTS / Tests based on the allele-frequency spectrum AFS(k):
...
> **Table 9.3** · `9.3` · page 65 · source: `Evolution_chapter9_055`
...
> Tests contrasting haplotype lengths of alternative alleles in the same population: Sabeti et al.'s $ r_{EHH} $: Ratio of the haplotype lengths ( $ EHH $) of two alternative alleles Wang et al.'s $ LDD $: Rate of linkage disequilibrium decay, modification of EHH Hanchard et al.'s $ nHS $: Haplotype diversity of the derived allele relative to the ancestral allele Voight et al.'s $ iHS $: Ratio of area under the EHH curve for ancestral vs. derived alleles Ferrer-Admetlla et al.'s $ nS_{L} $: Very similar to $ iHS $, with the number of consecutive shared polymorphic sites replacing the area under the EHH curve Field et al.'s $ SDS $: Distance to nearest singleton, yielding an estimated mean allelic branch length Barreiro et al.'s $ DIND $: Ratio of nucleotide diversity in derived vs. ancestral allele
> Tests contrasting haplotype lengths of the same allele in two populations: Sabeti et al.'s $ XP-EHH $, Tang et al.'s $ ln(Rsb) $: Ratio of area under the EHH curve in different populations Kimura et al.'s $ rHH $ vs. $ rMHH $ plot: Ratios of overall HH to HH based on most frequency haplotype Lange and Poll's $ X_{MD} $ test: Contrast of pairwise haplotype sharing between populations

---

## Evolution_chapter9_056 · HAPLOTYPE-BASED TESTS / Summary: Tests Based on Haplotype/LD Information

As summarized in *[See Table 9.2 at the end of this section.]*, different kinds of sweeps (hard, partial, and soft) leave different haplotype signals. Given the diversity of such signals, it is not surprising that there are a number of haplotype-based tests to detect these different features (*[See Table 9.3 at the end of this section.]*). LD-based tests are generally regarded as the most powerful for sweeps that are currently underway. Site-frequency spectrum tests often perform poorly under a partial sweep, as the distortion in the frequency spectrum is often not sufficiently powerful. Signatures from both a recently completed partial sweep, and a currently ongoing hard sweep, include long haplotypes at excessive frequencies, alleles that are at too high a frequency given other estimates of their age, an excess of one or a few haplotypes, and a reduction in haplotype diversity.
...
## Evolution_chapter9_057 · Using Molecular Data to Detect Selection: Signatures from Recent Single Events: Introduction / SEARCHES FOR SELECTION: HUMANS
...
> **Table 9.4** · `9.4` · page 67 · source: `Evolution_chapter9_057`
...
## Evolution_chapter9_059 · SEARCHES FOR SELECTION: HUMANS / Balancing Selection in Humans
...
## Evolution_chapter9_060 · Using Molecular Data to Detect Selection: Signatures from Recent Single Events: Introduction / SEARCHES FOR SELECTION: DOMESTICATED ORGANISMS
...
## Evolution_chapter9_061 · SEARCHES FOR SELECTION: DOMESTICATED ORGANISMS / The Process of Domestication
...
## Evolution_chapter9_062 · SEARCHES FOR SELECTION: DOMESTICATED ORGANISMS / Finding Domestication and Improvement Genes in Crops
...
## Evolution_chapter9_064 · Using Molecular Data to Detect Selection: Signatures from Recent Single Events: Introduction / Domestication and Improvement Genes in Rice
...
## Evolution_chapter9_065 · SEARCHES FOR SELECTION: DOMESTICATED ORGANISMS / Relative Strengths of Selection on Domestication vs. Improvement Genes
...
## Evolution_chapter9_066 · SEARCHES FOR SELECTION: DOMESTICATED ORGANISMS / Silkmoths and Flies
...
## Evolution_chapter9_067 · SEARCHES FOR SELECTION: DOMESTICATED ORGANISMS / Constraints on Finding Domestication and Improvement Genes Through Selective Signals
