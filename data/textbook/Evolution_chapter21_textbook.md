...
is the covariance between the value of selection unit, x, and the offspring, y, counting the paths through both parents ($ R_{m} $ and $ R_{f} $). When covariances are equal, this is twice the single parent-covariance, $ \sigma(x,y|\mathcal{R}_{1}) $. By analogy with the breeder's equation, Equation 21.1b is often written as
...
> **Table 21.2** · `21.2` · page 13 · source: `Evolution_chapter21_010`
...
## Evolution_chapter21_011 · THEORY OF EXPECTED SINGLE-CYCLE RESPONSE / The Selection Unit-offspring Covariance, $ \sigma(x, y) $
...
The coefficient of coancestry between $ x_1 $ and $ y $ depends upon the relationship between $ \mathcal{R}_1 $ and $ x_1 $. The designs covered in *[See Table 21.1 at the end of this section.]* involve four different relationships (Figure 21.2): (i) $ x_1 = \mathcal{R}_1 $ (a measured sib is a parent of $ y $), (ii) $ x_1 $ and $ \mathcal{R}_1 $ are sibs, (iii) $ \mathcal{R}_1 = P_1 $ (the parent of $ x_1 $), and (iv) $ \mathcal{R}_1 $ is the selfed-progeny of the parent of $ x_1 $. The path diagrams for computing $ \Theta_{x_1y} $ for these four relationships are given in Figure 21.3, and *[See Table 21.2 at the end of this section.]* summarizes the resulting genetic covariances. The parents, $ P_i $, are assumed to be non-inbred (i.e., $ \Theta_{P_i P_i} = 1/2 $). If they are inbred, then $ \Theta_{P_i P_i} = (1 + f_i)/2 $, where $ i $ is the inbreeding coefficient on that parent, and the expressions in *[See Table 21.2 at the end of this section.]* are multiplied by this additional factor (for each inbred parent). As an example of how the coefficients of coancestry given in *[See Table 21.2 at the end of this section.]* are used, consider family selection. Ignoring epistasis,
...
This follows because the first covariance, $ \sigma(z_{ij}, y) $, is for parent and offspring $ (\sigma_A^2/2) $, while the second covariance, $ \sigma(z_{ik}, y) $, follows using the appropriate value of $ 2\Theta $ from *[See Table 21.2 at the end of this section.]* (1/8 for half-sibs and 1/4 for full-sibs). Using the results from *[See Table 21.2 at the end of this section.]*, expressions for the sib selection, parental selection (progeny testing), and $ S_1 $ seed designs follow in similar fashion. These are summarized in *[See Table 21.3 at the end of this section.]*.
...
## Evolution_chapter21_012 · THEORY OF EXPECTED SINGLE-CYCLE RESPONSE / Among-family Selection:

**[Table]**

*[See Table 21.3 at the end of this section.]*

> **Table 21.3** · `21.3` · page 14 · source: `Evolution_chapter21_012`
...
> Sib selection / Remnant seed ( $ R_{1} $ is an unmeasured sib from family i) | $ \sigma(\overline{z}_{i},y\mid\mathcal{R}_{1})=r\left(\sigma_{A}^{2}/2\right)=\left\{\begin{matrix}\sigma_{A}^{2}/8&half-sibs\\ \sigma_{A}^{2}/4&full-sibs\end{matrix}\right. $
...
> Selection on family deviations (FD) | $ \sigma(z_{ij}-\overline{z}_{i},y\mid\mathcal{R}_{1})=(1-r_{n})(\sigma_{A}^{2}/2)=\left\{\begin{array}{ll}(1-1/n)(3/8)\sigma_{A}^{2}&\text{half-sibs}\\(1-1/n)(\sigma_{A}^{2}/4)&\text{full-sibs}\end{array}\right. $
> Strict within-family selection (FW) | $ \sigma(z_{ij}-\mu_i,y\mid\mathcal{R}_{1})=(1-r)(\sigma_A^2/2)=\left\{\begin{array}{ll}(3/8)\sigma_A^2&\text{half-sibs}\\\sigma_A^2/4&\text{full-sibs}\end{array}\right. $

---

## Evolution_chapter21_013 · THEORY OF EXPECTED SINGLE-CYCLE RESPONSE / Within-family Selection:
...
$$
\sigma(x {1},y\mid\mathcal{R} {1})=(r/2)\sigma {A}^{2}+(r/2)^{2}\sigma {A A}^{2}+(r/2)^{3}\sigma {A A A}^{2}+\cdots=\sum {u=1}\left(\frac{r}{2}\right)^{u}\sigma {A^{u}}^{2}
$$

The formal derivation (which follows from a Smith-Hazel index; Example A6.8) is given in our general treatment of index selection in Volume 3. We refer to the family index using these weights as the Lush index. Note that the weight $ (b 1) $ on family deviations is independent of the family size $ (n) $, while the weight on the family mean $ (b 2) $ depends on $ n $, approaching $ r/t $ for large families. Figure 21.7 plots the ratio of among- to within-family weights $ (b 2/b 1) $ for the Lush index as a function of $ t $ and $ n $. For small between-sib correlations $ (t) $, more weight is placed on family mean, while more weight is placed on within-family deviation when the sib correlation is large.
...
We can rearrange the Lush index as $ I L = z {ij} + B L \overline{z} i $, where substituting Equation 21.54 into Equation 21.50c returns

**[推导 Derivation]**

$$
B {L}=\frac{(r-t)n}{(1-r)[1+(n-1)t]}
$$
...
$$
R_{LI}=\bar{\imath}\sigma_Ah\sqrt{1+\frac{(r-t)^2(n-1)}{(1-t)[1+(n-1)t]}}
$$

As the previous example shows, by making the appropriate choice of index weights, we can always obtain a response at least as large as that expected under mass selection. Note from Equation 21.51 that $ \sigma {A} $ and $ \bar{\imath} {I} $ remain constant under different index weights, implying that the maximal response occurs if we choose the weights that maximize the correlation, $ \rho(I, y) $, between the index and offspring value (Equation 21.5 shows that this is equivalent to maximizing the correlation, $ \rho(I, A) $), between the index and breeding values of an individual. Lush (1947) showed that the resulting optimal index weights are

**[推导 Derivation]**

$$
b {1}=\frac{1-r}{1-t}\quad and\quad b {2}=\frac{1+(n-1)r}{1+(n-1)t}
$$
...
The resulting increase in response over that expected under individual selection ( $ R {m} = \bar{i}\sigma {A}h $; Equation 13.6b) is thus

**[推导 Derivation]**

$$
\frac{R_{LI}}{R_m}=\sqrt{1+\frac{(r-t)^2(n-1)}{(1-t)[1+(n-1)t]}}\geq 1
$$
...
where $t$ is the intraclass correlation of individual phenotypic values among sibs. Note that for large $B$, $\tau$ approaches 1.0. Hence, for schemes that place considerable weight on family means, the index scores for sibs within a family are almost perfectly corrected, and the effective number of independent order statistics approaches the number of families chosen. This is very reasonable, as $I$ approaches $\bar{z} {i}$ for large $B$, which is equivalent to among-family selection, giving the number of independent order statistics as the number of families, $m$.
...
$$
\sigma(z_{i j}-\mu_{i},y\left|\mathcal{R}_{1}\right.)=(1-r)\left(\sigma_{A}^{2}/2\right)=\left\{\begin{array}{l l}\left(3/8\right)\sigma_{A}^{2}&\mathrm{h a l f-s i b s}\\ \sigma_{A}^{2}/4&\mathrm{f u l l-s i b s}\end{array}\right.
$$
...
$$
\begin{align*}\sigma(z_{ij}-\overline{z}_{i},y\mid\mathcal{R}_1=x_{ij})=\sigma(z_{ij},y\mid\mathcal{R}_1)-\sigma(\overline{z}_{i},y\mid\mathcal{R}_1)=(1-r_n)\frac{\sigma_A^2}{2}\end{align*}
$$
...
*[See Table 21.4 at the end of this section.]*

> **Table 21.4** · `21.4` · page 16 · source: `Evolution_chapter21_014`
...
> Selection scheme | Formula
> --- | ---
> Half-sib among-family variance | $ \sigma^{2}(\overline{z}_{HS})=\frac{\sigma_{A}^{2}}{4}+\frac{(3/4)\sigma_{A}^{2}+\sigma_{D}^{2}+\sigma_{E_{s}}^{2}}{n}+\sigma_{E_{c}}^{2}(HS) $
> Full-sib among-family variance | $ \sigma^{2}(\overline{z}_{FS})=\frac{\sigma_{A}^{2}}{2}+\frac{\sigma_{D}^{2}}{4}+\frac{(1/2)\sigma_{A}^{2}+(3/4)\sigma_{D}^{2}+\sigma_{E_{s}}^{2}}{n}+\sigma_{E_{c}(FS)}^{2} $
> Half-sib with nested full-sibs (nested sibs) among-family variance ($ n_f $ females per male, $ n_s $ offspring per female, $ n=n_fn_s $ offspring per male) | $ \sigma^{2}\big(\overline{z}_{H S(F S)}\big)=\frac{\sigma_{A}^{2}}{4}\left(1+\frac{1}{n_{f}}+\frac{2}{n}\right)+\frac{\sigma_{D}^{2}}{4n_{f}}\left(1+\frac{3}{n_{s}}\right)+\frac{\sigma_{E_{s}}^{2}}{n}+\frac{\sigma_{E_{c}(F S)}^{2}}{n_{f}}+\sigma_{E_{c}(H S)}^{2} $
...
## Evolution_chapter21_015 · RESPONSE FOR PARTICULAR DESIGNS

The formal development of the response equations for any particular design follows from the generalized breeder's equation (Equations 21.1 through 21.4), using the appropriate selection-unit variance (*[See Table 21.4 at the end of this section.]*) and selection unit-offspring covariance (*[See Table 21.3 at the end of this section.]*). Results for a number of standard among- and within-family designs are developed below, with family-index selection examined at the end of the chapter.

---

## Evolution_chapter21_016 · RESPONSE FOR PARTICULAR DESIGNS / Overview of Among- and Within-family Response
...
## Evolution_chapter21_017 · RESPONSE FOR PARTICULAR DESIGNS / Among-family Selection:

Here the selection unit is $ \overline{z} $, the mean of a half-, full-, or nested-sib family. The type of sib family, together with the relatives used to produce the next generation, specifies the particular among-family design (*[See Table 21.1 at the end of this section.]*). Tables 21.3 and 21.4 and Equation 21.13a yields the selection response, $ R_{b} $, to a single cycle of among-family selection as
...
*[See Table 21.5 at the end of this section.]* family heritability, with
...
If males (sires or pollen plants) are progeny tested using a nested-sib design, wherein each male is crossed to $ n_{f} $ (unrelated) females (dams or seed plants), each of which has $ n_{s} $ sibs, the among-family variance is given in *[See Table 21.4 at the end of this section.]*, and the response becomes
...
*[See Table 21.5 at the end of this section.]*

> **Table 21.5** · `21.5` · page 20 · source: `Evolution_chapter21_017`
...
## Evolution_chapter21_018 · RESPONSE FOR PARTICULAR DESIGNS / Among-family Selection: Which Scheme Is Best?
...
## Evolution_chapter21_019 · RESPONSE FOR PARTICULAR DESIGNS / Within-family Selection:
...
$$
R_{WF}=\left\{\begin{array}{ll}\frac{(3/8)\sigma_{A}}{\sqrt{1-t(HS)}}h\left(\bar{\imath}_{x_{m}}+\bar{\imath}_{x_{f}}\right)&\text{half-sibs}\\\frac{(1/4)\sigma_{A}}{\sqrt{1-t(FS)}}h\left(\bar{\imath}_{x_{m}}+\bar{\imath}_{x_{f}}\right)&\text{full-sibs}\end{array}\right.
$$
...
## Evolution_chapter21_020 · RESPONSE FOR PARTICULAR DESIGNS / Realized Heritabilities
...
## Evolution_chapter21_021 · RESPONSE FOR PARTICULAR DESIGNS / Accounting for Epistasis
...
The previous expression assumed that $ \Delta_{xy} = 0 $, meaning that terms involving dominance are not included. Using the values of $ \Theta_{xy} $ from *[See Table 21.2 at the end of this section.]*, the parent-offspring covariance is
...
*[See Table 21.2 at the end of this section.]* shows that $ \Theta_{xy} = 1/16 $ and $ 1/8 $ when $ x $ is a half- or full-sib, respectively, of $ \mathcal{R} $. Expressed in terms of Wright's coefficient of relationship, $ r $,
...
## Evolution_chapter21_022 · RESPONSE FOR PARTICULAR DESIGNS / Response with Autotetraploids
...
## Evolution_chapter21_023 · EFFICIENCY OF FAMILY-BASED VS. INDIVIDUAL SELECTION
...
$$
c^{2}<\left\{\begin{aligned}&\frac{1}{16}\left(1-4h^{2}\right)&\text{half-sibs (for family and sib selection; }\gamma=1/4\text{)}\\&\frac{1}{4}\left(1-h^{2}\right)&\text{half-sibs (for parental and }S_{1}\text{ seed selection; }\gamma=1/2\text{)}\\&\frac{1}{4}\left(1-2h^{2}\right)&\text{full-sibs }(\gamma=1/2)\end{aligned}\right.
$$
...
*[See Table 21.6 at the end of this section.]* statistics. The correction for mass selection is generally very small and will be ignored here (see Equation 21.57b). Within-family deviations are negatively correlated within a family, $ (\rho = -1/[n - 1] $ for a family of size $ n $), as they are deviations from a common family mean. As a result, Dempfle (1990) and Hill et al. (1996) found that the resulting selection intensity for within-family deviations is thus slightly larger than the intensity for mass selection $ \bar{\nu}_m $, with
...
*[See Table 21.6 at the end of this section.]*

> **Table 21.6** · `21.6` · page 36 · source: `Evolution_chapter21_025`
...
## Evolution_chapter21_026 · EFFICIENCY OF FAMILY-BASED VS. INDIVIDUAL SELECTION / Within-family Selection Has Additional Long-term Advantages
...
## Evolution_chapter21_027 · RESPONSE WHEN FAMILIES ARE REPLICATED OVER ENVIRONMENTS
...
where $ N = n_{f}n_{e}n_{s} $ is the total number of half-sibs per male (Robertson et al. 1955; Webel and Lonnquist 1967; da Silva and Lonnquist 1968). Assuming no epistasis or genotype by environment interaction ($ G \times E $), we can express this among-family variance as
...
## Evolution_chapter21_029 · RESPONSE WHEN FAMILIES ARE REPLICATED OVER ENVIRONMENTS / Ear-to-Row Selection
...
## Evolution_chapter21_030 · RESPONSE WHEN FAMILIES ARE REPLICATED OVER ENVIRONMENTS / Modified Ear-to-Row Selection
...
## Evolution_chapter21_031 · SELECTION ON A FAMILY INDEX
...
## Evolution_chapter21_032 · SELECTION ON A FAMILY INDEX / Response to Selection on a Family Index
...
where $ \sigma(I,y|\mathcal{R}_{1}) $ is the covariance between the index value, $ I $, of a parent and the phenotype of its offspring, $ y $. The variances and covariances required for Equation 21.51 are obtained as follows. Using the covariances summarized in *[See Table 21.3 at the end of this section.]*,
...
Likewise, if we recall that $ \sigma^{2}(x+y)=\sigma_{x}^{2}+\sigma_{y}^{2}+2\sigma_{x,y} $, the variances summarized in *[See Table 21.4 at the end of this section.]* yield
...
## Evolution_chapter21_033 · SELECTION ON A FAMILY INDEX / Lush's Optimal Index
...
## Evolution_chapter21_034 · SELECTION ON A FAMILY INDEX / Correcting the Selection Intensity for Correlated Variables
