...
## chapter19_003 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / BLUP Selection
...
## chapter19_006 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / REML Estimation of Unknown Variance Components
...
## chapter19_007 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / REML Often Returns Variance Estimates Unbiased by Selection
...
## chapter19_009 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / The Basic Animal Model
...
## chapter19_010 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Response Is Measured by Change in Mean Breeding Values
...
## chapter19_011 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Fixed Effects Alter Heritabilities
...
## chapter19_012 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Model Validation
...
> **Example 19.2** · ref: `19.2` · source: `chapter19_012.json` · blocks 1–1
>
...
## chapter19_013 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Separating Genetic and Environmental Trends
...
In a least-squares analysis, any underlying environmental trend is assumed to be removed by contrasting selected and control populations (or contrasting populations selected in opposite directions). The rationale is that the $ k^{th} $ individual from population j in generation t can be described as

> **Formula (19.9)** · `19.9` · source: `chapter19_block_063` · Separating Genetic and Environmental Trends
>
> $$ y_{j,t k}=\mu+b_{j,t}+a_{j,t k}+e_{j,t k} $$
...
> **Formula (19.11)** · `19.11` · source: `chapter19_block_066` · Separating Genetic and Environmental Trends
>
> $$ y_{t k}=\mu+b_{t}+a_{t k}+e_{t k} $$


For $T$ generations, there are $T$ estimable fixed effects ($\mu$ and all but one $b_t$). Typically, one either constrains the $b_t$ to sum to zero or arbitrarily sets one of the $b_t$ equal to zero. We do the latter with $b_1$, so the vector of fixed effects becomes $$ \boldsymbol{\beta}=(\mu,b_{2},\cdots,b_{T})^{T} $$ and the corresponding design matrix, X, has zeros or ones in columns 2 though T, corresponding to the generation in which the individual was scored $$ \mathbf{X}=\begin{pmatrix}{{{1}}}&{{{0}}}&{{{\cdots}}}&{{{0}}} \\{{{1}}}&{{{0}}}&{{{\cdots}}}&{{{0}}} \\{{{\vdots}}}&{{{\vdots}}} \\{{{1}}}&{{{1}}}&{{{\cdots}}}&{{{0}}} \\{{{1}}}&{{{1}}}&{{{\cdots}}}&{{{0}}} \\{{{\vdots}}}&{{{\vdots}}} \\{{{1}}}&{{{0}}}&{{{\cdots}}}&{{{1}}} \\{{{1}}}&{{{0}}}&{{{\cdots}}}&{{{1}}}\end{pmatrix} $$

To remind the reader how X is obtained, recall that Xβ returns a vector that adjusts each observation for the fixed effects. From the rules of matrix multiplication, the adjustment for observation i is the inner product, $ x_i^T\beta $, where the row vector $ x_i^T $ corresponds to the ith row of X. Likewise, the jth column of X corresponds to the vector of weights (over all observations) for the jth fixed effect. Here, the first column is all ones, as all observations contain the mean, μ. Entries in column 2 (corresponding to b₂) are zero, unless an observation is from generation 2 (and hence has a mean of μ + b₂), in which case the entry is 1, and so forth. Hence, the first two displayed rows of the above X correspond to observations from generation 1, which have means of μ, the second pair of displayed rows to generation 2 (with means of μ + b₂), and the last pair to generation T (with means of μ + bT).

**[命题 Proposition]**

An alternative approach is to treat the $ b_i $ as random (as opposed to fixed) effects, drawn independently (i.e., uncorrelated across generations) from a normal distribution with a mean of zero and an unknown variance, $ \sigma_b^2 $. The assumption of no environmental correlation between adjacent generations can be questionable (at best), and treating the $ b_i $ as fixed, rather than random, removes these concerns (although at a cost of absorbing more degrees of freedom than with a random-effects analysis).

**[示例 Example]**

> **Example 19.3** · ref: `19.3` · source: `chapter19_013.json` · blocks 8–8
>
> Example 19.3. To examine the potential bias from $ G \times E $ and drift in the control population, Blair and Pollak (1984) examined a seven-generation selection experiment on 14-month greasy fleece weight in sheep. The model they assumed was that the mth individual in generation t, with fixed sex effect (male/female), $ v_i $, fixed age of dam effect (mature/immature), $ d_j $, rearing rank effect (single/twin), $ r_k $, and year effect, $ b_t $, had a phenotypic value of $$ y_{i j k t m}=v_{i}+d_{j}+r_{k}+b_{t}+a_{t m}+e_{t m} $$
> 
...
> 
> (ii) $ \widehat{y}_{s,t} - \widehat{b}_{c,t} $, the (fixed-effects-adjusted) phenotypic mean of the selected population minus the common environmental effect, as estimated from the control population.
> 
...
## chapter19_014 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Validation That a Trend Is Indeed Genetic
...
## chapter19_015 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Replicate Lines
...
## chapter19_016 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Estimating the Additive Variance at Generation t
...
> **Example 19.4** · ref: `19.4` · source: `chapter19_017.json` · blocks 5–5
>
> Example 19.4. Suppose individuals 1 through 4 are from the base population (and assumed unrelated), while individuals 5 and 6 are the offspring from 1 and 2, and 3 and 4, respectively. The offspring breeding values can be written as $$ A_{5}=\frac{1}{2}A_{1}+\frac{1}{2}A_{2}+s_{5}\qquad\mathrm{a n d}\qquad A_{6}=\frac{1}{2}A_{3}+\frac{1}{2}A_{4}+s_{6} $$
> 
> If individual 7 is the offspring of parents 5 and 6, its breeding value is $$ \begin{aligned}A_{7}&=\frac{1}{2}A_{5}+\frac{1}{2}A_{6}+s_{7}=\frac{1}{2}\left(\frac{1}{2}A_{1}+\frac{1}{2}A_{2}+s_{5}\right)+\frac{1}{2}\left(\frac{1}{2}A_{3}+\frac{1}{2}A_{4}+s_{6}\right)+s_{7}\\&=\frac{1}{4}\left(A_{1}+A_{2}+A_{3}+A_{4}\right)+\frac{1}{2}\left(s_{5}+s_{6}\right)+s_{7}\end{aligned} $$
> 
...
> 
> This linear relationship, highlighted in the last example, between the $ n \times 1 $ vector, a, of all breeding values and the $ m \times 1 $ vector, $ a_b $ (breeding values for the m base population founders), and the $ (n - m) \times 1 $ vector, s (Mendelian segregation values), can be formally expressed as follows. Defining $ \mathbf{w}^T = (\mathbf{a}_b^T, \mathbf{s}^T) $, we can write
> 
> > **Formula (19.17a)** · `19.17a` · source: `chapter19_block_095` · THE RELATIONSHIP MATRIX, A, ACCOUNTS FOR DRIFT AND DISEQUILIBRIUM
> >
> > $$ \mathbf{a}=\mathbf{T}\mathbf{w}=\mathbf{T}\begin{pmatrix}\mathbf{a}_{b}\\ \mathbf{s}\end{pmatrix} $$
> 
> 
> where the $ n \times n $ matrix, T, describes the flow of genes from ancestors to relatives. Indexing the elements in a so that ancestors precede their relatives, T is a lower-triangular matrix (all above-diagonal elements are zero) with diagonal values of one. Further, the upper-left $ m \times m $ submatrix of T, corresponding to the base population (and thus, no known ancestors), has all below-diagonal elements with value zero. For descendants of the base population founders, the off-diagonal element of T associated with (descendant) individuals i, j is
> 
> > **Formula (19.17b)** · `19.17b` · source: `chapter19_block_095` · THE RELATIONSHIP MATRIX, A, ACCOUNTS FOR DRIFT AND DISEQUILIBRIUM
> >
> > $$ T_{i,j}=\frac{1}{2}\left(T_{f,j}+T_{m,j}\right)\quad for\quad j<i $$
> 
> 
> where f and m correspond to the index for the father and mother of i. Equations 19.17a and 19.17b formalize the decomposition (highlighted in Example 19.4) of a current breeding value into base population and segregation contributions. From Equation 19.17a
> 
> > **Formula (19.18a)** · `19.18a` · source: `chapter19_block_095` · THE RELATIONSHIP MATRIX, A, ACCOUNTS FOR DRIFT AND DISEQUILIBRIUM
> >
> > $$ \mathbf{V a r}(\mathbf{a})=\mathbf{V a r}(\mathbf{T}\mathbf{w})=\mathbf{T}\mathbf{V a r}(\mathbf{w})\mathbf{T}^{T} $$
> 
> 
> Because we assume that the base population consists of unrelated individuals $ (\mathbf{A}_b = \mathbf{I}) $ in gametic-phase equilibrium (so that $ \sigma_A^2 = \sigma_a^2 $), $ \mathbf{Var}(\mathbf{a}_b) = \sigma_a^2 \mathbf{I} $, while $ \mathbf{Var}(\mathbf{s}) = (\sigma_a^2 / 2) \mathbf{F} $. Under multivariate normality, $ a_b $ and $ s $ are independent, and hence uncorrelated, yielding
> 
> > **Formula (19.18b)** · `19.18b` · source: `chapter19_block_095` · THE RELATIONSHIP MATRIX, A, ACCOUNTS FOR DRIFT AND DISEQUILIBRIUM
> >
> > $$ \mathbf{V a r}(\mathbf{w})=\mathbf{V a r}\begin{pmatrix}\mathbf{a}_{b}\\ \mathbf{s}\end{pmatrix}=\sigma_{a}^{2}\begin{pmatrix}\mathbf{I}&\mathbf{0}\\ \mathbf{0}&\mathbf{F}/2\end{pmatrix}=\sigma_{a}^{2}\boldsymbol{A} $$
> 
> 
> where $ \Lambda $ is a diagonal matrix with elements of one when both parents are unknown (base population) and $ 0.5(1 - \overline{f}) $ when both parents are known. If, due to missing data, only a single parent is known, the diagonal element becomes $ (3 - f)/4 $, where f is the inbreeding in the known parent (Kennedy et al. 1988). Putting Equations 19.18a and 19.18b together yields $$ \mathbf{V a r}(\mathbf{a})=\sigma_{A}^{2}\mathbf{A}=\mathbf{T}\mathbf{V a r}(\mathbf{w})\mathbf{T}^{T}=\sigma_{a}^{2}\mathbf{T}\boldsymbol{\Lambda}\mathbf{T}^{T} $$
> 
> Assuming $ \sigma_{A}^{2} = \sigma_{a}^{2} $ (no disequilibrium in the base population), then
> 
> > **Formula (19.18c)** · `19.18c` · source: `chapter19_block_096` · THE RELATIONSHIP MATRIX, A, ACCOUNTS FOR DRIFT AND DISEQUILIBRIUM
> >
> > $$ \mathbf{A}=\mathbf{T}\boldsymbol{\Lambda}\mathbf{T}^{T} $$
> 
> 
> as obtained by Henderson (1976) and Thompson (1977). As an aside, this expression, coupled with the simple form for T, is what allowed Henderson (1976) to obtain a very rapid way of computing $ A^{-1} $. The critical feature of Equation 19.18c is that $ \Lambda $, which describes the Mendelian sampling residuals, is independent of selection and assortative mating (under the infinitesimal model), and also accounts for the reduction in additive variance from genetic drift.
> 
...
> 
> When the genic variance, $ \sigma_{a}^{2} $, itself is changing due to selection, knowledge of A is no longer sufficient to account for all changes in $ \sigma_{A}^{2} $. Beniwal et al. (1992a) suggested that this complication might be (somewhat) accommodated by a modification of the additive-genetic covariance matrix. Consider the simple case of two time blocks with different (unknown) genic variances: the breeding values, $ a_1 $, in the first block have a base genic variance, $ \sigma_a^2(1) $, while the vector of breeding values, $ a_2 $, from the second block start with a genic variance of $ \sigma_a^2(2) $. First we decompose the diagonal matrix $ \Lambda $ into two blocks $$ \boldsymbol{A}=\left(\begin{array}{cc}\boldsymbol{A}_{1}&\mathbf{0}\\\mathbf{0}&\boldsymbol{A}_{2}\end{array}\right) $$ where $ A_{i} $ represents block i. The covariance matrix for the breeding values can be written as
> 
> > **Formula (19.19)** · `19.19` · source: `chapter19_block_098` · THE RELATIONSHIP MATRIX, A, ACCOUNTS FOR DRIFT AND DISEQUILIBRIUM
> >
> > $$ \mathbf{V a r}(\mathbf{a})=\mathbf{V a r}\begin{pmatrix}\mathbf{a}_{1}\\ \mathbf{a}_{2}\end{pmatrix}=\sigma_{a}^{2}(1)\mathbf{T}\begin{pmatrix}\boldsymbol{A}_{1}&\mathbf{0}\\ \mathbf{0}&\mathbf{0}\end{pmatrix}\mathbf{T}^{T}+\sigma_{a}^{2}(2)\mathbf{T}\begin{pmatrix}\mathbf{0}&\mathbf{0}\\ \mathbf{0}&\boldsymbol{A}_{2}\end{pmatrix}\mathbf{T}^{T} $$
> 
> 
> The variances $ \sigma_a^2(1) $ and $ \sigma_a^2(2) $ can be estimated separately using REML, and they correspond to the genic variances at the start of each block, which may be reduced by drift within the block. Heath et al. (1995) generalized this approach to allow the genic variance to change in each generation.
...
> **Example 19.5** · ref: `19.5` · source: `chapter19_018.json` · blocks 2–2
>
...
## chapter19_019 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Models with Additional Random Effects
...
> **Example 19.6** · ref: `19.6` · source: `chapter19_019.json` · blocks 5–5
>
...
## chapter19_020 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Common Family and Maternal Effects
...
> **Example 19.7** · ref: `19.7` · source: `chapter19_020.json` · blocks 1–1
>
...
## chapter19_021 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Treating Certain Breeding Values as Fixed Effects
...
## chapter19_022 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Dominance
...
## chapter19_024 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Introduction to Bayesian Statistics
...
> **Example 19.8** · ref: `19.8` · source: `chapter19_024.json` · blocks 7–7
>
...
> **Example 19.9** · ref: `19.9` · source: `chapter19_025.json` · blocks 3–3
>
...
## chapter19_026 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Computing Posteriors and Marginals: MCMC and the Gibbs Sampler
...
## chapter19_027 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Bayesian Analysis of the Animal Model
...
## chapter19_028 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / Application: Estimating Selection Response in Pig Litter-size Components
...
## chapter19_029 · Analysis of Short-term Selection Experiments: 2. Mixed-model and Bayesian Approaches: Introduction / LS, MM, OR BAYES?
