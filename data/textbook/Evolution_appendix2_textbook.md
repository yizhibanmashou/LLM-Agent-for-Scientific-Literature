...
as $ p(\mathbf{x} \mid \boldsymbol{\Theta}) = \ell(\boldsymbol{\Theta} \mid \mathbf{x}) $ is simply the likelihood function (LW Appendix 4) and $ 1/p(\mathbf{x}) $ is a constant (with respect to $ \boldsymbol{\Theta} $). Consequently, the posterior distribution is often written as
...
$$
p_{i}(\theta)=p(\theta)/\pi_{i}\qquad for\quad i=0,1
$$
...
which follows since $ f(\mathbf{x}, \boldsymbol{\Theta}_{1}) $ is constant with respect to $ \boldsymbol{\Theta}_{2} $. With the prior given by Equation A2.19, we can express the resulting posterior distribution as
...
$$
\begin{align*}p(x\mid\alpha,\beta)={\beta^\alpha\over\Gamma(\alpha)}x^{\alpha-1}e^{-\beta x}\quad{\rm for}\alpha,\beta,x>0\end{align*}
$$
...
$$
\Gamma(\alpha)=\int_{0}^{\infty}y^{\alpha-1}e^{-y}d y\qquad for\alpha>0
$$
