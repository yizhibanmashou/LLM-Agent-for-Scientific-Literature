...
Adaptation depends on how the various evolutionary processes shape variation in populations.
...
\section{THE POPULATION GENETICS OF SELECTION AND DRIFT}
...
\begin{equation}
u(p_{0})\simeq p_{0}\qquadif\qquad2N_{e}|s|\ll1
\tag{26.2c}
\label{eq:26_2c}
\end{equation}
...
\section{Fixation Probabilities for Alleles at a QTL}
...
Note that if the product of initial allele frequency and its standardized effect,  $ p_0 | a | / \sigma_z $, is sufficiently small, drift will dominate even if selection ( $ \bar{\tau} $) on the character is strong. With small values of  $ N_e \bar{\tau} $, only alleles of large effect or at moderate to high frequency are likely to be fixed by selection. As  $ N_e \bar{\tau} $ increases, favored alleles with smaller effects, or at lower frequencies, are increasingly more likely to become fixed. As a technical aside, the

956

careful reader might recall from Chapter 14 that sampling causes fluctuations in  $ \bar{\imath} $ in a finite population. This additional complication need not overly concern us, as Hill (1969a, 1985) and Kojima (1961) showed that the error introduced by assuming a constant  $ \bar{\imath} $ is small.

\section{Increased Recombination Rates Following Selection}
...
Example 26.2. Consider directional truncation selection on a normally distributed character in which the uppermost 20% of the population (p = 0.2) is saved. From Example 16.3, this yields a selection intensity of  $ \bar{\tau} = 1.40 $ and a reduction in variance of  $ \kappa = 0.781 $. If we assume initial (before selection) values of  $ h_0^2 = 0.5 $ and  $ \sigma_z^2(0) = 100 $, Example 16.2 yields (under the infinitesimal model) equilibrium values of  $ \widehat{d} = -12.54 $ and  $ \widehat{h}^2 = 0.428 $. Hence

\begin{equation}
G=1-\kappa\widehat{h}^{2}=1-0.781\cdot0.428=0.665
\end{equation}
...
\begin{equation}
\widehat{t}_{FS}=\frac{h_{0}^{2}/2+\widehat{d}/\sigma_{z(0)}^{2}}{1+\widehat{d}/\sigma_{z(0)}^{2}}=\frac{(0.5/2)-(12.54/100)}{1-(12.54/100)}=0.142
\end{equation}

Hence,  $ \sigma_w^2 = \bar{i}^2 \cdot \widehat{t}_{FS} = 1.4^2 \cdot 0.142 = 0.279 $. Assuming  $ \rho = -1/(N - 1) \simeq 0 $, Equation 26.9b yields

\begin{equation}
\widetilde{Q}_{*}=\frac{2}{2-G}=\frac{2}{2-0.665}=1.498
\end{equation}

Equation 26.6c yields an equilibrium effective population size of

\begin{equation}
N_{e}=\frac{N}{1+\widetilde{Q}_{*}^{2}\sigma_{w}^{2}}=\frac{N}{1+1.498^{2}\cdot0.279}=0.615N
\end{equation}
...
\section{DRIFT AND LONG-TERM SELECTION RESPONSE}
...
\section{Robertson's Theory of Selection Limits}
...
expressions for  $ u_i(p_0) $ to describe the limit, and for both  $ E(p_t) $ and  $ E[p_t(1-p_t)] $ to describe the dynamics. The most complete description, by Robertson (1960a), is for additive alleles, where  $ E[\Delta_i(t)] = 2a[E(p_t) - p_0] $. Recalling Equation 7.28a yields
...
\section{TESTS OF ROBERTSON'S THEORY OF SELECTION LIMITS}
...
\section{Weber's Selection Experiment on Drosophila Flight Speed}
...
\section{THE EFFECTS OF LINKAGE ON THE SELECTION LIMIT}
...
\section{OPTIMAL SELECTION INTENSITIES FOR MAXIMIZING LONG-TERM RESPONSE}
...
\section{EFFECTS OF POPULATION STRUCTURE ON LONG-TERM RESPONSE}
...
\section{Founder Effects and Population Bottlenecks}
...
\section{ASYMPTOTIC RESPONSE DUE TO MUTATIONAL INPUT}
...
\section{Results for the Infinitesimal Model}
...
\begin{equation}
1-e^{-t/(2N_{e})}=1-e^{-60/120}\simeq0.40
\end{equation}

of its expected asymptotic rate, yielding  $ 0.4 \cdot 0.32 = 0.13 $ as the expected response due to new mutants at  $ t = 60 $. Assuming the phenotypic variance remains relatively constant, with  $ \sigma_z^2 \simeq 5 $, the expected contribution at generation 60 from initial variation is

\begin{equation}
\bar{\imath}\frac{\sigma_{A,0}^{2}(t)}{\sigma_{z}}=\bar{\imath}\frac{h^{2}(0)\cdot\sigma_{z}^{2}\cdot e^{-t/(2N_{e})}}{\sigma_{z}}=1.4\cdot\frac{0.2\cdot5\cdot e^{-60/120}}{\sqrt{5}}\simeq0.38
\end{equation}
...
\section{Expected Asymptotic Response Under More General Conditions}
...
as obtained by Hill (1982b). The last equality assumes the distribution of mutational values and rates to be the same at each locus. When all mutations are additive (k = 0) and symmetric (E[\alpha] = 0), this reduces to our previous definition of  $ \sigma_m^2 $. More generally, with complete additivity, but removing the assumption that E[\alpha] = 0, we have
...
\section{Additional Models of Mutational Effects}
...
\section{Optimizing the Asymptotic Selection Response}
...
\begin{equation}
\sqrt{\frac{N_{e(w)}}{N_{e(m)}}}\geq\sqrt{2}
\tag{29.39a}
\label{eq:29_39a}
\end{equation}
...
\begin{equation}
\widetilde{\rho}(m)=\frac{\widetilde{\sigma}_{A}^{2}}{\sqrt{\widetilde{\sigma}_{A}^{2}(\widetilde{\sigma}_{A}^{2}+\sigma_{E}^{2})}}=\frac{2N_{e(m)}\sigma_{m}^{2}}{\sqrt{2N_{e(m)}\sigma_{m}^{2}(2N_{e(m)}\sigma_{m}^{2}+\sigma_{E}^{2})}}
\tag{29.39c}
\label{eq:29_39c}
\end{equation}
...
\begin{equation}
\rho(w)=\rho(z-\overline{z}_{f},A)\simeq\frac{\sigma(z-\overline{z}_{f},A)}{\sqrt{\sigma^{2}(A)\sigma^{2}(z-\overline{z}_{f})}}\simeq\frac{\sigma_{A}^{2}/2}{\sqrt{\sigma_{A}^{2}(\sigma_{G w}^{2}+\sigma_{E_{s}}^{2})}}
\tag{29.39d}
\label{eq:29_39d}
\end{equation}
...
\begin{equation}
\widetilde{\rho}(w)=\frac{\widetilde{\sigma}_{A}^{2}/2}{\sqrt{\widetilde{\sigma}_{A}^{2}(\widetilde{\sigma}_{A}^{2}/2+\sigma_{E}^{2})}}=\frac{N_{e(w)}\sigma_{m}^{2}}{\sqrt{2N_{e(w)}\sigma_{m}^{2}(N_{e(w)}\sigma_{m}^{2}+\sigma_{E}^{2})}}
\tag{29.39e}
\label{eq:29_39e}
\end{equation}
...
\begin{equation}
\frac{\widetilde{R}_{w}}{\widetilde{R}_{m}}=\left[\sqrt{\frac{N_{e(w)}}{N_{e(m)}}}\right]\left[\frac{\widetilde{\rho}_{(w)}}{\widetilde{\rho}_{(m)}}\right]\geq\sqrt{2}\frac{1}{\sqrt{2}}=1
\end{equation}

Thus,

and hence  $ \widetilde{R}_{w} \geq \widetilde{R}_{m} $. That is, for the same selection intensity, the asymptotic response is greater under within-family selection than under mass selection.
