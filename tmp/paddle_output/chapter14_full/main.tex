...
\section{Selection Intensities and Differentials Under Truncation Selection}
...
and we use this assumption throughout (unless stated otherwise). With an initial mean of  $ \mu $ and variance of  $ \sigma^2 $, and  $ p = \Pr(z \geq T) $ being the fraction saved, this conditional mean is given by LW Equation 2.14, which yields the expected selection differential as
...
\begin{equation}
\bar{\imath}=\frac{\varphi(1.645)}{0.05}=\frac{0.103}{0.05}\simeq2.06
\end{equation}
...
\begin{equation}
\bar{\imath}\simeq0.8+0.41\ln\left(\frac{1}{0.05}-1\right)\simeq2.01
\end{equation}

Finally, if we selected for the lowermost 5%,  $ \bar{\imath} = -2.06 $.

\section{Correcting the Selection Intensity for Finite Sample Sizes}
...
function of the phenotypic distribution, and  $ y_p $ be the truncation point (i.e.,  $ \Pr(y \geq y_p) = p $), Burrows's approximation is
...
\begin{equation}
\begin{align*}\mu_{y_p}=E\left[y\left|y\geq y_p\right.\right]=\frac{1}{p}\int_{y_p}^\infty x\varphi(x) dx\end{align*}
\end{equation}
...
Example 14.2. Consider the expected selection intensity on males when the upper 5% are used to form the next generation and phenotypes are normally distributed. If the sampled number is large,  $ \bar{\imath} \simeq 2.06 $ (Example 14.1). Suppose, however, that just 20 males are scored (phenotyped), with only the largest allowed to reproduce, in order to yield p = 0.05. The expected value for this individual is the expected value of the largest order statistic for a sample of size 20. For the unit

512

normal, this is  $ \simeq $1.87 (Harter 1961), and hence  $ E[\bar{\nu}_{(1,20)}] \simeq 1.87 $. There is considerable spread about this expected value, as the standard deviation of this order statistic is 0.525 (Sarhan and Greenberg 1962). How well do the approximations of  $ E[\bar{\nu}_{(1,20)}] $ perform? Burrows's approximation (Equation 14.4b) yields

\begin{equation}
E\left[\bar{\imath}_{(1,20)}\right]\simeq2.06-\frac{(20-1)}{2\left(20+1\right)2.06}=2.06-0.22=1.84
\end{equation}

Bulmer's approximation (Equation 14.4c) uses

\begin{equation}
\widetilde{p}=\frac{1+1/2}{20+1/40}\simeq0.075
\end{equation}

which returns  $ x_{[1-0.075]} \simeq 1.44 $. Because  $ \varphi(1.44) = 0.1415 $,  $ E[\bar{\imath}_{(1,20)}] \simeq 0.1415/0.075 \simeq 1.89 $.

\section{The Threshold/Liability Model}
...
\begin{equation}
R(1)=\mu_{2}-\mu_{1}=-x_{[1-0.340]}-(-x_{[1-0.224]})=-0.41-(-0.76)=0.35
\end{equation}

Likewise, the total response was

\begin{equation}
\mu_{6}-\mu_{1}=0.55-(-0.76)=1.31
\end{equation}

Selection differentials were calculated from q and p using Equation 14.9. For example, for generation 2,

\begin{equation}
S_{2}=\frac{\varphi(\mu_{2})}{q_{2}}\frac{p_{2}-q_{2}}{1-q_{2}}=\frac{\varphi(-0.41)}{0.34}\frac{(0.80-0.34)}{1-0.34}=0.75
\end{equation}
...
\begin{equation}
\widehat{h^{2}}=\frac{\sum R_{i}}{\sum_{i}S_{i}}=\frac{1.31}{3.91}=0.33
\end{equation}

giving an estimated heritability of the underlying liability for macroptery of around 30%.

\section{Direct Selection on the Threshold, T}
...
\section{The Logistic Regression Model for Binary Traits}
...
\section{BLUP Selection With Binary Data: Generalized Linear Mixed Models}
...
namely, the log of the odds ratio (probability of the trait being present divided by probability that the trait is absent). If  $ \ell(z) = p $, then  $ \logit(p) = z $, so that a logit-transformed p value recovers the liability value,
...
\section{RESPONSE IN DISCRETE TRAITS: POISSON-DISTRIBUTED CHARACTERS}
