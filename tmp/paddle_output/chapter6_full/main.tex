...
\title{Theorems of Natural Selection: Results of Price, Fisher, and Robertson}
...
\section{PRICE'S GENERAL THEOREM OF SELECTION}
...
Equation 6.7c relates the selection response to the covariance between the fitness,  $ w_i $, of an ancestor and the mean value,  $ \overline{z}_i $, of its descendants. The second term,  $ E(\overline{\delta}_i) $, is often thought
...
\begin{equation}
\begin{aligned}E(w_{i}\overline{\delta}_{i})&=E\{w_{i}\left[(b-1)(z_{i}-\mu)+e_{i}\right]\}\\&=(b-1)[E(w_{i}z_{i})-\mu E(w_{i})]+E(w_{i}e_{i})\end{aligned}
\end{equation}

Recalling that  $  E(w_i) = 1  $ gives

\begin{equation}
E(w_{i}\overline{\delta}_{i})=(b-1)\left[E(w_{i}z_{i})-\mu\right]+E(w_{i}e_{i})
\end{equation}

From the definition of a covariance

\begin{equation}
E(w_{i}z_{i})=\sigma(w_{i},z_{i})+E(w_{i})E(z_{i})=S+1\cdot\mu
\end{equation}

\begin{equation}
E(w_{i}e_{i})=\sigma(w_{i},e_{i})+E(w_{i})E(e_{i})=\sigma(w_{i},e_{i})+1\cdot0
\end{equation}
...
\begin{equation}
\Delta p=R_{z}=\sigma(w_{i},z_{i})+E(w_{i}\cdot0)=\sigma(w_{i},z_{i})
\end{equation}

Under random mating, the fitness  $ W_1 $ of an  $ A $ allele is simply its marginal fitness (Equation 5.7b),  $ W_1 = pW_{AA} + (1 - p)W_{Aa} $. Similarly,  $ W_2 = pW_{Aa} + (1 - p)W_{aa} $ and  $ E(W_i) = \overline{W} = pW_1 + (1 - p)W_2 $. Recalling that  $ w_i = W_i/\overline{W} $ and the definition of a covariance, we have
...
\begin{equation}
E(W_{i}z_{i})=\sum_{i=1}^{2}W_{i}z_{i}freq(category i)=\left[W_{1}\cdot1\cdot p\right]+\left[W_{2}\cdot0\cdot(1-p)\right]=p\cdot W_{1}
\end{equation}

Using this result along with  $ E(W_i) = \overline{W} $ and  $ E(z_i) = p $

\begin{equation}
\Delta p=\frac{1}{\overline{W}}\left(p W_{1}-\overline{W}p\right)=p\frac{\left(W_{1}-\overline{W}\right)}{\overline{W}}
\end{equation}
...
\section{The Breeder's Equation,  $ R = h^{2} S $}
...
The trait mean before selection is  $ \mu(p) = 1 - p^2 $, with a mean fitness of  $ \overline{W}(p) = 1 + p^2 $. The new frequency,  $ p' $, of allele  $ q $ after selection is half the frequency of  $ Qq $ plus the frequency of  $ qq $ (both after selection)

\begin{equation}
p^{\prime}=\frac{(1/2)2p(1-p)+2p^{2}}{1+p^{2}}=\frac{p(1+p)}{1+p^{2}}
\tag{6.16a}
\label{eq:6_16a}
\end{equation}
...
\begin{equation}
R_{z}=\mu(p^{\prime})-\mu(p)=p^{2}-(p^{\prime})^{2}=-2p^{3}\left(\frac{1-p(1+p^{2})/2}{(1+p^{2})^{2}}\right)
\tag{6.16b}
\label{eq:6_16b}
\end{equation}
...
\begin{equation}
\sigma_{A}^{2}=2p(1-p)a^{2}[1+k(2p-1)]^{2}=2p(1-p)(1/4)(2p)^{2}=2p^{3}(1-p)
\end{equation}

\begin{equation}
\sigma_{D}^{2}=[2p(1-p)a k]^{2}=p^{2}(1-p)^{2}
\end{equation}

Because  $ \sigma_{E}^{2} $ is assumed to be zero, the heritability becomes

\begin{equation}
h^{2}=\frac{\sigma_{A}^{2}}{\sigma_{A}^{2}+\sigma_{D}^{2}}=\frac{2p^{3}(1-p)}{2p^{3}(1-p)+p^{2}(1-p)^{2}}=\frac{2p}{1+p}
\tag{6.16c}
\label{eq:6_16c}
\end{equation}
...
\begin{equation}
\mu^{*}=1\cdot\frac{1}{\overline{W}}\left(1-p^{2}\right)+0\cdot\frac{2}{\overline{W}}p^{2}=\frac{1-p^{2}}{\overline{W}}=\frac{1-p^{2}}{1+p^{2}}
\tag{6.16d}
\label{eq:6_16d}
\end{equation}
...
\begin{equation}
R_{z}=h^{2}S=\left(\frac{2p}{1+p}\right)\left(-p^{2}\frac{1-p^{2}}{1+p^{2}}\right)=-\frac{2p^{3}(1-p)}{1+p^{2}}
\tag{6.16f}
\label{eq:6_16f}
\end{equation}
...
\section{FISHER'S FUNDAMENTAL THEOREM OF NATURAL SELECTION}
...
\section{The Classical Interpretation of Fisher's Fundamental Theorem,  $ R_w = \sigma_A^2(w) $}
...
\begin{equation}
\Delta\overline{W}=\frac{d\overline{W}}{d p}\Delta p+\frac{1}{2}\frac{d^{2}\overline{W}}{d p^{2}}\left(\Delta p\right)^{2}
\end{equation}
...
\begin{equation}
\frac{d^{2}\overline{W}}{d p^{2}}=2\left(W_{A A}-2W_{A a}+W_{a a}\right)
\end{equation}

and recalling Equation 6.18f, the residual term becomes

\begin{equation}
\frac{1}{2}\frac{d^{2}\overline{W}}{d p^{2}}\Delta p^{2}=\left(W_{A A}-2W_{A a}+W_{a a}\right)\left(\frac{p(1-p)}{\overline{W}}\alpha\right)^{2}
\end{equation}
...
\section{What Did Fisher Really Mean?}
...
\section{IMPLICATIONS OF FISHER'S THEOREM FOR TRAIT VARIATION}
...
\begin{equation}
W_{11}=1-s\quad W_{12}=1\quad W_{22}=1-s
\end{equation}

Letting  $ p = \text{freq}(A_1) $, from Equation 6.18e and the definition of the average effect of a substitution under random mating, we have

\begin{equation}
\alpha=\alpha_{A}-\alpha_{a}=p\big[\big(1-s\big)-1\big]+\big(1-p\big)\big[1-\big(1-s\big)\big]=s(1-2p)
\end{equation}

giving the additive genetic variance in fitness as

\begin{equation}
\sigma_{A}^{2}(W)=2p(1-p)\alpha^{2}=2p(1-p)s^{2}(1-2p)^{2}
\end{equation}
...
\section{Traits Correlated With Fitness Have Lower Heritabilities}
...
\section{Traits Correlated With Fitness Have Higher Levels of Both Additive and Residual Variance}
...
\section{Nonadditive Genetic Variance for Traits Under Selection}
...
\section{1968 Version:  $ R_{z} = \sigma_{A}(w, z) $}
...
Likewise, under the fundamental theorem (Equation 6.17c),  $ \Delta A_w = \sigma^2(A_w) $. Substituting
...
\section{1966 Version:  $ R_{z} = \sigma(w, A_{z}) $}
...
\section{Accuracy of the Secondary Theorem}
...
the average effect,  $ \alpha_{j} $, of an allele on character value and the average excess,  $ s_{i} $, of that allele on relative fitness. Recall that the definition of a covariance is  $ \sigma(x,y) = E(x\,y) - E(x)\,E(y) $, and note that the first term above is simply  $ 2E(\alpha_{j}s_{j}) $. Because (by definition),  $ E(\alpha_{j}) = 0 $, the first sum is thus the covariance between  $ \alpha_{j} $ and  $ s_{j} $, in other words, the additive genetic covariance between relative fitness and the focal trait. Summing over all loci, we can express Equation 5.22 as
...
\section{Connecting Robertson’s Results With Those of Price, Fisher, and Lush}
...
\section{THE BREEDER'S EQUATION FRAMED WITHIN THE PRICE EQUATION}
...
\section{Partial Covariance and the Spurious Response to Selection}
...
\begin{equation}
x=\mu_{x}+\beta_{x|z}z+e_{x}=\mu_{x}+\left(\frac{\sigma(x,z)}{\sigma_{z}^{2}}\right)z+e_{x}
\end{equation}
...
\begin{equation}
\sigma(x,y||z)=\sigma(e_{x},e_{y})=\sigma\left[x-\left(\frac{\sigma(x,z)}{\sigma_{z}^{2}}\right)z,y-\left(\frac{\sigma(y,z)}{\sigma_{z}^{2}}\right)z\right]
\end{equation}

which, by expansion, leads to

\begin{equation}
\begin{aligned}\sigma(x,y||z)&=\sigma(x,y)-\left(\frac{\sigma(x,z)}{\sigma_{z}^{2}}\right)\sigma(y,z)-\left(\frac{\sigma(y,z)}{\sigma_{z}^{2}}\right)\sigma(x,z)+\left(\frac{\sigma(y,z)}{\sigma_{z}^{2}}\right)\left(\frac{\sigma(x,z)}{\sigma_{z}^{2}}\right)\sigma_{z}^{2}\\&=\sigma(x,y)-\frac{\sigma(x,z)\sigma(y,z)}{\sigma_{z}^{2}}\quad(6.33\mathrm{d})\end{aligned}
\end{equation}
...
\section{Parent-Offspring Regressions Before and After Selection}
...
\section{Heywood's Decomposition of Response}
