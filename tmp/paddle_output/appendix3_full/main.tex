...
Consider the integral  $ I(y) = \int f(y \mid x) p(x) \, dx $, which we approximate by
...
\begin{equation}
\widehat{q}(f,g)=\frac{1}{n}\sum_{i=1}^{n}\frac{g(\mathbf{x}_{i})}{g(\mathbf{x}_{i})+f(\mathbf{x}_{i})}\rightarrow q(f,g)
\end{equation}

where  $ \mathbf{x}_1, \cdots, \mathbf{x}_n $ are random draws from  $ f $. For example, if  $ f $ is a multivariate normal with a mean vector of 0 and a variance-covariance matrix of  $ \mathbf{G}_1 $, then

\begin{equation}
f(\mathbf{x})=(2\pi)^{-n/2}|\mathbf{G}_{1}|\exp\left(-\frac{\mathbf{x}^{T}\mathbf{G}_{1}^{-1}\mathbf{x}}{2}\right)
\end{equation}

where g is similarly defined, but with  $ G_{2} $ replacing  $ G_{1} $, which yields

\begin{equation}
\widehat{q}(f,g)=\frac{1}{n}\sum_{i=1}^{n}\frac{\mid\mathbf{G}_{2}\mid\exp\left(-\mathbf{x}_{i}^{T}\mathbf{G}_{2}^{-1}\mathbf{x}_{i}/2\right)}{\mid\mathbf{G}_{2}\mid\exp\left(-\mathbf{x}_{i}^{T}\mathbf{G}_{2}^{-1}\mathbf{x}_{i}/2\right)+\mid\mathbf{G}_{1}\mid\exp\left(-\mathbf{x}_{i}^{T}\mathbf{G}_{1}^{-1}\mathbf{x}_{i}/2\right)}
\end{equation}
...
\begin{equation}
\begin{array}{l}P(Rain tomorrow\mid Rain today)=0.5\\ P(Sunny tomorrow\mid Rain today)=0.25\\ P(Cloudy tomorrow\mid Rain today)=0.25\end{array}
\end{equation}
...
Note that this Markov chain is irreducible, as all states communicate with each other. Suppose today is sunny. What is the weather expected to be like two days from now? Seven days? Here  $ \pi(0) = (0 \quad 1 \quad 0) $, which yields

\begin{equation}
\boldsymbol{\pi}(2)=\boldsymbol{\pi}(0)\mathbf{P}^{2}=(0.375\quad0.25\quad0.375)
\end{equation}
...
Conversely, suppose today is rainy, so  $ \pi(0) = (1 \quad 0 \quad 0) $. The expected future weather becomes

\begin{equation}
\pi(2)=\begin{pmatrix}0.4375&0.1875&0.375\end{pmatrix}\quad and\quad\pi(7)=\begin{pmatrix}0.4&0.2&0.4\end{pmatrix}
\end{equation}
...
\section{1. Start with any initial value,  $ \theta_{0} $, satisfying  $ f(\theta_{0}) > 0 $.}
...
\begin{equation}
\begin{align*}p(x)=C\cdot x^{-(n/2+1)}\cdot\exp\left({-a\over2x}\right)\end{align*}
\end{equation}
...
\begin{equation}
\alpha=\min\left(\frac{f(\theta^{*})}{f(\theta_{t-1})},1\right)=\min\left(\frac{(39.82)^{-2.5}\cdot\exp(-2/39.82)}{(1)^{-2.5}\cdot\exp(-2/2\cdot1)},1\right)=0.0007
\end{equation}
...
Under Metropolis-Hasting, we sample from  $ q(x, y) = \Pr(x \to y \mid q) $ and then accept the move with probability  $ \alpha(x, y) $, so the transition probability kernel is given by

\begin{equation}
\Pr(x\rightarrow y)=q(x,y)\alpha(x,y)=q(x,y)\cdot\min\left[\frac{p(y)q(y,x)}{p(x)q(x,y)},1\right]
\end{equation}

If the Metropolis-Hasting kernel satisfies  $ P(x \to y) p(x) = P(y \to x) p(y) $, or

\begin{equation}
q(x,y)\alpha(x,y)p(x)=q(y,x)\alpha(y,x)p(y)\quad for all x,y
\end{equation}
...
\begin{equation}
P(x\rightarrow y)p(x)=q(x,y)p(x)\quad and\quad P(y\rightarrow x)p(y)=q(y,x)p(y)
\end{equation}
...
\begin{equation}
\alpha(x,y)=\frac{p(y)q(y,x)}{p(x)q(x,y)}\quad and\quad\alpha(y,x)=1
\end{equation}
...
\begin{equation}
\alpha(x,y)=1\quad and\quad\alpha(y,x)=\frac{q(x,y)p(x)}{q(y,x)p(y)}
\end{equation}
...
\section{Burning-in the Sampler}
...
\section{Choosing a Jumping (Proposal) Distribution}
...
Example A3.5. Suppose we wish to use a  $ \chi^2 $ as our proposal distribution. Recall from Equation A2.27b, that for  $ x \sim \chi_n^2 $,
...
\section{Autocorrelation and Sample Size Inflation}
...
a sequence, $(\theta_1, \cdots, \theta_n)$, of length $n$. Correlations can occur between adjacent members, $\rho(\theta_i, \theta_{i+1}) \neq 0$, and, more generally, between more distant members, $\rho(\theta_i, \theta_{i+k}) \neq 0$. The $k$th order autocorrelation, $\rho_k$, can be estimated by

\begin{equation}
\widehat{\rho}_{k}=\frac{\mathrm{Cov}(\theta_{t},\theta_{t+k})}{\mathrm{Var}(\theta_{t})}=\frac{\sum\limits_{t=1}^{n-k}\left(\theta_{t}-\overline{\theta}\right)\left(\theta_{t+k}-\overline{\theta}\right)}{\sum\limits_{t=1}^{n-k}\left(\theta_{t}-\overline{\theta}\right)^{2}},\quad\mathrm{with}\quad\overline{\theta}=\frac{1}{n}\sum\limits_{t=1}^{n}\theta_{t}
\end{equation}
...
\section{The Monte Carlo Variance of a MCMC-Based Estimate}
...
\begin{equation}
\widehat{n}=\frac{\widehat{\gamma}(0)}{\mathrm{Var}\left(\widehat{h}\right)}
\end{equation}
...
Correlograms, which plot serial autocorrelations as a function of the time lag, are also very useful in accessing a run from an MCMC sampler. A plot of  $ \rho_k $ versus  $ k $ (the  $ k $th order autocorrelation versus the lag) should show geometric decay if the sampler series closely
...
\section{More Formal Approaches}
...
\section{Practical MCMC: How Many Chains and How Long Should They Run?}
...
\begin{equation}
p(x,y)=\frac{n!}{(n-x)!x!}\ y^{x+\alpha-1}\left(1-y\right)^{n-x+\beta-1}
\end{equation}
...
\begin{equation}
p(z\mid q,n)\propto\frac{q^{z}(1-q)^{n-z}}{z!(n-z)!}\quad for\quad0\leq z\leq n
\end{equation}
...
\begin{equation}
p(z\mid a,b)\propto z^{a-1}(1-z)^{b-1}\quad for\quad0\leq z\leq1
\end{equation}
...
\section{Using the Gibbs Sampler to Approximate Marginal Distributions}
...
\begin{equation}
\overline{x}_{3}=\frac{5+3+7}{3}=5,\quad\overline{y}_{3}=\frac{0.5+0.33+0.56}{3}=0.46
\end{equation}
...
\begin{equation}
\mathrm{Var}(x)_{3}=(\overline{x^{2}})_{3}-(\overline{x}_{3})^{2}=2.67,\qquad\mathrm{Var}(y)_{3}=(\overline{y^{2}})_{3}-(\overline{y}_{3})^{2}=0.25
\end{equation}

and their covariance as

\begin{equation}
\mathrm{Cov}(x,y)_{3}=(\overline{xy})_{3}-\overline{x}_{3}\cdot\overline{y}_{3}=2.47-5\cdot0.46=0.16
\end{equation}
...
\begin{equation}
p_{3}(x)=\frac{10!}{x!(10-x)!}\left[\frac{0.5^{x}(1-0.5)^{10-x}+0.33^{x}(1-0.33)^{10-x}+0.56^{x}(1-0.56)^{10-x}}{3}\right]
\end{equation}
...
\section{REJECTION SAMPLING AND APPROXIMATE BAYESIAN COMPUTATION (ABC)}
