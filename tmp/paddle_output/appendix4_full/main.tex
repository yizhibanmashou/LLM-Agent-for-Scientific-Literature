...
Example A4.2. Reconsider the data from Example A4.1. The  $ Z_i $ values are easily obtained using  $ R $, as the command  $ \text{qnorm}(1-\text{p}) $ returns  $ Z $ satisfying  $ \text{Pr}(U \leq Z) = 1 - p $, or (equivalently)  $ \text{Pr}(U > Z) = p $. For example,  $ Z_1 $ is calculated by  $ \text{qnorm}(1-0.1) $, or 1.281. Similarly
...
\begin{equation}
\sum_{i=1}^{5}Z_{i}=6.754,\qquad hence\qquad Z_{s}=\frac{6.754}{\sqrt{5}}=3.020
\end{equation}

Because  $ \Pr(U > 3.020) = 0.00126 $, as in Example A4.1, the combined p value is highly significant.
...
\section{BONFERRONI CORRECTIONS AND THEIR EXTENSIONS}
...
\section{Standard Bonferroni Corrections}
...
Example A4.3. Suppose we have $n = 100$ independent tests and wish to obtain an overall $\pi$ value of 0.05. What value of $\alpha$ should be used for each individual test to achieve an experiment-wide false-positive rate of 0.05? The Dunn-Sidak correction suggests
...
\section{Sequential Bonferroni Corrections}
...
\begin{equation}
k^{*}=\max_{i}\left(p(n-i+j)>\pi\frac{j}{i}\right)\quad for all j=1,\cdots,i
\end{equation}
...
\section{Cheverud's Method and Other Approach for Dealing with Dependence}
...
\section{DETECTING AN EXCESS NUMBER OF SIGNIFICANT TESTS}
...
\section{How Many False Positives?}
...
\begin{equation}
\sum_{j=15}^{250}\Pr(j\ false\ positives)=\sum_{j=15}^{250}\frac{250!}{(250-j)!j!}(1-0.025)^{250-j}0.025^{j}
\end{equation}
...
\section{Schweder-Spjøtvoll plots}
...
\section{Estimating  $ n_{0} $: Subsampling From a Uniform Distribution}
...
\begin{equation}
\begin{aligned}\widehat{\pi}_{0}(a,b)&=\frac{Number of p(i)values in(a,b)}{n(b-a)}\\&=\frac{Fraction of p(i)values in(a,b)}{b-a}\end{aligned}
\end{equation}
...
Storey and Tibshirani (2003) considered the number of $p$ values exceeding some tunable parameter value, $\lambda$ (taking $a = \lambda$ and $b = 1$ in Equation A4.8b), on the logic that for larger values of $\lambda$, most of these draws are from the uniform corresponding to draws from the null. Let $\widehat{\pi}_0(\lambda)$ denote the estimated fraction of truly null hypotheses based on using a tuning
...
\section{Estimating  $ n_{0} $: Mixture Models}
...
\section{FDR: THE FALSE-DISCOVERY RATE}
...
\section{Morton's Posterior Error Rate (PER) and the FDR}
...
\begin{equation}
\frac{\alpha\cdot0.95}{\alpha\cdot0.95+0.80\cdot0.05}=0.05
\end{equation}

yields  $ \alpha = 0.0022 $, and hence, setting this as the Type-I error gives a PER of 5%.
...
\begin{equation}
PER=\frac{24.995}{24.995+1}=0.961
\end{equation}
...
\section{A Technical Aside: Different Definitions of False-discovery Rate}
...
\section{The Benjamini-Hochberg FDR Estimator}
...
\section{A (Slightly More) Formal Derivation of the Estimated FDR}
...
\section{Closing Caveats in Using the FDR}
...
\section{Informal, or Narrative, Meta-analysis}
...
\section{Standardizing Effect Sizes}
...
\section{Fixed-effects, Random-effects, and Mixed-model Meta-analysis}
...
\section{Publication and Other Sources in Bias}
...
\begin{equation}
y_{i}=a+bs_{i}+e,\quad where\quad y_{i}=(T_{i}-\overline{T})
\end{equation}
...
\section{Bias When Estimating Magnitudes}
...
\begin{equation}
\begin{align*}E[|x|]=\sigma\sqrt{2\over\pi}\exp\left(-{\mu^2\over2\sigma^2}\right)+|\mu|\cdot\textrm{erf}\left({\mu|\over\sigma\sqrt{2}}\right)\end{align*}
\end{equation}
...
which follows because for large values of x,  $ e^{-x} \to 0 $ and  $ \mathrm{erf}(x) \to 1 $.
