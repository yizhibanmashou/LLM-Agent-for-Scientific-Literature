...
\title{Selection Response in Natural Populations}
...
\section{EVOLUTION IN NATURAL POPULATIONS: WHAT IS THE TARGET OF SELECTION?}
...
\section{Direct and Correlated Responses}
...
\section{Environmentally Generated Correlations Between Fitness and Traits}
...
\begin{equation}
S_{alkaloid level}=\sigma(alkaloid level,soil nitrate)\beta_{soil nitrate}\neq0
\end{equation}

\begin{equation}
R_{alkaloid level}=\sigma(A_{alkaloid level},A_{soil nitrate})\beta_{soil nitrate}=0\cdot\beta_{soil nitrate}=0
\end{equation}
...
\section{THE FISHER-PRICE-KIRKPATRICK-ARNOLD MODEL FOR EVOLUTION OF BREEDING DATE}
...
\section{MODIFYING THE BREEDER'S EQUATION FOR NATURAL POPULATIONS}
...
\section{Complications in the Absence of Environmental Change}
...
\section{Additional Complications From Environmental Change}
...
\section{IS A FOCAL TRAIT THE DIRECT TARGET OF SELECTION?}
...
\section{Robertson's Theorem: Response Prediction Without Regard to the Target of Selection}
...
\section{Robertson Consistency Tests}
...
\section{Rausher's Consistency Criteria}
...
namely, the selection differential,  $ S_A = \sigma(A, w) $, based on the breeding value of a trait is simply  $ h^2 $ times the phenotypic selection differential,  $ S_z = \sigma(z, w) $. Although Rausher's
...
The  $ \beta_A = \beta_E $ and  $ \beta_A \neq 0 $ tests were suggested before the application of mixed models to natural populations, leaving the critical issue of how to estimate breeding values
...
\section{Morrissey et al.'s Consistency Criteria}
...
\section{The Breeder’s Equation Versus the Secondary Theorem}
...
Which approach, the breeder’s equation,  $ R = h^2 S = \sigma^2(A)\beta $, or the secondary theorem,  $ R = \sigma(A_z, w) $, should be used by an investigator? In large part, this depends on the question
...
\section{APPLYING MIXED MODELS TO NATURAL POPULATIONS: BASICS}
...
\section{Animal-model Analysis in Natural Populations: Overview}
...
\section{Obtaining the Relationship Matrix: Direct Observation of the Pedigree}
...
\section{Obtaining the Relationship Matrix: Marker Data}
...
\begin{equation}
s_{\ell}=p_{\ell}^{2}+\left(1-p_{\ell}\right)^{2}=1-2p_{\ell}(1-p_{\ell})
\end{equation}
...
\begin{equation}
\left(T_{x,\ell}-2p_{\ell}\right)\left(T_{y,\ell}-2p_{\ell}\right)
\end{equation}

The variance in  $ T_\ell $ is  $ \sigma^2(T_\ell) = E[T_\ell^2] - (E[T_\ell])^2 $, which under, Hardy-Weinberg, becomes

\begin{equation}
\left[0^{2}\cdot(1-p_{\ell})^{2}+1^{2}\cdot2\cdot p_{\ell}(1-p_{\ell})+2^{2}\cdot p_{\ell}^{2}\right]-(2p_{\ell})^{2}=2p_{\ell}(1-p_{\ell})
\end{equation}
...
To relate  $ \text{corr}(T_{x,\ell}, T_{y,\ell}) $ with  $ \theta_{xy} $, we write  $ T_{x,\ell} = b_{x,\ell} + b'_{x,\ell} $, where  $ b $ and  $ b' $ represent the two SNP alleles in  $ x $, to yield that

\begin{equation}
\sigma(T_{x,\ell},T_{y,\ell})=\sigma(b_{x,\ell}+b_{x,\ell}^{\prime},b_{y,\ell}+b_{y,\ell}^{\prime})=4\sigma(b_{x,\ell},b_{y,\ell})
\end{equation}
...
\section{Consequences of Pedigree Errors}
...
\section{APPLYING MIXED MODELS TO NATURAL POPULATIONS}
...
\section{Consistency Tests: Accuracy, Reliability, and Caveats when Using PBVs}
...
\section{Bivariate Animal Models: REML Estimates of  $ \sigma(A_{z}, A_{w}), S_{A} $, and  $ S_{z} $}
...
\section{Next-generation Analysis: Generalized Mixed Models}
...
\section{Next-generation Analysis: Modeling Missing Data}
...
\section{Detecting Genetic Trends}
...
\section{CAUSES OF APPARENT FAILURES OF RESPONSE IN NATURAL POPULATIONS}
...
\section{Cryptic Evolution: Genetic Change Masked by Environmental Change}
...
\section{Antler Size in Red Deer: The Focal Trait Is Not the Target of Selection}
...
\section{Lower Heritabilities in Environments with Stronger Selection?}
...
\section{Fitness Tradeoffs and Multivariate Constraints}
