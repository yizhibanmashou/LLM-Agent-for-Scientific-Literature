...
\title{Using Molecular Data to Detect Selection: Signatures from Recent Single Events}
...
\section{AN OVERVIEW OF STRATEGIES BASED ON SEGREGATING VARIATION}
...
\section{Attempts to Account for Departures From the Equilibrium Model}
...
\section{SNP Ascertainment Bias}
...
\section{SNP Polarity Assignment Errors}
...
\section{Background Selection as the More Appropriate Null?}
...
\section{Structure of the Remainder of This Chapter}
...
\section{ALLELE-FREQUENCY CHANGE IN A SINGLE POPULATION}
...
\section{Allele-frequency Change Over Two Sample Points: The Waples Adjusted Test}
...
\begin{equation}
\sigma^{2}(p_{t})=p_{0}(1-p_{0})\left[1-\left(1-\frac{1}{2N_{e}}\right)^{t}\right]\simeq p_{0}(1-p_{0})\frac{t}{2N_{e}}\quad\mathrm{f o r}t\ll N_{e}
\end{equation}
...
For simplicity, assume sampling without replacement, so that  $ \sigma\left(\widehat{p}_{0},\widehat{p}_{t}\right)=0 $, with the variances are given by Equations 9.2a and 9.2b. The resulting covariance matrix, V, becomes

\begin{equation}
\frac{\mathbf{V}}{p_{0}(1-p_{0})}=\begin{pmatrix}{{{\frac{1}{2\cdot223}}}}&{{{0}}} \\{{{0}}}&{{{\frac{1}{2\cdot1341}+\frac{8}{2000}\left[1-\frac{1}{2\cdot1341}\right]}}}\end{pmatrix}=\begin{pmatrix}{{{0.0022}}}&{{{0}}} \\{{{0}}}&{{{0.0044}}}\end{pmatrix}
\end{equation}

Because  $ V^{-1} $ appears in both the numerator and the denominator of Equation 9.4, the unknown constant,  $ p_{0}(1 - p_{0}) $, cancels out, allowing us to simply use the above right-hand matrix for  $ V $, yielding

\begin{equation}
GLS\left(p_{0}\right)=\frac{\mathbf{1}^{T}\mathbf{V}^{-1}\mathbf{p}}{\mathbf{1}^{T}\mathbf{V}^{-1}\mathbf{1}}=\frac{49.496}{674.762}=0.0734
\end{equation}

Equation 9.2c yields the sampling variance for the difference in allele frequencies as

\begin{equation}
\begin{aligned}\sigma^{2}\left(\widehat{\delta}_{t}\right)&\simeq p_{0}(1-p_{0})\left[\frac{1}{2n_{0}}+\frac{1}{2n_{t}}+\frac{t}{2N_{e}}\left(1-\frac{1}{2n_{t}}\right)\right]\\ &=0.0734\cdot0.9266\left[\frac{1}{446}+\frac{1}{2682}+\frac{8}{2000}\left(1-\frac{1}{2682}\right)\right]=0.0004495\end{aligned}
\end{equation}
...
\section{Allele-frequency Change Over a Times Series: The Fisher-Ford Test}
...
\begin{equation}
\mathbf{y}=\begin{pmatrix}0.616\\ 0.478\\ 0.387\end{pmatrix},\quad\mathbf{1}=\begin{pmatrix}1\\ 1\\ 1\end{pmatrix},\quad\mathbf{y}\text{ielding}\quad\widehat{y}_{0}=\frac{\mathbf{1}^{T}\mathbf{V}^{-1}\mathbf{y}}{\mathbf{1}^{T}\mathbf{V}^{-1}\mathbf{1}}=\frac{418.851}{774.701}=0.541
\end{equation}
...
\section{Schaffer's Linear Trend Test}
...
\section{Scans and Simulation-based Approaches}
...
\section{Birthdate Selection Mapping (BDSM)}
...
\section{DIVERGENCE BETWEEN POPULATIONS: TWO-POPULATION COMPARISONS}
...
\section{DIVERGENCE BETWEEN POPULATIONS:  $ F_{ST} $-BASED TESTS}
...
\section{Tests Based on  $ F_{ST} $-generated Branch-lengths}
...
\section{The Lewontin-Krakauer Test: Basics}
...
\section{Second-generation Lewontin-Krakauer Tests: Model-based Adjustments}
...
\section{Third-generation Lewontin-Krakauer Tests: Correcting for Population Structure}
...
\section{ALLELE-FREQUENCY CORRELATIONS WITH ENVIRONMENTAL VARIABLES}
...
\section{Joost's Spatial Analysis Method (SAM)}
...
\section{Accounting for Population Structure: Coop's Bayenv and Frichot's LFMM}
...
\section{CHANGES IN THE CHROMOSOMAL PATTERN OF NEUTRAL VARIATION}
...
\section{Simple Visual Scans for Changes in Nucleotide and STR Diversity}
...
\section{Tests Based on STR Variation Across Populations}
...
\section{Tests of Sweeps Using Bottleneck Models}
...
\section{Tests of Sweeps Using Genomic Positional Information: CLRT-GOF}
...
Because Equation 9.16a gives a proper likelihood for each site, one might imagine that the total likelihood  $ L(\boldsymbol{\Theta} \mid \mathbf{k}, n) $, given the vector  $ \mathbf{k} $ of the  $ k_i $ values for each site, is simply the product of the site-specific likelihoods over all  $ m $ sites
...
\begin{equation}
\begin{aligned}p_{S,i}=P_{e}(n)p_{i}+\sum_{\ell=0}^{n-1}P_{e}(\ell)\left[\Pr\left(i+1+\ell-n\mid\ell+1\right)\frac{i+1+\ell-n}{\ell+1}\right.\\ \left.+\Pr\left(i\mid\ell+1\right)\frac{\ell+1-i}{\ell+1}\right]\end{aligned}
\tag{9.18c}
\label{eq:9_18c}
\end{equation}
...
\section{Tests of Sweeps Using Genomic Positional Information: XP-CLR}
...
\section{Model Fragility: Demography, Mutation, Recombination, and Gene Conversion}
...
\section{TESTS BASED ON SITE-FREQUENCY SPECTRUM STATISTICS}
...
\section{Summary Statistics Based on Infinite-sites Models}
...
\begin{equation}
\widehat{\theta}_{S}=\frac{S}{a_{10}},\mathrm{w i t h}a_{10}=\sum_{i=1}^{9}\frac{1}{i}=2.83\mathrm{y i e l d i n g}\widehat{\theta}_{S}=\frac{12}{2.83}=4.24
\end{equation}

\begin{equation}
\widehat{\theta}_{\Pi}=\Pi=4\qquad\widehat{\theta}_{1^{*}}=\frac{n}{n-1}\eta_{1}=\frac{10}{9}\cdot3=3.33
\end{equation}
...
which is simply the average pairwise difference, II. As with S, II is symmetric with respect to i and n - i, so that both folded and unfolded data return the same estimate. Taking  $ c_1 = 1 $,  $ c_i > 1 = 0 $ yields  $ g = \zeta_1 $ (the number of derived singletons) and  $ h(n) = 1 $, recovering the  $ \widehat{\theta}_1 $ estimator.
...
\begin{equation}
a_{7}=\sum_{i=1}^{6}\frac{1}{i}=2.45,\qquad b_{7}=\sum_{i=1}^{6}\frac{1}{i^{2}}=1.49
\end{equation}

\begin{equation}
\widehat{\theta}_{S}=\frac{S}{a_{n}}=\frac{45}{2.45}=18.38,\qquad\widehat{\theta}_{\Pi}=\Pi=15.38
\end{equation}

305

\begin{equation}
\beta_{D}=\frac{1}{2.45^{2}+1.49}\left(\frac{2(7^{2}+7+3)}{9\cdot7(7-1)}-\frac{7+2}{7\cdot2.45}+\frac{1.49}{2.45^{2}}\right)=0.00475
\end{equation}

\begin{equation}
\alpha_{D}=\frac{1}{2.45}\left(\frac{7+1}{3(7-1)}-\frac{1}{2.45}\right)-0.00475=0.01005
\end{equation}

\begin{equation}
D=\frac{\widehat{\theta}_{\Pi}-\widehat{\theta}_{S}}{\sqrt{\alpha_{D}S+\beta_{D}S^{2}}}=\frac{15.38-18.38}{\sqrt{0.01005\cdot45+0.00475\cdot45^{2}}}=-0.945
\end{equation}
...
\section{Fu and Li's  $ D^{*} $ and  $ F^{*} $ Tests}
...
\section{Adjusting the Null to Account for Nonequilibrium Populations}
...
\section{Support via a Preponderance of Evidence}
...
\section{Recombination Makes Site-frequency Tests Conservative}
...
\section{Defining and Inferring Haplotypes}
...
\section{Overview of Haplotype-based Tests}
...
\section{The Ewens-Watterson Test}
...
\section{Other Infinite-alleles Tests: Conditioning on  $ \hat{\theta} $}
...
These coefficients are called Stirling numbers, hence the  $ S_n^i $ notation. This is a test for a deficiency of rare alleles/haplotypes, and hence is one-sided. Fu showed that the W test is more
...
\section{Other Infinite-alleles Tests: Conditioning on S}
...
\section{Garud et al.'s  $ H_{12} $ and  $ H_{2} $ Tests}
...
\section{Recombination and Infinite-alleles-based Tests}
...
\section{Pairwise Disequilibrium Tests: Kelly's  $ Z_{nS} $ and Kim and Nielsen's  $ \omega_{max} $}
...
\section{Contrasting Allele-frequency vs. Intra-allelic Variation Estimates of Haplotype Age}
...
Example 9.14. Recall Example 2.3, where we showed (following Stephens et al. 1998) that the estimated age of the HIV-resistant CCR5- $ \delta $32 mutation (roughly 5100 generations from Equation 2.12) was incompatible with an independent estimate of its age based on intra-allelic

322

variation (roughly 28 generations). Is this conclusion changed if Equation 9.38b and 9.38d are used instead of Equation 2.12 to estimate age from frequency? In Example 2.3, we used  $ p = 0.10 $ and  $ N_e = 5000 $. Taking  $ n = 4000 $ as the approximate sample size of Europeans used to estimate the allele frequency, Equation 9.38b gives the MLE as

\begin{equation}
\mathrm{MLE}(t)=-2N_{e}\left(\ln(1-p)+\frac{2}{n}\right)=-10,000\left(\ln(0.9)+\frac{2}{4000}\right)=1048\{\text{generations}\}
\end{equation}
...
\begin{equation}
\Pr(S=3\mid t\mu)=\frac{\lambda^{3}}{3!}\exp(-\lambda)\quad where\quad\lambda=n\mu t=0.0004498t
\end{equation}
...
\begin{equation}
\Pr(S\leq3\mid\lambda)=\exp(-\lambda)\left(1+\lambda+\frac{\lambda^{2}}{2!}+\frac{\lambda^{3}}{3!}\right)
\end{equation}
...
\section{Long-haplotype Tests Using Within-population Data: rEHH, LDD, iHS,  $ nS_{L} $, SDS, and DIND}
...
\section{Long-haplotype Tests Using Between-population Data: XP-EHH, Rsb, rHH, rMHH, and  $ \chi_{MD} $}
...
\section{Tests based on the allele-frequency spectrum AFS(k):}
\begin{table}[h]
\centering
\begin{tabular}{|c|c|}
\hline
...
\section{Summary: Tests Based on Haplotype/LD Information}
...
\section{SEARCHES FOR SELECTION: HUMANS}
...
\section{Recent and Current Selection in Humans}
...
\section{Balancing Selection in Humans}
...
\section{SEARCHES FOR SELECTION: DOMESTICATED ORGANISMS}
...
The distinction between genes involved in the initial domestication events and those involved in the subsequent improvement of varieties can be rather subtle. Operationally, domestication genes are assumed to be present in all descendant varieties from the domestication gene.
...
\section{The Process of Domestication}
...
\section{Finding Domestication and Improvement Genes in Crops}
...
\section{Domestication and Improvement Genes in Rice}
...
\section{Domestication and Improvement Genes in Maize}
...
\section{Relative Strengths of Selection on Domestication vs. Improvement Genes}
...
\section{Constraints on Finding Domestication and Improvement Genes Through Selective Signals}
