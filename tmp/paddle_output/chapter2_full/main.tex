...
\title{Neutral Evolution in One-
and Two-Locus Systems}
...
where the elements of the row vector  $ \mathbf{x}(t) $ are the probabilities that the allele is present in  $ i = 0, 1, \ldots, 2N $ copies in generation  $ t $. Note that  $ P_{ij} $, which refers to the element in row  $ i $ and  $ j $
...
\begin{equation}
\mathbf{P}=\begin{pmatrix}{{{1}}}&{{{0}}}&{{{0}}} \\{{{0.25}}}&{{{0.50}}}&{{{0.25}}} \\{{{0}}}&{{{0}}}&{{{1}}}\end{pmatrix}
\end{equation}
...
\begin{equation}
\mathbf{P}^{2}=\begin{pmatrix}{{{1}}}&{{{0}}}&{{{0}}} \\{{{0.375}}}&{{{0.250}}}&{{{0.375}}} \\{{{0}}}&{{{0}}}&{{{1}}}\end{pmatrix},\quad\mathbf{P}^{5}=\begin{pmatrix}{{{1}}}&{{{0}}}&{{{0}}} \\{{{0.48438}}}&{{{0.03125}}}&{{{0.48438}}} \\{{{0}}}&{{{0}}}&{{{1}}}\end{pmatrix}
\end{equation}
...
\section{LOSS OF HETEROZYGOSITY BY RANDOM GENETIC DRIFT}
...
\section{PROBABILITIES AND TIMES TO FIXATION OR LOSS}
...
\begin{equation}
\bar{t}_{a}(p)=E[\bar{t}_{a}(p+\delta p)]+1
\end{equation}
...
\begin{equation}
\begin{align*}E[\bar{t}_{a}(p+\delta p)]&\simeq E\left[\bar{t}_{a}(p)+\delta p\frac{\partial\bar{t}_{a}(p)}{\partial p}+\frac{(\delta p)^{2}}{2}\frac{\partial^{2}\bar{t}_{a}(p)}{\partial p^{2}}\right]\\&=\bar{t}_{a}(p)+E[\delta p]\frac{\partial\bar{t}_{a}(p)}{\partial p}+\frac{E[(\delta p)^{2}]}{2}\frac{\partial^{2}\bar{t}_{a}(p)}{\partial p^{2}}\end{align*}
\end{equation}
...
\begin{equation}
\overline{t}_{a}(p)-\overline{t}_{a}(p)-1\simeq\frac{p(1-p)}{2\cdot2N}\frac{\partial^{2}\overline{t}_{a}(p)}{\partial p^{2}}
\end{equation}

which implies the differential equation

\begin{equation}
\frac{\partial^{2}\bar{t}_{a}(p)}{\partial p^{2}}\simeq-\frac{4N}{p(1-p)}
\end{equation}
...
\begin{equation}
\left(\frac{2}{p}\right)\frac{\partial\bar{t}_{f}(p)}{\partial p}+\frac{\partial^{2}\bar{t}_{f}(p)}{\partial p^{2}}\simeq-\frac{4N}{p(1-p)}
\end{equation}
...
\section{THE AGE OF A NEUTRAL ALLELE}
...
\begin{equation}
\widehat{t}=-\frac{4\cdot5000\cdot0.1\log(0.1)}{0.9}=5116\{\text{generations}\}
\end{equation}
...
However, an independent (and more direct) estimate of the allele's age can be obtained by considering the variation in haplotypes among all sequences carrying this mutation. The  $ \delta32 $ mutation is in strong linkage disequilibrium with allele 215 at the AFMB marker (a highly
...
\begin{equation}
t\simeq\ln(\pi)/\ln(1-c)=27.4\{\mathrm{g e n e r a t i o n s}\}
\end{equation}
...
\begin{equation}
\widehat{t}=-\frac{(4N)0.7\ln(0.7)}{1-0.7}=1.45N
\end{equation}
...
the solution of which is  $ \tau_{0.025} = 0.49 $, or  $ 0.49 \times (2N) = 0.98N $ generations (Equation 9.38d gives an approximation for  $ \tau $ when n is large). The same procedure can be used to find that  $ \tau_{0.975} = 3.9N $, showing that the confidence interval about the mean value is very asymmetric.

\section{ALLELE-FREQUENCY DIVERGENCE AMONG POPULATIONS}
...
Because there are no systematic forces causing the allele frequency to increase or decrease,  $ E(p_t) = p_0 $, so the first quantity on the right is  $ p_0(1 - p_0) $. The quantity  $ E[p_t(1 - p_t)] $ is half
...
bright red-orange,  $ bw^{75}bw = \text{deep red-brown} $, and  $ bwbw = \text{white} $. (Two separate experiments were performed, one with 107 and the other with 105 populations, but the results are so similar that they have been pooled in the following analysis.)
...
\section{HIGHER-ORDER ALLELE-FREQUENCY MOMENTS}
...
\begin{equation}
\begin{aligned}\sigma^{2}(H_{t})&=E\{[2p_{t}(1-p_{t})]^{2}\}-\{E[2p_{t}(1-p_{t})]\}^{2}\\&=E(4p_{t}^{2})-E(8p_{t}^{3})+E(4p_{t}^{4})-[E(2p_{t}-p_{t}^{2})]^{2}\end{aligned}
\end{equation}
...
\begin{equation}
\begin{array}{ccc}{{{A}}}&{{{a}}} \\{{{B}}}&{{{p}}}&{{{q-p}}} \\{{{b}}}&{{{0}}}&{{{1-q}}} \\\end{array}
\end{equation}

and the resulting initial values for D and  $ r^{2} $ become

\begin{equation}
D_{\mathrm{AB}}=p_{AB}p_{ab}-p_{aB}p_{Ab}=p(1-q)-0\cdot(q-p)=p(1-q)
\end{equation}
...
\begin{equation}
\begin{array}{cccc}{{{A}}}&{{{a}}} \\{{{B}}}&{{{p}}}&{{{0}}} \\{{{b}}}&{{{0}}}&{{{1-p}}} \\\end{array},\qquad or equivalently \quad\begin{array}{cccc}{{{\mathrm{A}}}}&{{{\mathrm{a}}}} \\{{{\mathrm{B}}}}&{{{q}}}&{{{0}}} \\{{{\mathrm{b}}}}&{{{0}}}&{{{1-q}}} \\\end{array}
\end{equation}
...
problem under consideration is short ( $ t \ll 2N $) and the initial level of within-population variation is high (relative to the mutational rate of production of new heterozygosity per generation), the contribution from mutation will be negligible, and the preceding expressions will be quite adequate. However, for longer-term evolutionary issues, such as the maintenance of variation in natural populations and interspecific divergence, mutation cannot be ignored.

The incorporation of mutation into a neutral model of evolution is relatively straightforward. For example, suppose there are  $ k $ possible alleles at a locus, each with a mutation rate  $ u $ per generation (the  $ k $-exchangeable alleles model). The dynamics of heterozygosity can then be obtained by recalling from above that the expected frequency of heterozygotes in generation  $ t + 1 $ in the absence of mutation is  $ [1 - (1/2N)]H_t = \lambda_1H_t $, whereas the expected frequency of homozygotes is  $ 1 - \lambda_1H_t $. Following mutation, the heterozygous state will be retained if: (1) neither allele mutated, the probability of which is  $ (1 - 2u) $, ignoring the very small probability of double mutations to the same state; or (2) one of the alleles mutated to a different state than the other, the probability of which is  $ [2u(k - 2)/(k - 1)] $, assuming that all allelic types are equally mutationally exchangeable. On the other hand, homozygotes will be mutationally converted to heterozygotes at a rate of  $ 2u $. Thus, the expected dynamics of heterozygosity can be expressed as
...
Although the preceding results lead to predicted equilibrium allele frequencies, heterozygosities, etc., when viewed in isolation, such results obscure the long-term dynamics of neutral mutations. Given a population of size N, on average  $ 2N_u $ new mutations arise per nucleotide site per generation, each with initial frequency  $ 1/(2N) $. As noted earlier, the probability of fixation of a neutral allele is simply equal to its initial frequency, so that in the long run, at a neutral site, there is an average turnover rate of  $ 2N_u \cdot 1/(2N) = u $ mutations per generation. This simple but powerful result tells us that the long-term rate of nucleotide substitution at a neutral site is equal to the mutation rate, regardless of the size of the population, a hallmark of the neutral theory of molecular evolution (Kimura 1983). This long-term flux occurs in the face of the maintenance of quasi-steady-state within-population
...
\begin{equation}
E(r_{IBD}^{2})=\frac{1}{1+4Nc}=\frac{1}{1+\rho}
\end{equation}
...
sampled gametes will be drawn from different gametes leading to the preceding generation, the probability of which is  $ 1 - \frac{1}{2N} $. In the latter case, joint IBD will still exist if it happened to have been present for the two chromosomal segments in the preceding generation with no recombination again occurring in either. Summing up, the recursion equation for joint IBD is

\begin{equation}
Q_{t}=\frac{1}{2N}(1-c)^{2}+\left(1-\frac{1}{2N}\right)(1-c)^{2}Q_{t-1}
\end{equation}

Setting  $ Q_{t} = Q_{t-1} $ gives the equilibrium solution as

\begin{equation}
\tilde{Q}=\frac{1/(2N)}{1-[1-1/(2N)](1-c)^{2}}\simeq\frac{1}{1+4N c}
\end{equation}
...
\section{THE DETAILED STRUCTURE OF NEUTRAL VARIATION}
...
\section{The Infinite-alleles Model and the Associated Allele-frequency Spectrum}
...
\section{A A G  $ \underline{A} $ C C
A A G G C C
A A G  $ \underline{A} $ C C
A A G G C C
A A G G C  $ \underline{A} $}
...
\section{The Infinite-Sites Model and the Associated Site-frequency Spectrum}
An alternate framework for summarizing molecular variation is embodied in the infinites-
sites model, which treats a region as a series of L sites. Each new mutation is again viewed as
unique, but now occurring at a novel (not currently segregating) site, and as with the infinite-
alleles model, recombination within the region is assumed to be insignificant. Because
this model allows only a single mutation per site, a particular variant at a polymorphic
site is either ancestral (original) or derived (mutated), and each segregating site is treated
as biallelic. With sequence data from one or more outgroups, one can often polarize the
nucleotides at any particular site, determining which is derived by assuming the outgroup
harbors the ancestral state. In the absence of such information, the minor-allele frequency,
the frequency of the rarest nucleotide, is reported for each site. In the following expressions,
we will make use of both measures of the population mutation rate noted above, with
 $ \theta_L = 4NuL = \theta L $.
...
\section{THE GENEALOGICAL STRUCTURE OF A POPULATION}
...
\section{MUTATION-MIGRATION-DRIFT EQUILIBRIUM}
...
\section{Quantifying Population Structure:  $ F_{ST} $}
...
\section{Mutation-migration-drift Equilibrium Values of  $ F_{ST} $}
