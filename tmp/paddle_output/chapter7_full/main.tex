...
\section{SELECTION AND MUTATION AT SINGLE LOCI}
...
\begin{equation}
\mathrm{f r e q}(a a)+\mathrm{f r e q}(A a)=\mu^{2}+2\mu(1-\mu)\simeq2\mu
\end{equation}

whereas for a recessive, the frequency of selected individuals is

\begin{equation}
\mathrm{f r e q}(a a)=(\sqrt{\mu})^{2}=\mu
\end{equation}
...
\begin{equation}
\overline{W}=freq(AA)=(1-\mu)^{2}\simeq1-2\mu
\end{equation}
...
Example 7.2. Albinism in humans is caused by a recessive allele, with an estimated frequency of albinos of around 1/20,000 (Cavalli-Sforza and Bodmer 1971). If we assume that albinos are at a moderate selective disadvantage (s = 0.1) and at mutation-selection equilibrium, what is the estimated mutation rate to albino alleles? Assuming genotype frequencies in Hardy-Weinberg equilibrium, so that  $ \tilde{p}^2 = 1/20,000 $, from Equation 7.6c,

\begin{equation}
\widetilde{p}^{2}=\frac{1}{20,000}=\left(\sqrt{\frac{\mu}{0.1}}\right)^{2}
\end{equation}

which implies that  $ \mu = 5 \times 10^{-6} $. Conversely, if we were to assume a mutation rate of  $ \mu = 10^{-5} $, the strength of selection against albinism would be inferred from

\begin{equation}
\widetilde{p}^{2}=\frac{1}{20,000}=\left(\sqrt{\frac{10^{-5}}{s}}\right)^{2}
\end{equation}
...
\begin{equation}
\varphi=\frac{1}{1+4N_{e}\mu(1-\tilde{p})}=\frac{1}{1+\theta(1-\tilde{p})}
\end{equation}

where  $ \theta = 4N_e\mu $, with  $ \mu $ being defined here as the total mutation rate to deleterious alleles. This expression is closely connected to the results on mutation-drift balance presented in Chapter 2, where we showed that  $ \theta/(1 + \theta) $ is the probability of a heterozygote, and hence  $ 1/(1 + \theta) $

179

is the probability of a homozygote. To obtain the diversity within the set of disease alleles, here we restrict ourselves to the population of normal alleles,  $ 2N_e(1 - \bar{p}) $, as these are the source of mutations to new disease alleles. At mutation-selection equilibrium, this flux of new mutations is countered by the selective removal of existing alleles. The reciprocal of  $ \varphi $, which asymptotically approaches 1.0 for  $ \theta \ll 1 $, provides an estimate of the effective number of alleles.
...
\section{SELECTION AND DRIFT AT SINGLE LOCI}
...
\section{Probability of Fixation Under Additive Selection}
...
\section{Probability of Fixation Under Arbitrary Selection}
...
homozygotes of a completely recessive allele  $ (4N_e s \gg 1) $, a close approximation is given by
...
\section{Fixation of Overdominant and Underdominant Alleles}
...
For the situation in which the two homozygotes have equal fitness and heterozygotes experience a reduction in fitness of s, Lande (1979b) found that if  $ sN_e/N \ll 1 $ (a condition
...
\section{Expected Allele Frequency in a Particular Generation}
...
\section{JOINT INTERACTION OF SELECTION, DRIFT, AND MUTATION}
...
\section{HALDANE'S PRINCIPLE AND THE MUTATION LOAD}
...
\section{FIXATION ISSUES INVOLVING TWO LOCI}
...
\section{The Hill-Robertson Effect}
...
\section{Mutations with Contextual Effects}
