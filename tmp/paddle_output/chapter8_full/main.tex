...
\title{Hitchhiking and Selective Sweeps}
...
Gene B under this situation resembles a hitch-hiker riding along with a host driver.
...
\section{SWEEPS: A BRIEF OVERVIEW}
...
\section{Hitchhiking, Sweeps, and Partial Sweeps}
...
\section{Selection Alters the Coalescent Structure at Linked Neutral Sites}
...
\section{THE BEHAVIOR OF A NEUTRAL LOCUS LINKED TO A SELECTED SITE}
...
\begin{equation}
\frac{x}{1-x}=\epsilon e^{st_{x}}
\end{equation}

which, upon rearranging (with  $ \epsilon = 1/[4N_{e}s] $), yields
...
\begin{equation}
\begin{aligned}dq(t)&=q^{\prime}(t)-q(t)=\{p^{\prime}(t)q_{A}(t)+[1-p^{\prime}(t)]q_{a}(t)\}-\{p(t)q_{A}(t)+[1-p(t)]q_{a}(t)\}\\&=[p(t)+dp(t)]q_{A}(t)+[1-p(t)-dp(t)]q_{a}(t)-\{p(t)q_{A}(t)+[1-p(t)]q_{a}(t)\}\\&=dp(t)\left[q_{A}(t)-q_{a}(t)\right]\\ \end{aligned}
\end{equation}
...
\begin{equation}
p(0)\frac{1-p(t)}{p(t)}=e^{-st}
\end{equation}

Noting that  $ e^{ab} = (e^{a})^{b} $, we can write
...
\begin{equation}
\Delta_{q}=\delta_{q}(0)\int_{p_{o}}^{1}e^{-c t}d p=\delta_{q}(0)p(0)^{c/s}\int_{p_{o}}^{1}\left(\frac{1-p}{p}\right)^{c/s}d p
\end{equation}
...
\section{Reduction in Genetic Diversity Around a Sweep}
...
\section{The Messer-Neher Estimator of s}
...
\section{Recovery of Variation Following a Sweep}
The signal left by even a strong sweep is transient, as new mutations will eventually restore heterozygosity at neutral sites back to their equilibrium value  $ (H_0 = 4N_e\mu) $ before the sweep. Kim and Stephan (2000) found that the expected heterozygosity  $ t $ generations after
...
\section{Effects of Sweeps on the Variance in Microsatellite Copy Number}
...
\begin{equation}
\begin{align*}\phi^*(y)dy=(1-x)\phi(x)dx=(1-x)\frac{\theta}{x}dx\end{align*}
\end{equation}

The first step follows because  $ (1 - x) $ is the probability that a neutral allele (initially at a frequency of  $ x $) is in repulsion with the favorable allele, and the second step follows from Equation 2.34a. Recalling (for alleles in repulsion) that  $ y = x(1 - f_s) $, we have both  $ x = y/(1 - f_s) $ and  $ dx = dy/(1 - f_s) $. Substituting for  $ x $ and  $ dx $ in this expression yields

\begin{equation}
(1-x)\frac{\theta}{x}dx=\left(1-\frac{y}{1-f_{s}}\right)\frac{\theta(1-f_{s})}{y}\frac{dy}{1-f_{s}}
\end{equation}

which, after multiplication, yields the repulsion piece of the post-selection SFS as

\begin{equation}
\phi^{*}(y)d y=\theta\left(\frac{1}{y}-\frac{1}{1-f_{s}}\right)d y\quad\mathrm{f o r}\quad1/(2N)\leq y\leq1-f_{s}
\end{equation}

Conversely, with a probability of $x$, the derived allele is in coupling phase and is swept to a frequency of $y = f_{s} + x(1 - f_{s})$. For $x \simeq 0$, $y \simeq f_{s} + 0(1 - f_{s}) = f_{s}$, while the upper limit

224

follows from our restriction to segregating sites, yielding  $ f_s \leq y \leq 1 - 1/(2N) $. Following the same logic as before yields

\begin{equation}
\phi^{*}(y)d y=x\phi(x)d x=x\frac{\theta}{x}d x=\theta d x
\end{equation}

As before, the first step follows because $x$ is the probability that a neutral allele (initially at a frequency of $x$) is in coupling with the favorable allele. With an allele in coupling, $y = f_s + x(1 - f_s)$, yielding $x = (y - f_s)/(1 - f_s)$ and $dx = dy/(1 - f_s)$. Substituting for $dx$ into the previous expression yields the coupling piece of the post-sweep SFS as

\begin{equation}
\phi^{*}(y)d y=\frac{\theta}{1-f_{s}}d y\quad\mathrm{f o r}\quad f_{s}\leq y\leq1-1/(2N)
\end{equation}

Putting these two pieces together recovers Equation 8.13.

\section{The Pattern of Linkage Disequilibrium}
...
\section{Summary: Signatures of a Hard Sweep}
...
\begin{equation}
f_{s}\simeq\left\{\begin{array}{l l}{p(0)^{-c/(2h s)}\simeq1-\frac{c}{2h s}\ln[p(0)]}&{\mathrm{f o r~}p(0)\gg1/(2N_{e}s)}\\ {}&{}\\ {(4N_{e}s)^{-c/(2h s)}\simeq1-\frac{c}{2h s}\ln(4N_{e}s)}&{\mathrm{f o r~}p(0)=1/(2N)}\\ \end{array}\right.
\end{equation}

Total change in the frequency of a linked neutral marker:  $ \Delta_q \simeq [1 - q(0)] f_s $

\begin{equation}
q(\infty)=q(0)+\Delta_{q}=f_{s}+q(0)(1-f_{s})
\end{equation}

\begin{equation}
\frac{H_{h}}{H_{0}}=1-f_{s}^{2}
\end{equation}

\begin{equation}
\frac{H(t)}{H_{0}}=1-f_{s}^{2}e^{-t/(2N_{e})}
\end{equation}
...
(5) Remaining on the order of  $ 0.5N_{e} $ generations for signature (1),  $ 0.4N_{e} $ generations for (2),  $ 1.4N_{e} $ generations for (3), and  $ 0.1N_{e} $ generations for (4b).
...
\section{Signatures of a Soft Sweep}
...
\section{Sweeps Using Standing Variation}
...
\section{How Likely is a Sweep Using Standing Variation?}
Hermisson and Pennings (2005), Przeworski et al. (2005), and Berg and Coop (2015) used

232

population-genetic models to examine the likelihood of a sweep arising from standing variation. To consider the probability of such an event over a series of replicate populations, let  $ \phi(x) $ denote the distribution of the frequency, x, for the soon-to-be favored allele A, and  $ u(x) $ its probability of fixation under the new environment given x. The probability,  $ \Pr_{sv} $, that a sweep occurs using standing variation at this locus is simply
...
Now suppose that we examine this population at  $ T = 0.5 $, namely, at  $ N_e $ generations. From Equation 8.18c, the probability that at least one mutation that is destined to become fixed will arise by this time is

\begin{equation}
\begin{aligned}\Pr_{new}(T)&=1-\exp\left(-Th\alpha_{b}\theta_{b}\right)\\&=1-\exp\left(-0.5\cdot\left(1/2\right)\cdot400\cdot0.0033\right)=0.281\end{aligned}
\end{equation}
...
\begin{equation}
\Pr_{ex}(0.5)=\frac{\Pr_{sv}}{\Pr_{sv}+(1-\Pr_{sv})\Pr_{new}(T)}=\frac{0.013}{0.013+(1-0.013)\cdot0.281}=0.05
\end{equation}

\section{“Hardening” and “Softening” of a Sweep}
...
\section{Recurrent Mutation of the Favorable Allele Cannot be Ignored}
...
\begin{equation}
\begin{aligned}E(number of new favorable mutations that arise)&\approx2N(1/2)\mu_{b}\tau=(N/N_{e})(N_{e}\mu_{b})\tau\\&=(N/N_{e})(\theta_{b}/4)\cdot2\ln(4N_{e}s)/s\\&=2N\theta_{b}\ln(\alpha_{b})/\alpha_{b}\tag{8.22a}\end{aligned}
\end{equation}
...
Example 8.11. To get a feel for the expected number of new favorable mutations that arise during a sweep, consider the values used in Example 8.9 ( $ N_e = 10^6 $,  $ \theta_b = 0.0033 $,  $ s = 10^{-4} $,  $ \alpha_b = 400 $). From Equation 8.22a (and assuming  $ N \simeq N_e $), we expect

\begin{equation}
2N_{e}\theta_{b}\ln(\alpha_{b})/\alpha_{b}=2\cdot10^{6}\cdot0.0033\ln(400)/400\approx90
\end{equation}
...
\begin{equation}
\theta_{b}\ln(4N_{e}s)=0.0033\ln(400)=0.02
\end{equation}
...
\section{Impact of Geographic Structure}
...
\section{GENOME-WIDE IMPACT OF REPEATED SELECTION AT LINKED SITES}
...
\section{Association Between Levels of Variation and Localized Recombination Rates}
...
\section{Impact of Recurrent Hard Sweeps on Levels of Neutral Variation}
...
\section{A Few Large or Many Small Sweeps?}
...
\section{Selective Interference and the Hill-Robertson Effect}
...
\section{Background Selection: Reduction in Variation Under Low Recombination or Selfing}
...
\section{Background Selection vs. Recurrent Selective Sweeps}
...
\section{Sweeps, Background Selection, and Substitution Rates}
...
This roughly four-fold higher ratio of polymorphism to divergence for the putative deleterious mutations  $ (P \to U) $ is highly significant (Fisher's exact test yields  $ p = 6.4 \cdot 10^{-5} $). Similarly, note that the divergence ratio for these two classes

\begin{equation}
\frac{P\rightarrow U}{U\rightarrow P}=\frac{52}{32}=1.625
\end{equation}
...
\section{A Paradigm Shift Away From the Neutral Theory of Molecular Evolution?}
