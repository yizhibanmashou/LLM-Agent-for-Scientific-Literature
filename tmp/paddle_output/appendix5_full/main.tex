...
Much of the presentation that follows is in matrix notation, and for this I offer no apology as this has rapidly become an essential tool of any serious student of animal breeding.
Henderson (1973)
...
\section{THE GEOMETRY OF VECTORS AND MATRICES}
...
\section{Comparing Vectors: Lengths and Angles}
...
\section{Matrices Describe Vector Transformations}
...
\begin{equation}
\mathbf{G}=\begin{pmatrix}4&-2\\ -2&2\end{pmatrix}\quad and\quad\boldsymbol{\beta}=\begin{pmatrix}1\\ 3\end{pmatrix},\quad yielding\quad\mathbf{R}=\mathbf{G}\boldsymbol{\beta}=\begin{pmatrix}-2\\ 4\end{pmatrix}
\end{equation}

The resulting direction of change in character means is different from that most favored by natural selection. Selection ( $ \beta $) favors an increase in trait one ( $ z_1 $), but when the genetic covariance structure is taken into account, the resulting change in the mean of  $ z_1 $ is negative. If we take the appropriate inner products, we find  $ \|\beta\| = \sqrt{10} $,  $ \|\mathbf{R}\| = \sqrt{20} $, and  $ \beta^T \mathbf{R} = 10 $. Equation A5.2a returns
...
\section{Orthonormal Matrices: Rigid Rotations}
...
Example A5.2. Determine the eigenstructure of the genetic covariance matrix G shown in Example A5.1. Writing the characteristic equation, and recalling the expression for the determinant of a  $ 2 \times 2 $ matrix (LW Equation 8.12a), yields

\begin{equation}
\begin{aligned}\left|\mathbf{G}-\lambda\mathbf{I}\right|&=\left|\begin{pmatrix}4-\lambda&-2\\ -2&2-\lambda\end{pmatrix}\right|\\&=(4-\lambda)(2-\lambda)-(-2)^{2}=\lambda^{2}-6\lambda+4=0\end{aligned}
\end{equation}

Alternatively, if we use the Laplace expansion (Equation A5.7), and note that  $ \mathrm{tr}(\mathbf{G}) = 4 + 2 = 6 $ and  $ |\mathbf{G}| = 4 \cdot 2 - (-2)^2 = 4 $, we will also recover the characteristic equation, which has solutions

\begin{equation}
\lambda_{1}=3+\sqrt{5}\simeq5.236\qquad\lambda_{2}=3-\sqrt{5}\simeq0.764
\end{equation}

The associated unit eigenvectors (which as easily obtained, along with the eigenvectors, by using the R command eigen) are

\begin{equation}
\mathbf{e}_{1}\simeq\begin{pmatrix}-0.851\\ 0.526\end{pmatrix}\qquad\mathbf{e}_{2}\simeq\begin{pmatrix}0.526\\ 0.851\end{pmatrix}
\end{equation}

These are orthogonal as  $ \mathbf{e}_{1}^{T}\mathbf{e}_{2}=0 $.

The eigenstructure of G shows why the vector of responses,  $ \mathbf{R} $, is rotated away from the direction of the vector that corresponds to the direction of selection,  $ \beta $. From Example A5.1,  $ ||\boldsymbol{\beta}|| = \sqrt{10} $, while  $ \mathbf{e}_1^T \boldsymbol{\beta} \simeq 0.727 $ and  $ \mathbf{e}_2^T \boldsymbol{\beta} \simeq 3.079 $. Because  $ ||\mathbf{e}_1|| = ||\mathbf{e}_2|| = 1 $, Equation A5.2a simplifies to

\begin{equation}
\cos(\theta|\mathbf{e}_{1},\boldsymbol{\beta})\simeq\frac{0.727}{\sqrt{10}}\simeq0.230\qquad and\qquad\cos(\theta|\mathbf{e}_{2},\boldsymbol{\beta})\simeq\frac{3.079}{\sqrt{10}}\simeq0.974
\end{equation}

giving the angle between $\mathbf{e}_1$ and $\boldsymbol{\beta}$ as $\theta(\mathbf{e}_1, \boldsymbol{\beta}) \simeq 76.7^\circ$, while $\theta(\mathbf{e}_2, \boldsymbol{\beta}) \simeq 13.2^\circ$. Applying Equation A5.3b, the corresponding scaled projections of $\boldsymbol{\beta}$ on these eigenvectors are

\begin{equation}
\begin{aligned}\lambda_{1}Proj(\boldsymbol{\beta}on\mathbf{e}_{1})&=\lambda_{1}\cos(\theta|\mathbf{e}_{1},\boldsymbol{\beta})||\boldsymbol{\beta}||\mathbf{e}_{1}=\left(5.236\cdot0.230\cdot\sqrt{10}\right)\mathbf{e}_{1}\\&=3.803\left(\begin{array}{c}-0.851\\0.526\end{array}\right)=\left(\begin{array}{c}-3.236\\2\end{array}\right)\end{aligned}
\end{equation}

\begin{equation}
\begin{aligned}\lambda_{2}\mathrm{Proj}(\boldsymbol{\beta}\mathrm{on}\mathbf{e}_{2})&=\lambda_{2}\cos(\theta|\mathbf{e}_{2},\boldsymbol{\beta})||\boldsymbol{\beta}||\mathbf{e}_{2}=\left(0.764\cdot0.974\cdot\sqrt{10}\right)\mathbf{e}_{2}\\&=2.353\begin{pmatrix}0.526\\ 0.851\end{pmatrix}=\begin{pmatrix}1.236\\ 2\end{pmatrix}\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}\mathbf{R}=\mathbf{G}\boldsymbol{\beta}&=\lambda_{1}Proj(\boldsymbol{\beta}on\mathbf{e}_{1})+\lambda_{2}Proj(\boldsymbol{\beta}on\mathbf{e}_{2})\\&=\begin{pmatrix}-3.236\\ 2\end{pmatrix}+\begin{pmatrix}1.236\\ 2\end{pmatrix}=\begin{pmatrix}-2\\ 4\end{pmatrix}\end{aligned}
\end{equation}
...
\section{2. The eigenvalues and eigenvectors of a symmetric matrix are all real.}
...
\section{4. A symmetric matrix A can be diagonalized as}
\begin{equation}
...
recovering the spectral decomposition (Equation A5.9a). The last step in Equation A5.10d follows because  $ e_i^T e_j = 0 $ for  $ i \neq j $. Because of this feature, Equation A5.10a is also called the spectral factorization or eigendecomposition of A.
...
where the square root matrix,  $ A^{1/2} $, satisfies  $ A^{1/2}A^{1/2} = A $, and  $ A^{-1/2} $ satisfies  $ A^{-1/2}A = AA^{-1/2} = A^{1/2} $, as well as  $ A^{-1/2}A^{1/2} = A^{1/2}A^{-1/2} = I $.
...
Example A5.3. Consider the additive-genetic covariance matrix G from Examples A5.1 and A5.2. Recalling the results from Example A5.2 and using Equation A5.10a, we can express G as  $ U\Lambda U^{T} $, where

\begin{equation}
\boldsymbol{A}=\left(\begin{array}{cc}{{{5.241}}}&{{{0}}} \\{{{0}}}&{{{0.765}}}\end{array}\right)\quad and\quad\mathbf{U}=(\mathbf{e}_{1}\quad\mathbf{e}_{2})=\left(\begin{array}{cc}\left(-0.851\right)&\\0.526&\end{array}\right)\quad\left(\begin{array}{l}{{{0.526}}} \\{{{0.851}}}\end{array}\right)
\end{equation}
...
\section{Correlations Can Be Removed by a Matrix Transformation}
...
\begin{equation}
\begin{aligned}\mathbf{y}=\mathbf{U}^{T}\mathbf{z}&=\begin{pmatrix}\mathbf{e}_{1}^{T}\\\mathbf{e}_{2}^{T}\end{pmatrix}\begin{pmatrix}z_{1}\\z_{2}\end{pmatrix}\\&=\begin{pmatrix}-0.851&0.526\\0.526&0.851\end{pmatrix}\begin{pmatrix}z_{1}\\z_{2}\end{pmatrix}\\&=\begin{pmatrix}-0.851z_{1}+0.526z_{2}\\0.526z_{1}+0.851z_{2}\end{pmatrix}\end{aligned}
\end{equation}

1301

From Equation A5.15b,  $ \mathbf{V}_{\mathbf{y}} = \mathbf{A} $ as given in Example A5.3, showing that  $ y_1 $ and  $ y_2 $ are uncorrelated with  $ \sigma^2(y_1) = \lambda_1 = 5.241 $ and  $ \sigma^2(y_2) = \lambda_2 = 0.765 $. Hence, by considering the new coordinate system with

\begin{equation}
y_{1}=\mathbf{e}_{1}^{T}\mathbf{z}=-0.851z_{1}+0.526z_{2}\quad and\quad y_{2}=\mathbf{e}_{2}^{T}\mathbf{z}=0.526z_{1}+0.851z_{2}
\end{equation}
...
\begin{equation}
\begin{aligned}\mathbf{y}=\boldsymbol{\Lambda}^{-1/2}\mathbf{U}^{T}\mathbf{z}&=\begin{pmatrix}{{{1/\sqrt{\lambda_{1}}}}}&{{{0}}} \\{{{0}}}&{{{1/\sqrt{\lambda_{2}}}}}\end{pmatrix}\begin{pmatrix}{{{\mathbf{e}_{1}^{T}}}} \\{{{\mathbf{e}_{2}^{T}}}}\end{pmatrix}\begin{pmatrix}{{{z_{1}}}} \\{{{z_{2}}}}\end{pmatrix}\\&=\begin{pmatrix}{{{1/\sqrt{5.236}}}}&{{{0}}} \\{{{0}}}&{{{1/\sqrt{0.764}}}}\end{pmatrix}\begin{pmatrix}{{{-0.851}}}&{{{0.526}}} \\{{{0.526}}}&{{{0.851}}}\end{pmatrix}\begin{pmatrix}{{{z_{1}}}} \\{{{z_{2}}}}\end{pmatrix}\\&=\begin{pmatrix}{{{-0.372}}}&{{{0.230}}} \\{{{0.602}}}&{{{0.974}}}\end{pmatrix}\begin{pmatrix}{{{z_{1}}}} \\{{{z_{2}}}}\end{pmatrix}\end{aligned}
\end{equation}
...
An alternative set of uncorrelated random variables follows from the Cholesky decomposition, which can be compute in R using the cho1 command. (As an aside, cho1 returns the upper-triangular version of the decomposition, which is simply the transpose of the lower-triangular version). The resulting decomposition is

\begin{equation}
\mathbf{G}=\begin{pmatrix}{{{4}}}&{{{-2}}} \\{{{-2}}}&{{{2}}}\end{pmatrix}=\mathbf{C}\mathbf{C}^{T}=\begin{pmatrix}{{{2}}}&{{{0}}} \\{{{-1}}}&{{{1}}}\end{pmatrix}\begin{pmatrix}{{{2}}}&{{{-1}}} \\{{{0}}}&{{{1}}}\end{pmatrix}
\end{equation}

\begin{equation}
\mathbf{y}=\mathbf{C}^{-1}\mathbf{z}=\begin{pmatrix}{{{0.5}}}&{{{0}}} \\{{{0.5}}}&{{{1}}}\end{pmatrix}\begin{pmatrix}{{{z_{1}}}} \\{{{z_{2}}}}\end{pmatrix}
\end{equation}

yielding

\begin{equation}
y_{1}=z_{1}/2\quad and\quad z_{2}=z_{1}/2+z_{2}
\end{equation}
...
\begin{equation}
\mathbf{P}=\mathbf{U}\boldsymbol{\Lambda}\mathbf{U}^{T}
\end{equation}

where  $ A $ is a diagonal matrix and  $ \mathbf{U}^T\mathbf{U} = \mathbf{U}\mathbf{U}^T = \mathbf{I} $. Defining  $ \mathbf{B} = \mathbf{U}\boldsymbol{\Lambda}^{-1/2} $, Equation A5.15e showed that  $ \mathbf{B}^T\mathbf{P}\mathbf{B} = \mathbf{I} $. Next, note for  $ \mathbf{M} = \mathbf{B}^T\mathbf{G}\mathbf{B} $, that  $ \mathbf{M} = \mathbf{M}^T $ (i.e.,  $ \mathbf{M} $ is symmetric), as

\begin{equation}
\mathbf{M}^{T}=\left(\mathbf{B}^{T}\mathbf{G}\mathbf{B}\right)^{T}=\mathbf{B}^{T}\mathbf{G}^{T}\mathbf{B}=\mathbf{B}^{T}\mathbf{G}\mathbf{B}=\mathbf{M}
\end{equation}

Hence, we can also diagonalize M.

\begin{equation}
\mathbf{C}^{T}\mathbf{M}\mathbf{C}=\mathbf{D}
\end{equation}

where D is a diagonal matrix and  $ \mathbf{C}^T\mathbf{C} = \mathbf{C}\mathbf{C}^T = \mathbf{I} $. Thus,

\begin{equation}
\mathbf{C}^{T}\mathbf{M}\mathbf{C}=\mathbf{C}^{T}\left(\mathbf{B}^{T}\mathbf{G}\mathbf{B}\right)\mathbf{C}=\left(\mathbf{B}\mathbf{C}\right)^{T}\mathbf{G}\left(\mathbf{B}\mathbf{C}\right)=\mathbf{D}
\end{equation}

Defining

\begin{equation}
\mathbf{T}=\mathbf{B}\mathbf{C}=\mathbf{U}\boldsymbol{\Lambda}^{-1/2}\mathbf{C}
\end{equation}

we have from the previous expression that

\begin{equation}
\mathbf{T}^{T}\mathbf{G}\mathbf{T}=\mathbf{D}
\end{equation}

Likewise,

\begin{equation}
\mathbf{T}^{T}\mathbf{P}\mathbf{T}=\left(\mathbf{B}\mathbf{C}\right)^{T}\mathbf{P}\left(\mathbf{B}\mathbf{C}\right)=\mathbf{C}^{T}\left(\mathbf{B}^{T}\mathbf{P}\mathbf{B}\right)\mathbf{C}=\mathbf{C}^{T}\mathbf{C}=\mathbf{I}
\end{equation}

showing that the matrix T satisfies Equation A5.16.

\section{CANONICAL AXES OF QUADRATIC FORMS}
...
where  $ \lambda_i $ and  $ e_i $ are the eigenvalues and associated (normalized, i.e.,  $ \|\mathbf{e}_i\| = 1 $) eigenvectors of A. The new axes defined by the  $ e_i $ vectors are the canonical (or principal)
...
\begin{equation}
\sigma^{2}(y)=a^{2}\sigma^{2}(x_{1})+2ab\sigma(x_{1},x_{2})+b^{2}\sigma^{2}(x_{2})=b^{T}V_{X}b
\end{equation}
...
\section{Implications for the Multivariate Normal Distribution}
Recall the probability density function for the multivariate normal distribution (LW Chapter 8)

\begin{equation}
\begin{align*}\phi(\mathbf{x})=(2\pi)^{-n/2}\left|\mathbf{V}_{\mathbf{x}}\right|^{-1/2}\exp\left[-{1\over2}(\mathbf{x}-\boldsymbol{\mu})^{T}\mathbf{V}_{\mathbf{x}}^{-1}\left(\mathbf{x}-\boldsymbol{\mu}\right)\right]\end{align*}
\end{equation}
...
\section{Principal Components of the Variance-Covariance Matrix}
...
which occurs when  $ \mathbf{c}_1 = \mathbf{e}_1 $ (the normalized eigenvector associated with the leading eigenvalue  $ \lambda_1 $). This vector is the first principal component (often abbreviated as  $ \mathbf{PC1} $), and
...
Example A5.7. Jolicoeur and Mosimann (1960) measured three carapace characters in 24 males of the painted turtle (Chrysemys picta marginata). Letting  $ z_1 $ be the carapace length,  $ z_2 $ be the maximum carapace width, and  $ z_3 $ be the carapace height, the resulting sample covariance matrix ( $ S_z $, the sample estimate of  $ V_z $) for these data was found to be

\begin{equation}
\mathbf{S}_{\mathbf{z}}=\begin{pmatrix}138.77&79.15&37.38\\79.15&50.04&21.65\\37.38&21.65&11.26\end{pmatrix}
\end{equation}

Hence,  $ \mathrm{tr}(\mathbf{S}_{\mathbf{Z}})=138.77+50.04+11.26=200.07 $. Using  $ \mathbf{R} $, the eigenvalues of  $ \mathbf{S}_{\mathbf{Z}} $ are found to be

\begin{equation}
\lambda_{1}=195.280,\qquad\lambda_{2}=3.687,\qquad\lambda_{3}=1.103
\end{equation}

which (as expected) sum to the value of the trace, 200.07. The associated (normalized) eigenvectors are similarly found to be

\begin{equation}
\mathbf{e}_{1}=\begin{pmatrix}0.840\\ 0.492\\ 0.229\end{pmatrix},\qquad\mathbf{e}_{2}=\begin{pmatrix}0.488\\ -0.870\\ 0.079\end{pmatrix},\qquad\mathbf{e}_{3}=\begin{pmatrix}0.213\\ 0.043\\ -0.971\end{pmatrix}
\end{equation}
...
\begin{equation}
y_{1}=\mathbf{e}_{1}^{T}\mathbf{z}=0.840z_{1}+0.492z_{2}+0.229z_{3}
\end{equation}
...
\begin{equation}
y_{2}=\mathbf{e}_{2}^{T}\mathbf{z}=0.488z_{1}-0.870z_{2}+0.079z_{3}
\end{equation}

\begin{equation}
y_{3}=\mathbf{e}_{3}^{T}\mathbf{z}=0.213z_{1}+0.043z_{2}-0.971z_{3}
\end{equation}
...
\section{TESTING FOR MULTIVARIATE NORMALITY}
...
Example A5.8. Consider again the data of Jolicoeur and Mosimann (1960) on carapace characters in 24 male turtles. Are the characters  $ z_1 $ (carapace length) and  $ z_2 $ (maximum carapace width) jointly bivariate normally distributed? Here  $ n = 2 $ and  $ m = 24 $ and

\begin{equation}
\overline{\mathbf{z}}=\left(\begin{array}{l}113.13.\\ 88.29\end{array}\right),\mathbf{S}_{\mathbf{Z}}=\left(\begin{array}{l l}138.77&79.15\\ 79.15&50.04\end{array}\right),\mathbf{S}_{\mathbf{Z}}^{-1}=\left(\begin{array}{c c}0.0737&-0.1165\\ -0.1165&0.2043\end{array}\right)
\end{equation}

where  $ S_{Z} $ is the sample covariance matrix. A partial list of the 24 vectors of observations is

\begin{equation}
\mathbf{z}_{1}=\begin{pmatrix}93\\ 74\end{pmatrix},\quad\cdots,\quad\mathbf{z}_{11}=\begin{pmatrix}113\\ 88\end{pmatrix},\quad\cdots,\quad\mathbf{z}_{24}=\begin{pmatrix}135\\ 106\end{pmatrix}
\end{equation}

Applying Equation A5.23, these observations translate into the distances

\begin{equation}
d_{1}^{2}=4.45,\quad\cdots,\quad d_{11}^{2}=0.002,\quad\cdots,\quad d_{24}^{2}=9.277
\end{equation}
...
\begin{equation}
\chi_{2}^{2}\left(\frac{23-1/2}{24}\right)=\chi_{2}^{2}\left(0.9375\right)
\end{equation}
...
\section{Mardia's Test: Multivariate Skewness and Kurtosis}
...
is (under MVN) a draw from a chi-square distribution with  $ f = 2(2 + 1)(2 + 2)/6 = 4 $ degrees of freedom. Because  $ \text{Prob}(\chi_4^2 \geq 2.717) \simeq 0.606 $, this is not significant. Turning to kurtosis, Equation A5.25b yields
