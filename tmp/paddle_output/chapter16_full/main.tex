...
1. Changes in the Additive Variance}
...
\section{CHANGES IN VARIANCE DUE TO GAMETIC-PHASE DISEQUILIBRIUM}
...
\section{CHANGES IN VARIANCE UNDER THE INFINITESIMAL MODEL}
...
\begin{equation}
\widetilde{d}=\widetilde{h}^{4}\widetilde{\delta}(\sigma_{z}^{2})=(0.6)^{2}(-8.8)=-3.2\quad and\quad\widetilde{\sigma}_{A}^{2}=\widetilde{h}^{2}\widetilde{\sigma}_{z}^{2}=0.6\cdot52.7=31.6
\end{equation}

Assuming the infinitesimal model holds, if selection stops, the additive variance is expected to eventually increase to

\begin{equation}
\sigma_{A}^{2}=\sigma_{a}^{2}=\widetilde{\sigma}_{A}^{2}-\widetilde{d}=31.6+3.2=34.8
\end{equation}
...
\section{Within- and Among-Family Variance Under the Infinitesimal Model}
...
Because $e$ is the residual of a regression, it is uncorrelated with both $A_f$ and $A_m$ (LW Chapters 3 and 8). Computing variances and assuming random mating (so that $A_f$ and $A_m$
...
\begin{equation}
E[a_{i}\cdot a_{j}]=fE[a^{2}]+(1-f)E[a]\cdot E[a]=fE[a^{2}]
\end{equation}

and now

\begin{equation}
E\left[\frac{(a_{j}-a_{i})^{2}}{4}\right]=\frac{E[a^{2}]}{4}+\frac{E[a^{2}]}{4}-f\frac{E[a^{2}]}{2}=(1-f)\frac{E[a^{2}]}{2}
\end{equation}
...
\section{Accounting for Inbreeding and Drift}
...
where  $ \kappa = 1 - \sigma_{z^*}^2 / \sigma_z^2 $ is the fractional reduction in phenotypic variance following selection (Equation 16.10a). When population size is finite, the additive variance in any particular
...
Suppose selection is such that Equation 16.10a is satisfied. We allow for differential selection on the sexes by letting the variance after selection in males and females be  $ \sigma^2(z_m^*) = (1 - \kappa_m)\sigma_z^2 $ and  $ \sigma^2(z_f^*) = (1 - \kappa_f)\sigma_z^2 $, respectively. If parental phenotypes are uncorrelated (i.e., there is random mating),
...
\begin{equation}
\kappa=\frac{2\varphi\left(x_{[1/2+p/2]}\right)x_{[1/2+p/2]}}{p}
\tag{16.11b}
\label{eq:16_11b}
\end{equation}
...
\begin{equation}
\kappa=-\frac{2\varphi\left(x_{[1-p/2]}\right)x_{[1-p/2]}}{p}
\tag{16.11c}
\label{eq:16_11c}
\end{equation}

\begin{equation}
\sigma_{z^{*}(t)}^{2}=\frac{\sigma^{2}\left[z_{f}^{*}(t)\right]}{2}+\frac{\sigma^{2}\left[z_{m}^{*}(t)\right]}{2}=\left(1-\kappa\right)\sigma_{z}^{2}(t)
\tag{16.12a}
\label{eq:16_12a}
\end{equation}
...
\section{Changes in Correlated Characters}
...
\section{Directional Truncation Selection: Theory}
...
Example 16.3. Suppose directional truncation selection is performed (equally on both sexes) on a normally distributed character with  $ \sigma_z^2 = 100 $,  $ h^2 = 0.5 $, and  $ p = 0.20 $ (the upper 20% of the population is saved). To examine the impact of LD on the selection response, we first need to compute the fraction,  $ \kappa $, of phenotypic variance removed by selection. To apply Equation 16.15b, we need to compute both  $ x_{[0.8]} $ and  $ \bar{\tau} $. To do so, note that for a unit normal random variable,  $ U $, that  $ \Pr(U \leq 0.842) = 0.8 $, yielding  $ x_{[0.8]} = 0.842 $, and from Equation 14.3a

\begin{equation}
\bar{\imath}=\varphi(x_{[0.8]}/0.2=\varphi(0.842)/p=0.280/0.20=1.400
\end{equation}
...
\begin{equation}
\kappa=\bar{\imath}\left(\bar{\imath}-x_{[0.8]}\right)=1.400\left(1.400-0.842\right)=0.781
\end{equation}

With  $ \kappa $ in hand, Equation 16.12d becomes

\begin{equation}
d(t+1)=\frac{d(t)}{2}-0.391\frac{\left[50+d(t)\right]^{2}}{100+d(t)}
\end{equation}
...
\begin{equation}
h^{2}(2)=\frac{\sigma_{A}^{2}(2)}{\sigma_{z}^{2}(2)}=\frac{\sigma_{A}^{2}(0)+d(2)}{\sigma_{z}^{2}(0)+d(2)}=\frac{50-11.90}{100-11.90}=0.43
\end{equation}
...
\begin{equation}
\gamma=\frac{2\cdot0.5-1+\sqrt{1+4\cdot0.5\cdot(1-0.5)\cdot0.781}}{2(1+0.781)}=0.3746
\end{equation}
...
\begin{equation}
\tilde{h}^{2}=\frac{\gamma}{1+\gamma-h^{2}}=\frac{0.3746}{1+0.3746-0.5}=0.43
\end{equation}

Again, this matches the value found by iteration.

How does this reduction in  $ \sigma_A^2 $ influence the per-generation change in mean,  $ R(t) $? Because  $ \bar{t} $ is unchanged (being entirely a function of the fraction, p, of adults saved), but  $ h^2 $ and  $ \sigma_z^2 $ change over time, substituting  $ \sigma_A(t) = h(t)\sigma_z(t) $ into Equation 13.6b yields the single-generation response in generation t as

\begin{equation}
R(t)=h^{2}(t)\bar{\imath}\sigma_{z}(t)=1.40h^{2}(t)\sqrt{\sigma_{z}^{2}+d(t)}=1.40h^{2}(t)\sqrt{100+d(t)}
\end{equation}
...
\section{Directional Truncation Selection: Experimental Results}
...
\section{Effects of Epistasis: Does the Griffing Effect Overpower the Bulmer Effect?}
...
\section{Double-Truncation Selection: Theory}
...
\begin{equation}
d(t+1)=\frac{d(t)}{2}\mp0.429\frac{\left[50+d(t)\right]^{2}}{100+d(t)}
\end{equation}
...
\section{Double Truncation Selection: Experimental Results}
...
\section{RESPONSE UNDER NORMALIZING SELECTION}
...
\begin{equation}
d(t+1)=\frac{d(t)}{2}-\frac{1}{2}\frac{[50+d(t)]^{2}}{300+d(t)}
\end{equation}
...
\section{SELECTION WITH ASSORTATIVE MATING}
...
\section{Results Using the Infinitesimal Model}
...
\section{Assortative Mating and Enhanced Response}
...
where  $ R_{rm}(t) = \bar{i} h_{rm}^2(t) \sqrt{\sigma_z^2 + d_{rm}(t)} $ is the selection response in generation t of random mating, and the values of  $ h_{rm}^2(t) $ and  $ d_{rm}(t) $ come from Example 16.3.
...
\section{Disruptive Selection, Assortative Mating, and Reproductive Isolation}
