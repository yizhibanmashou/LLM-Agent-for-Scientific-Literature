...
\title{Using Molecular Data to Detect Selection: Signatures from Multiple Historical Events}
...
\section{BRIEF OVERVIEW OF DIVERGENCE-BASED TESTS}
...
\section{A History of Selection Alters the Ratio of Polymorphic to Divergent Sites}
...
\section{A History of Positive Selection Alters the Ratio of Silent- to Replacement-site Substitution Rates}
...
\section{Divergence-based Tests are Biased Toward Conservative Sites}
...
\section{What Fraction of the Genome is Under Functional Constraints?}
...
\section{THE HKA AND MCDONALD-KREITMAN TESTS}
...
\section{The Hudson-Kreitman-Aguadé (HKA) Test}
...
\begin{equation}
S_{1}^{A}+S_{2}^{A}=9+8=a_{81}(414\cdot\widehat{\theta_{1}}+79\cdot\widehat{\theta_{2}})
\end{equation}

\begin{equation}
D_{1}+D_{2}=210+18=\left(\widehat{\tau}+1\right)\left(4052\cdot\widehat{\theta_{1}}+324\cdot\widehat{\theta_{2}}\right)
\end{equation}

\begin{equation}
D_{1}+S_{1}^{A}=210+9=4052\cdot\widehat{\theta_{1}}\left(\widehat{\tau}+1+a_{81}\right)
\end{equation}

where  $ a_{81} = \sum_{i=1}^{80} 1/i = 4.965 $. The solutions to this system were found to be

\begin{equation}
\widehat{\tau}=6.73,\quad\widehat{\theta_{1}}=6.6\cdot10^{-3},\quad and\quad\widehat{\theta_{2}}=9.0\cdot10^{-3}
\end{equation}
...
three parameters to fit $(\theta_1, \theta_2, \tau)$, which results in a test with one degree of freedom. Because $\Pr(\chi_1^2 > 6.09) = 0.014$, the test indicates a significant departure from the equilibrium neutral model.
...
\section{The McDonald-Kreitman (MK) Test: Basics}
...
\begin{equation}
NI=\frac{P_{a}/D_{a}}{P_{s}/D_{s}}=\frac{16/38}{2/30}=6.42
\end{equation}
...
\begin{equation}
NI=\frac{P_{a}/D_{a}}{P_{s}/D_{s}}=\frac{14,311/20,467}{15,750/34,099}=1.514
\end{equation}
...
\section{The McDonald-Kreitman Test: Caveats}
...
\begin{equation}
NI\simeq\frac{1+\beta K}{1+q(\delta^{\beta}-1)}
\end{equation}
...
\section{Dominance in Fitness and the MK Test}
...
\section{Fluctuating Selection Coefficients and MK Tests}
...
\section{Recombinational Bias in Extended MK Tests}
...
\section{ESTIMATING PARAMETERS OF ADAPTIVE EVOLUTION}
...
\section{Estimating the Fraction,  $ \alpha $, of Substitutions That are Adaptive}
...
\begin{equation}
\sigma^{2}(\alpha)=E[\alpha^{2}]-\overline{\alpha}^{2}=q\alpha_{*}^{2}-q^{2}\alpha_{*}^{2}=\alpha_{*}q(1-q)
\end{equation}
...
\begin{equation}
\overline{\alpha}+\sigma^{2}(\alpha)=0.1+\left[0.2^{2}\cdot0.5(1-0.5)\right]=0.11
\end{equation}
...
\section{How Common Are Adaptive Substitutions?}
...
\section{Estimating the Rate,  $ \lambda $, of Adaptive Substitutions}
...
\begin{equation}
\lambda=\frac{0.10\cdot0.008}{14\cdot10^{6}}=5.7\cdot10^{-11}per site,per year
\end{equation}

Assuming a generation time of 25 years, this corresponds to a rate of  $ 2.3 \cdot 10^{-12} $ per site, per generation.

As a point of comparison, Andolfatto (2007) contrasted X chromosome genes in Drosophila melanogaster (for polymorphism data) and D. simulans (as the outgroup for divergence). The estimated  $ \alpha $ was 0.5, while  $ d_a = 0.028 $, and  $ t = 10^7 $ generations, yielding

\begin{equation}
\lambda=\frac{0.50\cdot0.028}{2\cdot10^{7}}=7.0\cdot10^{-10}per site,per generation
\end{equation}
...
\section{THE SAWYER-HARTL POISSON RANDOM FIELD MODEL}
...
\section{Basic Structure of the Model}
...
As a brief aside, we mentioned above that certain maximum-likelihood versions of the basic MK test use a distribution of fitness effects (often denoted as DFE in the literature),  $ \varphi(s|\Delta) $, where  $ \Delta $ denotes the vector of distribution parameters (such as the  $ \alpha $ and  $ \beta $ parameters for a gamma distribution; see Equation A2.25a) (Bierne and Eyre-Walker 2004; Eyre-Walker et al. 2006; Welch 2006; Boyko et al. 2008; Eyre-Walker and Keightley 2009; Keightley and Eyre-Walker 2012). Under this model, the expected SFS becomes
...
\begin{equation}
\alpha=\frac{2\cdot267.11\cdot0.08}{(2\cdot267.11\cdot0.08)+0.172}=0.996
\end{equation}
...
\begin{equation}
\alpha=\frac{2\gamma}{2\gamma+\left(p_{0}/p_{b}\right)}=\frac{20}{20+0.985/0.015}=0.23
\end{equation}
...
\section{INSIGHT Analysis of Human Transcription Factors}
...
\section{PHYLOGENY-BASED DIVERGENCE TESTS}
...
\section{Parsimony-based Ancestral Reconstruction Tests}
...
\section{Maximum-likelihood-based Codon Tests}
...
\section{Bayesian Estimators of Sites Under Positive Selection}
...
where $D$ is the pattern of codons for that site in the tree, and the prior $\operatorname{Pr}(\text{class } i)$—the values for $p_0$, $p_b$, and $p_d$—is estimated by maximum likelihood. The case of interest is whether the codon belongs to the class of advantageous sites, $\operatorname{Pr}(\omega > 1 \mid D)$,
...
\section{CONNECTING THE PARAMETERS OF ADAPTIVE EVOLUTION}
...
385
 ...
 $ \gamma $ The scaled strength of selection,  $ 2N_{e,s} $
 ...
 $ \gamma = \frac{\omega - f}{2p_{b}} = \frac{\omega - p_{0}}{2p_{b}} $ (Equation 10.25b)
...
\section{THE SEARCH FOR SELECTION: CLOSING COMMENTS}
...
\section{Caution is in Order When Declaring Positive Selection}
...
\section{Curbing Our Enthusiasm}
