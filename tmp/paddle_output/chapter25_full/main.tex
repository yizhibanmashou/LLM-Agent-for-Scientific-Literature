...
\section{IDEALIZED LONG-TERM RESPONSE IN A LARGE POPULATION}
...
\section{Expected Contribution From a Single Locus}
...
\section{Dudley's Estimators of a, n, and  $ p_{0} $}
...
Example 25.2. The idealized response curves in Figure 25.1 were generated using Equation 25.3 to compute the expected allele-frequency change at each locus, assuming there is no gametic-phase disequilibrium. We assumed complete additivity ($k = 0$ and no epistasis), that $\sigma_E^2 = 100$, and $n$ identical loci underlying the character. Equation 25.3 yields

\begin{equation}
\Delta p_{t}=\frac{a\bar{\imath}p_{t}(1-p_{t})}{\sigma_{z}(t)}=\frac{a\bar{\imath}p_{t}(1-p_{t})}{\sqrt{\sigma_{A}^{2}(t)+\sigma_{E}^{2}}}\simeq\frac{a\bar{\imath}p_{t}(1-p_{t})}{\sqrt{2na^{2}p_{t}(1-p_{t})+100}}
\end{equation}

Strictly speaking, the last expression is an approximation, albeit a close one, as  $ 2na^2p_t(1-p_t) $ is the genic variance,  $ \sigma_a^2(t) $, at generation  $ t $, while the additive genetic variance equals the genic

919

variance plus the disequilibrium contribution,  $ \sigma_A^2(t) = \sigma_a^2(t) + d(t) $, as discussed in Chapters 16 and 24. Iteration generates the response curves shown in the figure.
...
\begin{equation}
t_{0.5}=t_{0.75,0.5}\simeq\frac{1}{s}\ln\left(\frac{\left(3/4\right)\left(1-\left[1/2\right]\right)}{\left[1/2\right]\left(1-\left[3/4\right]\right)}\right)=\left(\frac{\sigma_{z}}{a\bar{\imath}}\right)\ln(3)=\frac{\sqrt{n}}{\bar{\imath}}\ln(3)
\end{equation}
...
the probability of observing the most extreme genotype (BB at all loci) in the base population is  $ (1/4)^{25} \simeq 10^{-15} $

\section{MAJOR GENES VERSUS POLYGENIC RESPONSE: THEORY}
...
\section{Lande's Model: Response With a Major Gene in an Infinitesimal Background}
...
\begin{equation}
W(z)=\left\{\begin{aligned}1&\quad for z\geq T\\ 0&\quad for z<T\end{aligned}\right.
\end{equation}

The marginal fitnesses become

\begin{equation}
\overline{W}_{i}=\int_{T}^{\infty}\varphi(z,\mu+\alpha_{i},\sigma^{2})d z=\Pr\left[U>\left(T^{*}-\frac{\alpha_{i}}{\sigma}\right)\right]
\end{equation}
...
\begin{equation}
\begin{aligned}\overline{W}=q=(1-p)^{2}\Pr(U>T^{*})+2p(1-p)(1-s_{1})\Pr\left[U>\left(T^{*}-\frac{\alpha_{1}}{\sigma}\right)\right]\\+p^{2}(1-s_{2})\Pr\left[U>\left(T^{*}-\frac{\alpha_{2}}{\sigma}\right)\right]\end{aligned}
\end{equation}
...
\begin{equation}
\mu_{s_{i}}=\mu_{i}+\sigma\frac{\varphi(T,\mu+\alpha_{i},\sigma^{2})}{\Pr[U>(T^{*}-\alpha_{i}/\sigma)]}
\end{equation}

implying a directional selection differential of

\begin{equation}
S_{i}=\mu_{s_{i}}-\mu_{i}=\sigma\frac{\varphi(T,\mu+\alpha_{i},\sigma^{2})}{\Pr[U>(T^{*}-\alpha_{i}/\sigma)]}
\end{equation}
...
\section{MAJOR GENES VERSUS POLYGENIC RESPONSE: DATA}
...
\section{Major Genes Appear to Be Important in Response to Anthropogenically Induced Selection}
...
\section{What is the Genetic Architecture of Response in Long-term Selection Experiments?}
...
\section{Estimating Selection Limits and Half-lives}
...
\section{General Features of Long-term Selection Experiments}
...
\section{3. Reproductive fitness usually declines as selection proceeds.}
...
\section{The Nature of Selection Limits}
...
\section{INCREASES IN VARIANCES AND ACCELERATED RESPONSES}
...
\section{Scale and Environmental Impacts on Variances}
...
\section{CONFLICTS BETWEEN NATURAL AND ARTIFICIAL SELECTION}
...
\section{Accumulation of Lethals in Selected Lines}
...
\begin{equation}
\begin{aligned}z&=\mathrm{freq}^{\prime}(Bb)p_{Bb}(z)+\mathrm{freq}^{\prime}(bb)p_{bb}(z)\\&=\left(\frac{2p}{1+p}\right)p_{Bb}(z)+\left(\frac{1-p}{1+p}\right)p_{bb}(z)\end{aligned}
\end{equation}
...
\begin{equation}
q=\left(\frac{2p}{1+p}\right)\Pr(z_{Bb}>T)+\left(\frac{1-p}{1+p}\right)\Pr(z_{bb}>T)
\end{equation}
...
\begin{equation}
\mathrm{freq}^{\prime\prime}(Bb)=\left(\frac{2p}{1+p}\right)\frac{\Pr(U>T^{*}-a/\sigma)}{q}
\end{equation}
...
If we let  $ \hat{p} $ denote the equilibrium frequency of B in the zygotes at the start of the next generation (before natural selection), by rearranging Equation 25.14, it follows that

\begin{equation}
q(1+p)p^{\prime\prime}=p\Pr(U>T^{*}-a/\sigma)
\end{equation}
...
Because  $ p'' = p = \widehat{p} $ at equilibrium, this reduces to

\begin{equation}
q\left(1+\widehat{p}\right)=\Pr(U>T^{*}-a/\sigma)
\end{equation}

Combining this result with Equation 25.13 yields

\begin{equation}
2\widehat{p}\operatorname{Pr}\left(U>T^{*}-a/\sigma\right)+\left(1-\widehat{p}\right)\operatorname{Pr}(U>T^{*})=\operatorname{Pr}(U>T^{*}-a/\sigma)
\end{equation}
...
\section{Lerner's Model of Genetic Homeostasis}
...
the fitnesses under directional selection are approximately  $ 1 - \bar{\imath} a / \sigma_z : 1 : 1 + \bar{\imath} a / \sigma_z $, resulting in (assuming weak selection; e.g.  $ s_1, s_2, \bar{\imath} a / \sigma \ll 1 $) approximate total fitnesses of
...
\section{Artificial Selection Countered by Natural Stabilizing Selection}
