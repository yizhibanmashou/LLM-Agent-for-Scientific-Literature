...
def _find_dotenv(start: Optional[Path] = None) -> Optional[Path]:
    """Find the project-level ``.env`` file.

    The repository root ``.env`` is the single source of local secrets.  The
    optional upward search remains only as a fallback for tests and external
    reuse.
    """
