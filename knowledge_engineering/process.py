...
Usage:
    python knowledge_engineering/process.py \
...
def strip_latex_markup(tex_text: str) -> str:
    ...
    if len(body.strip()) < 200:
        title_payload = extract_command_payload(tex_text, "title")
        if len(title_payload.strip()) > len(body.strip()):
            text = title_payload
        else:
            text = body
    else:
...
def clear_directory(path: str) -> int:
    """Recursively clear all children under path while preserving the directory itself."""
...
def _normalize_formula_reference_id(reference: str) -> str:
    ...
    if reference.startswith("formula_"):
        return reference[len("formula_") :]
    return reference


def _formula_sort_key(reference: str) -> tuple[int, int, str]:
    label = _normalize_formula_reference_id(reference)
...
def sort_formula_reference_ids(references: Iterable[str]) -> list[str]:
    ...
    for reference in references:
        normalized = _normalize_formula_reference_id(reference)
...
def _table_reference_key(chapter_name: str, table_id: str) -> str:
    chapter = str(chapter_name or "").strip().lower()
    table = str(table_id or "").strip()
    if not chapter or not table:
        return ""
...
def sort_table_reference_keys(reference_keys: Iterable[str]) -> list[str]:
    ...
    def _key_sort(item: str) -> tuple:
        parts = item.split(":", 1)
        if len(parts) != 2:
            return (_chapter_reference_sort_key(""), _table_sort_key(""), item)
        chapter_name, table_id = parts
        return (_chapter_reference_sort_key(chapter_name), _table_sort_key(table_id), item)
...
def sort_table_reference_ids(references: Iterable[str]) -> list[str]:
    ...
    for reference in references:
        ...
        if value and value not in seen:
            seen.append(value)
    return sorted(seen, key=_table_sort_key)
...
def _extract_rows_from_html_table(table_html: str) -> list[list[str]]:
    rows: list[list[str]] = []
    for row_match in HTML_TABLE_ROW_PATTERN.finditer(str(table_html or "")):
        row_cells: list[str] = []
        for cell_match in HTML_TABLE_CELL_PATTERN.finditer(row_match.group("body")):
            cell_text = html.unescape(cell_match.group("body") or "")
...
def _extract_tables_from_paddle_raw(tex_path: str, chapter_name: str) -> list[TableEntry]:
    ...
    for page_index, page_payload in enumerate(pages, start=1):
        ...
        for row in rows:
            ...
            if not content:
                continue
            page_blocks.append({"label": label, "content": content, "bbox": bbox})
        ...
        for table_block in table_blocks:
            ...
            if best_caption is None and isinstance(fallback_caption, dict):
                best_caption = fallback_caption
...
def _numeric_dict_delta(before: dict[str, Any], after: dict[str, Any]) -> dict[str, int]:
...
def _clamp_float(raw: Any, *, default: float, minimum: float = 0.0, maximum: float = 1.0) -> float:
    try:
        value = float(raw)
    except (TypeError, ValueError):
        value = default
...
def _parse_chapter_allowlist(raw: str, fallback: set[str]) -> set[str]:
...
def _chapter_in_allowlist(chapter_name: str, allowlist: set[str]) -> bool:
...
def _resolve_effective_llm_phase(chapter_name: str, llm_policy: dict[str, Any]) -> int:
    ...
    if base_phase >= 3:
        ...
        if _chapter_in_allowlist(chapter, phase3_chapters):
            return 3
    if base_phase >= 2:
        phase2_chapters = llm_policy.get("phase2_chapters", set())
        if _chapter_in_allowlist(chapter, phase2_chapters):
            return 2
    return 1
